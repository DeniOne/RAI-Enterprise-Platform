...
| Module complexity | 38+ модулей, высокий coupling | **Частично закрыто / growth-governance + последовательные boundary-refactor срезы введены** | `apps/api/src/modules/` содержит 38 top-level доменных директорий, `packages/prisma-client/schema.prisma`, `scripts/architecture-budget-gate.cjs`, `scripts/architecture-budgets.json`, `docs/05_OPERATIONS/DEVELOPMENT_GUIDELINES/ARCHITECTURE_GROWTH_GUARDRAILS.md`, `apps/api/src/shared/front-office/front-office-threading.service.ts`, `apps/api/src/shared/front-office/front-office-communication.repository.ts`, `apps/api/src/shared/front-office/front-office-outbound.service.ts`, `apps/api/src/shared/rai-chat/agent-interaction-contracts.ts`, `apps/api/src/shared/rai-chat/rai-chat.dto.ts`, `apps/api/src/shared/rai-chat/rai-tools.types.ts`, `apps/api/src/shared/rai-chat/rai-chat-widgets.types.ts`, `apps/api/src/shared/rai-chat/response-composer-presenters.ts`, `apps/api/src/shared/rai-chat/execution-adapter-heuristics.ts`, `apps/api/src/shared/rai-chat/crm-tool-schemas.ts`, `apps/api/src/shared/rai-chat/crm-tool-helpers.ts`, `apps/api/src/shared/rai-chat/front-office-routing.policy.ts`, `apps/api/src/shared/rai-chat/front-office-tool-schemas.ts`, `apps/api/src/shared/rai-chat/front-office-tool-helpers.ts`, `apps/api/src/shared/rai-chat/contracts-tool-schemas.ts`, `apps/api/src/shared/rai-chat/contracts-tool-helpers.ts`, `apps/api/src/shared/rai-chat/rai-tools-builtins.ts`, `apps/api/src/shared/rai-chat/rai-tools-log-helpers.ts`, `apps/api/src/shared/rai-chat/intent-router.types.ts`, `apps/api/src/shared/rai-chat/runtime-governance-policy.types.ts`, `apps/api/src/shared/rai-chat/explainable-result.types.ts`, `apps/api/src/shared/rai-chat/security/agent-config-blocked.error.ts`, `apps/api/src/shared/rai-chat/security/budget-exceeded.error.ts`, `apps/api/src/shared/rai-chat/security/risk-policy-blocked.error.ts`, `apps/api/src/shared/rai-chat/security/security-violation.error.ts`, `apps/api/src/modules/rai-chat/tools/crm-tools.registry.ts`, `apps/api/src/modules/rai-chat/tools/front-office-tools.registry.ts`, `apps/api/src/modules/rai-chat/tools/contracts-tools.registry.ts`, `apps/api/src/modules/rai-chat/tools/rai-tools.registry.ts`, `apps/api/src/modules/rai-chat/runtime/runtime-governance-control.service.ts`, `apps/api/src/modules/rai-chat/runtime/agent-runtime.service.ts`, `apps/api/src/modules/rai-chat/supervisor-forensics.service.ts`, `apps/api/src/modules/rai-chat/supervisor-agent.service.ts`, `apps/api/src/modules/tech-map/tech-map.service.ts`, `apps/api/src/shared/tech-map/tech-map-mapping.helpers.ts`, `apps/api/src/shared/tech-map/tech-map-mapping.helpers.spec.ts`, `apps/api/src/shared/tech-map/tech-map-prisma-includes.ts`, `apps/api/src/shared/tech-map/dto/approval.dto.ts`, `apps/api/src/shared/tech-map/dto/crop-zone.dto.ts`, `apps/api/src/modules/consulting/consulting.controller.ts`, `apps/api/src/modules/finance-economy/economy/application/economy.service.ts`, `apps/api/src/shared/finance-economy/economy-ingest.helpers.ts`, `apps/api/src/modules/finance-economy/ofs/application/decision-intelligence.service.ts`, `apps/api/src/shared/finance-economy/decision-intelligence.helpers.ts`, `apps/api/src/shared/finance-economy/decision-intelligence.types.ts`, `apps/api/src/shared/finance-economy/contracts/finance-ingest.contract.ts`, `apps/api/src/shared/finance-economy/config/finance-config.module.ts`, `apps/api/src/shared/finance-economy/config/finance-config.service.ts`, `apps/api/src/modules/commerce/services/party.service.ts`, `apps/api/src/modules/commerce/services/asset-role.service.ts`, `apps/api/src/shared/commerce/party.helpers.ts`, `apps/api/src/shared/commerce/asset-role.helpers.ts`, `apps/api/src/shared/commerce/dto/create-party.dto.ts`, `apps/api/src/shared/commerce/dto/create-jurisdiction.dto.ts`, `apps/api/src/shared/commerce/dto/create-regulatory-profile.dto.ts`, `apps/api/src/shared/consulting/dto/complete-operation.dto.ts`, `apps/api/src/shared/consulting/dto/create-harvest-plan.dto.ts`, `apps/api/src/shared/consulting/dto/save-harvest-result.dto.ts`, `apps/api/src/shared/consulting/dto/transition-plan-status.dto.ts`, `apps/api/src/shared/consulting/dto/update-draft-plan.dto.ts`, `apps/api/src/shared/explainability/agent-config.dto.ts`, `apps/api/src/shared/explainability/dto/autonomy-status.dto.ts`, `apps/api/src/shared/generative-engine/contradiction/counterfactual-engine.ts`, `apps/api/src/shared/generative-engine/contradiction/conflict-explainability-builder.ts`, `apps/api/src/shared/generative-engine/risk/risk-metric-calculator.ts`, `apps/api/src/shared/generative-engine/yield/input-data-snapshot.ts` | Сложность остаётся высокой, но growth-gate уже работает, а уже выполнены последовательные boundary-refactor разрезы по `rai-chat/tech-map/consulting/finance-economy/commerce/explainability/generative-engine`; tranche по строковым и file-count budget-warning для watch-модулей закрыт (`rai-chat=28260/134`, `consulting=4613/35`, `finance-economy=4303/37`, `commerce=3324/31`), а структурный долг теперь сконцентрирован в лимитах `schema.prisma + top-level modules` и non-budgeted hotspots (`advisory/front-office/integrity`) |
...
- выполнен четвёртый практический boundary-refactor: CRM/Commerce presenter-layer вынесен из `response-composer.service.ts` в `apps/api/src/shared/rai-chat/response-composer-presenters.ts`; размер `rai-chat` по `architecture-gate` дополнительно снижен с `29605` до `28777` строк;
- выполнен пятый практический boundary-refactor: execution heuristics/mapping слой вынесен из `agent-execution-adapter.service.ts` в `apps/api/src/shared/rai-chat/execution-adapter-heuristics.ts`; размер `rai-chat` по `architecture-gate` дополнительно снижен с `28777` до `28410` строк.
- выполнен шестой deep-slice `runtime-governance event/control`: из `agent-runtime.service.ts` вынесен отдельный governance control-layer (`runtime-governance-control.service.ts`); `AgentRuntimeService` сокращён с `659` до `498` строк, runtime тестовый контур остаётся зелёным.
- выполнен седьмой deep-slice `supervisor orchestration`: forensic/audit post-processing вынесен из `supervisor-agent.service.ts` в `supervisor-forensics.service.ts`; `SupervisorAgent` оставлен orchestration-only, а supervisor/runtime/rai-chat spec-контуры синхронизированы под новый provider.
- выполнен восьмой deep-slice `tool registries`: из `crm-tools.registry.ts` и `front-office-tools.registry.ts` вынесены policy/schemas/helpers в `apps/api/src/shared/rai-chat/*`; в `modules/rai-chat/tools/front-office-routing.policy.ts` оставлен re-export bridge.
- выполнен девятый deep-slice `tool registries`: из `contracts-tools.registry.ts` вынесены payload schemas и mapping/json helper-слой в `apps/api/src/shared/rai-chat/contracts-tool-schemas.ts` и `contracts-tool-helpers.ts`.
- выполнен десятый deep-slice `tool orchestration`: из `rai-tools.registry.ts` вынесены built-in schemas/handlers (`echo_message`, `workspace_snapshot`) и log payload helper в `apps/api/src/shared/rai-chat/rai-tools-builtins.ts` и `rai-tools-log-helpers.ts`.
- выполнен одиннадцатый deep-slice `tech-map mapping/snapshot`: из `tech-map.service.ts` вынесены mapping функции для `ValidationInput`, DAG nodes и activation snapshots в `apps/api/src/shared/tech-map/tech-map-mapping.helpers.ts`.
- выполнен двенадцатый deep-slice `tech-map include-layer`: повторяющиеся Prisma include-деревья вынесены из `tech-map.service.ts` в `apps/api/src/shared/tech-map/tech-map-prisma-includes.ts`.
- выполнен тринадцатый deep-slice `consulting controller context`: повторяющаяся сборка `UserContext`/execution context вынесена в `toUserContext()` и `toExecutionContext()`; `consulting.controller.ts` сокращён с `467` до `367` строк.
- выполнен четырнадцатый deep-slice `finance-economy OFS decision intelligence helpers`: validation/driver composition/reason mapping/lever normalization/scenario mapping вынесены из `decision-intelligence.service.ts` в `apps/api/src/shared/finance-economy/decision-intelligence.helpers.ts`.
- выполнен пятнадцатый deep-slice `finance-economy ingest/types helpers`: helper-слой ingest replay/integrity вынесен из `economy.service.ts` в `apps/api/src/shared/finance-economy/economy-ingest.helpers.ts`, а типы decision-intelligence вынесены в `apps/api/src/shared/finance-economy/decision-intelligence.types.ts`.
- выполнен шестнадцатый deep-slice `commerce party/asset helpers`: relation/rules helper-слой вынесен в `apps/api/src/shared/commerce/party.helpers.ts`, а asset-role mapping/overlap/type-detection helper-слой — в `apps/api/src/shared/commerce/asset-role.helpers.ts`.
- выполнен семнадцатый deep-slice `tech-map dto`: `approval.dto.ts` и `crop-zone.dto.ts` вынесены из module-boundary в `apps/api/src/shared/tech-map/dto`.
- выполнен восемнадцатый deep-slice `explainability dto`: `agent-config.dto` и `autonomy-status.dto` вынесены в `apps/api/src/shared/explainability`, module-bridge удалён.
- выполнен девятнадцатый deep-slice `generative-engine contradiction/risk/yield`: module-bridge для `counterfactual-engine` и `conflict-explainability-builder` удалён, а `risk-metric-calculator.ts` и `yield/input-data-snapshot.ts` вынесены в `apps/api/src/shared/generative-engine/*`.
- стабилизирован runtime/supervisor/tools test-контур после extraction-срезов: закрыт DI-долг `agent-runtime/runtime-spine` спеков (`ContractsAgent`, `RuntimeGovernanceEventService`, `Crm/FrontOffice/ContractsToolsRegistry`, `RuntimeGovernanceFeatureFlagsService`) и обновлены устаревшие assertions по memory-profile/context в `supervisor/rai-chat` спеках; текущий snapshot `rai-chat` в `architecture-gate` — `28122` строк.
- deep-slices по `tech-map` дали измеримый эффект: размер модуля снижен по `architecture-gate` с `6087` до `6020`, затем до `5941`, затем до `5888` строк; file-count также снижен до `59` (warn-порог снят).
- deep-slices по `consulting`, `finance-economy` и `commerce` дали измеримый эффект: размеры модулей дополнительно снижены по `architecture-gate` до `4613/35`, `4303/37` и `3324/31` (последовательно с `4763/40`, `4417/41` и `3599/34`), file-count warnings для всех трёх модулей сняты.
- deep-slices по `explainability` и `generative-engine` закрыли как строковый, так и file-count pressure для этих двух hotspots: `explainability` снижен до `8089` строк и `41` файла (было `8136/43`), `generative-engine` — до `6743` строк и `69` файлов (было `6869/73`).
...
- текущий tranche по `Module complexity` теперь логически закрыт по строковым budget-warning для `rai-chat + explainability + generative-engine + tech-map + consulting + finance-economy + commerce`;
- следующий инженерный шаг здесь — не ещё один guard-script, а следующая волна структурного упрощения: структурные лимиты `schema.prisma/top-level modules` и дальнейшая нормализация non-budgeted hotspots.
...
1. **Архитектурное упрощение схемы и модулей**
   ...
   - growth-governance для схемы и модулей уже введён, и уже сделаны девятнадцать boundary-refactor срезов (дополнительно закрыты `tech-map dto`, `explainability dto` и `generative-engine contradiction/risk/yield` shared-срезы);
   - следующий крупный выигрыш теперь лежит в structural debt wave: лимиты `schema.prisma/top-level modules` и non-budgeted hotspots, а не в закрытии базовых foundation-gap.
...
- `pnpm gate:architecture` -> growth-report показывает `schema.prisma=6107`, `top-level modules=38`; после дополнительной file-count wave `rai-chat=28260/134`, `consulting=4613/35`, `finance-economy=4303/37`, `commerce=3324/31`, `tech-map=5888/59`, `explainability=8089/41` и `generative-engine=6743/69` вышли ниже watch-module file-warn порогов
...
- четвёртый boundary-refactor выполнен на `rai-chat` response-composer presenter-слое (ещё один измеримый шаг снижения размера модуля);
- пятый boundary-refactor выполнен на `rai-chat` execution-adapter heuristics слое (ещё один измеримый шаг снижения размера модуля);
- шестой deep-slice выполнен на `rai-chat` runtime-governance event/control слое (`agent-runtime` + `runtime-governance-control`);
- седьмой deep-slice выполнен на `rai-chat` supervisor forensics слое (`supervisor-agent` + `supervisor-forensics`);
- восьмой deep-slice выполнен на `rai-chat` tool registries слое (`crm-tools` + `front-office-tools` с выносом policy/schemas/helpers в shared boundary);
- девятый deep-slice выполнен на `rai-chat` contracts registry слое (`contracts-tools` с выносом schemas/helpers в shared boundary);
- десятый deep-slice выполнен на `rai-chat` orchestration слое (`rai-tools.registry` с выносом built-ins/log helpers в shared boundary);
- одиннадцатый deep-slice выполнен на `tech-map` mapping/snapshot слое (`tech-map.service` с выносом mapping helpers в shared boundary);
- двенадцатый deep-slice выполнен на `tech-map` include-layer (`tech-map.service` с выносом Prisma include-деревьев в shared boundary);
- тринадцатый deep-slice выполнен на `consulting` controller-context слое (`consulting.controller.ts` с выделением `toUserContext()` и `toExecutionContext()`);
- четырнадцатый deep-slice выполнен на `finance-economy` decision-intelligence helper-слое (`decision-intelligence.service.ts` с выносом в `shared/finance-economy/decision-intelligence.helpers.ts`);
- пятнадцатый deep-slice выполнен на `finance-economy` ingest/types helper-слое (`economy.service.ts` + `decision-intelligence.service.ts` с выносом в `shared/finance-economy/economy-ingest.helpers.ts` и `decision-intelligence.types.ts`);
- шестнадцатый deep-slice выполнен на `commerce` party/asset helper-слое (`party.service.ts` + `asset-role.service.ts` с выносом в `shared/commerce/party.helpers.ts` и `asset-role.helpers.ts`);
- текущий tranche по `Module complexity` для `rai-chat + explainability + generative-engine + tech-map + consulting + finance-economy + commerce` логически закрыт по строковым и file-count budget-warning; следующий фокус — structural debt (`schema.prisma/top-level modules` и non-budgeted hotspots) плюс постоянный discipline-control над частично закрытыми foundation-блоками.
