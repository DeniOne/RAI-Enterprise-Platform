...
- [x] Выполнен `prisma generate` через локальный бинарник Prisma (`packages/prisma-client/node_modules/.bin/prisma.cmd`).
- [x] Снят блокер `api` typecheck (`TS2347` в `crm.service.ts`) — `apps/api tsc --noEmit` проходит.
- [x] Модуль приведён к `memory-bank/LANGUAGE_POLICY.md`: пользовательские тексты `exploration` и Swagger-описания переведены на русский язык.
- [x] Обновлены документы: `docs/07_EXECUTION/PROMPT_EXPLORATION_MODULE.md`, `docs/10_FRONTEND_MENU_IMPLEMENTATION/00_MASTER_MENU_MAP.md`.
- [~] Повторные `apps/api` e2e-прогоны в текущей сессии ограничены инфраструктурным timeout раннера.
...
- [x] **Consulting Navigation**: Полный рефакторинг `Sidebar.tsx` и `navigation-policy.ts` под каноничную модель.
...
- [x] **CMR Domain**: Внедрен трёхсторонний механизм согласования (`DeviationReview`) с FSM.
...
- [x] **Event-driven Projection**: `EmployeeProfile` переведен на модель проекции внешних факторов. Исключены PII (имена), внедрен `orgUnitId` для управленческого контекста.
...
- [x] **Compliance Signaling**: Движок `ComplianceEngine` для автоматического расчёта статуса (без блокировки).
...
- [x] Реализация Basic Economics (Связка активов с техкартой и активацией)
...
- [x] **Explanation Layer**: Слой "ПОЧЕМУ" интегрирован во всю вью для интерпретации состояний.
...
- [x] **Settlement Guard**: Любое расчётное событие обязано иметь ледже-проекцию (атомарно).
...
- [x] **Prisma Client**: Регенерация клиента с учётом изменений схем для Контура 1.
...
- [x] **DB Guard Enforcement**: Атомарная валидация метаданных транзакций и контроль платёжеспособности (no negative cash).
...
- [x] **Burn Rate Logic**: Формализация расчёта операционного расхода (Operating avg).
...
- [x] **KPI Read-Model**: `KpiService` с расчётом ROI, Delta и себестоимости.
...
- [x] **Settlement Guard**: Внедрен блокирующий инвариант: расчётные события (`SETTLEMENT`) обязаны порождать проводки.
...
- [x] **Tail Risk (P05)**: Реализован формальный расчёт P05 (Probability of Collapse) для R3 рисков.
- [x] **Audit & Liability**: Внедрено тегирование ответственности (`LiabilityTag`) и неизменяемый аудит всех блокировок.
- [x] **Safety Hardening**: Строгое разделение режимов регулирования, исключающее "тихий" захват контроля системой.
...
- [x] Added explicit click-path mapping (block -> route) for all clickable elements
