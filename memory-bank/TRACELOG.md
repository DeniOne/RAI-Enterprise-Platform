[2026-04-02 18:27Z] Добавлен тестовый пакет реального хозяйства «Казьминский» для ручного пути до `TechMap`
- В `apps/api/test/real-farms/kazminskiy/` создан полный тестовый пакет под сценарий:
  - хозяйство `СПК колхоз племзавод «Казьминский»`
  - поле `KZM-01`
  - площадь около `1000 га`
  - культура `озимый рапс`
  - сезон `2026/2027`
- Пакет включает:
  - регистрационный пакет хозяйства
  - паспорт тестового поля
  - территориальный агро-метео baseline
  - синтетический, но правдоподобный контекст хозяйства
  - готовые тексты для внутреннего setup-чата
  - готовые тексты для внешнего гл. агронома хозяйства в `front-office`
  - пошаговый ручной `end-to-end` тест от регистрации до генерации техкарты
- Важные логические решения:
  - гл. агроном хозяйства жёстко отделён от внутреннего `chief_agronomist`;
  - сбор контекста для техкарты моделируется через `gap-driven clarification`, а не через полную анкету;
  - поле намеренно сделано крупным и неоднородным, чтобы тестировать не демо-сценарий, а реалистичный производственный контур.
- Практический эффект:
  - у команды появился готовый пакет для ручного боевого теста на реальном хозяйстве, которого ещё нет в базе;
  - все участники прогона получают единый источник тестовых предпосылок и сообщений;
  - тест можно проводить без дополнительного придумывания поля, агрохимии, техники и базового климатического контекста.
...
- Зарегистрирован `CLAIM-ARCH-RAPESEED-INT-001` в `DOCS_MATRIX.md`. Пройдены строгие проверки линтера `lint:docs:matrix:strict`.
- Практический эффект: определён контракт, инварианты и трёхфазный non-breaking сценарий расширения существующего production-ready Engine новыми агрономическими правилами.
[2026-04-01 12:04Z] Синхронизация репозитория: выполнен git pull
- Получены обновления в `docs/00_STRATEGY/TECHMAP/`.
- Добавлены новые исследования по канонической техкарте и параметрическому ядру.
- Практический эффект: локальная копия актуализирована, новые стратегические доки по TechMap доступны для работы.
...
[2026-04-02 10:35Z] Добавлены два новых брифа по историческим данным для хозяйства и ядра ИИ

- В доменный слой добавлены:
  - `docs/02_DOMAINS/AGRO_DOMAIN/TECHMAP_HISTORICAL_FARM_DATA_BRIEF.md`
  - `docs/02_DOMAINS/AGRO_DOMAIN/TECHMAP_AI_CORE_HISTORICAL_DATA_BRIEF.md`
- Первый документ фиксирует состав исторических данных от хозяйства для индивидуального анализа и построения индивидуальной базы:
  - история культур;
  - история урожайности;
  - операции;
  - агрохимия;
  ...
  - ресурсы;
  - экономика;
  - история исполнения техкарт.
- Второй документ фиксирует состав исторических данных для обучения ядра ИИ:
  - научные и нормативные данные;
  - массовая статистика;
  - региональные агрометео и оперативные сводки;
  - агрохимия и почвенные обследования;
  - сортоиспытания;
  - обезличенные хозяйственные и полевые наборы;
  - обязательные требования к происхождению, агрегированию и правам использования.
- Практический эффект:
  - теперь разделены два разных контура данных: индивидуальная база хозяйства и обучающий корпус ядра ИИ;
  - команда может отдельно собирать `farm intelligence` и `AI core training corpus`, не смешивая частные хозяйственные данные с межрегиональным обучающим слоем.

[2026-04-02 11:10Z] Брифы техкарт и исторических данных переведены в плотный табличный формат
...
- Что изменено по смыслу:
  - документы больше не являются в основном описательными;
  - они переведены в рабочий формат реестров факторов;
  - для каждого фактора теперь задана единица измерения, формат значения, обязательность и практическое назначение;
  - исторические и обучающие брифы теперь явно включают технику, погодные ограничения, окна операций и факторы влияния на урожай.
- Практический эффект:
  - документы можно использовать как реальные таблицы-опросники и шаблоны загрузки данных;
  - при сборе данных у хозяйства и внешних институтов теперь меньше неоднозначности, какие именно величины и в каком виде нужны системе.

[2026-04-02 11:35Z] Добавлен экспорт табличных шаблонов для techmap-брифов

- Добавлен скрипт `scripts/export-techmap-brief-templates.cjs`.
- Скрипт:
  - читает markdown-таблицы из четырёх доменных документов по техкартам;
  - объединяет строки в плоские табличные шаблоны;
  - добавляет служебные колонки `Фактическое значение`, `Источник / файл`, `Дата источника`, `Комментарий`;
  - сохраняет результат в `docs/02_DOMAINS/AGRO_DOMAIN/TEMPLATES/` в формате `CSV` с `UTF-8 BOM` и разделителем `;`, пригодным для открытия в Excel.
- Практический эффект:
  - брифы можно сразу отправлять хозяйствам и внешним поставщикам данных как рабочие таблицы;
  - шаблоны стали воспроизводимыми и не требуют ручной перекладки из markdown в spreadsheet.

[2026-04-02 12:05Z] Пользовательский текст в techmap-брифах и шаблонах переведён на русский
...
- Обновлены документы:
  ...
  - `docs/02_DOMAINS/AGRO_DOMAIN/TECHMAP_HISTORICAL_FARM_DATA_BRIEF.md`
  - `docs/02_DOMAINS/AGRO_DOMAIN/TECHMAP_AI_CORE_HISTORICAL_DATA_BRIEF.md`
- Из человекочитаемых колонок и пояснений убраны англоязычные формулировки:
  - `availability`
  - `growth regulator`
  - `branch selection`
  - `risk model`
  ...
  - `auditability`
  - `reproducibility`
  - и аналогичные описательные подписи.
- Технические идентификаторы и канонические поля в обратных кавычках сохранены как технические обозначения.
- После изменения документы должны использоваться как полностью русскоязычный пользовательский контур, а `CSV`-шаблоны должны регенерироваться из тех же русских описаний.
[2026-04-02 12:18Z] Реестры и CSV-шаблоны techmap-документов дочищаются до полностью русского пользовательского слоя; англоязычные подписи в таблицах заменяются на русские, при сохранении технических идентификаторов в backticks.
[2026-04-02 17:58Z] Экспорт techmap-брифов расширен: кроме CSV теперь генерируются XLSX-шаблоны с двумя листами — «Факторы» и «Инструкция». Лист «Инструкция» содержит правила заполнения, критичность `P0/P1/P2`, требования к источнику и формату значений.
