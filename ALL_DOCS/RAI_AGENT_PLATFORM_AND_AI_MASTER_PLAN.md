...
>
> Активное дополнение к master-plan:
...
| Связка адаптера для future-role | Подтверждена: non-canonical roles используют explicit `executionAdapterRole`, проходят eval/governance и реально исполняются через `/api/rai/chat` без новой hardcoded runtime branch. |
...
| Interactive clarification overlay | Подтверждён: reusable clarification/window pattern уже проходит через `/api/rai/chat`, frontend store и live HTTP smoke как минимум для `Агроном-А -> tech_map_draft` и `Экономист-А -> compute_plan_fact`. |
| Unified AI Dock + widget migration | Подтверждён: левый dock пересобран в `header -> signals -> history -> composer`, legacy `widgets[]` автоматически мигрируются в `workWindows[]`, а richer output families (`structured_result`, `related_signals`, `comparison`) реально рендерятся через единый overlay protocol. |
...
- исполнение adapter-bound future-role и подтверждение control-plane.

Итог: **Stage 2 build-out платформы в основном завершён**.

Дополнительное уточнение по product ownership wave:

- `crm_agent` уже работает как canonical CRM owner-agent;
- `front_office_agent` уже работает как canonical first-wave ingress owner-agent;
- `contracts_agent` уже поднят как canonical commerce owner-agent для договоров, обязательств, fulfillment, invoices, payments и AR balance.

Дополнительное уточнение после handoff-closeout:
- будущие роли больше не являются «иллюзией только на уровне control-plane»;
- для non-canonical role действует governed contract:
  - onboarding manifest обязан нести `runtimeProfile.executionAdapterRole`;
  - eval/governance используют этот binding;
  - runtime исполняет через существующий canonical adapter;
  - `agents/config` показывает binding как часть эффективной runtime-истины.

Дополнительное уточнение по interactive UX слою:
- первый reusable `clarification loop` уже не является только blueprint-идеей;
- для `agronomist / tech_map_draft` и `economist / compute_plan_fact` реализованы:
  - `pendingClarification`
  - `workWindows[]`
  - `context_acquisition`
  - `context_hint`
  - режимы `inline / panel / takeover`
  - стек свёрнутых окон
  - `category / priority`
  - `parentWindowId / relatedWindowIds`
  - cross-window action `focus_window`
  - route actions обратно в продуктовый workflow
  - auto-resume после добора обязательного контекста
- этот slice подтверждён:
  - targeted backend specs
  - targeted web specs
  - live HTTP smoke на initial clarification paths
  - live HTTP smoke на completed resume paths
...
5. У агентов ещё не оформлена first-class фокусная зона ответственности как отдельный platform contract.
   - Сейчас специализация в значительной степени держится на route/regex/tool-intent эвристике.
   - Следующий продуктово-архитектурный слой вынесен в отдельное дополнение:
     [RAI_AGENT_PLATFORM_AND_AI_MASTER_PLAN_ADDENDUM_AGENT_FOCUS_AND_CONTEXT.md](/root/RAI_EP/docs/00_STRATEGY/STAGE%202/RAI_AGENT_PLATFORM_AND_AI_MASTER_PLAN_ADDENDUM_AGENT_FOCUS_AND_CONTEXT.md)
   - Там зафиксированы обязательные сущности следующего этапа:
     `Agent Focus Contract`, `Intent Catalog`, `Required Context Contract`, `UI Action Surface Contract`.
   - Первый runtime-backed slice этого слоя уже начат в коде:
     - единый contract source подключён к `IntentRouter`
     - `clarificationResume` больше не собирается отдельными ad-hoc ветками
     - clarification/windows для `agronomist` и `economist` строятся от общих contract definitions
   - Но platform-wide покрытие для всех agent families ещё не завершено.

6. Interactive agent UX реализован уже не как один спец-кейс, а как первый reusable platform pattern.
   - `AgronomAgent / tech_map_draft`, `EconomistAgent / compute_plan_fact`, `KnowledgeAgent / query_knowledge` и `MonitoringAgent / emit_alerts` уже живые.
   - Общий window manager, widget migration, pinned windows и richer output families уже доведены до platform-wide состояния для reference agents Stage 2.
...
| `RAI_AGENT_PLATFORM_AND_AI_MASTER_PLAN_ADDENDUM_AGENT_FOCUS_AND_CONTEXT.md` | `ACTIVE ADDENDUM` | Дополнение к master-plan по фокусным зонам ответственности агентов, intent-каталогу и context contracts. |
| `RAI_AGENT_DOMAIN_OWNERSHIP_MAP.md` | `ACTIVE MAP` | Карта доменов платформы, owner-агентов, intent-owner и handoff paths. |
| `RAI_AGENT_EVOLUTION_AND_LIFECYCLE.md` | `ACTIVE CANON` | Канон жизненного цикла агентов: onboarding, promotion, canary, freeze, rollback, retirement, versioning. |
...
- [x] Ввести каноническую `AgentDefinition`.
- [x] Ввести `AgentRuntimeProfile`.
- [x] Ввести `AgentAutonomyMode`.
- [x] Ввести `AgentMemoryPolicy`.
- [x] Ввести `AgentCapabilityPolicy`.
- [x] Ввести `AgentConnectorBinding`.
- [x] Ввести канонический `AgentOutputContract`.
- [x] Зафиксировать audit contract для agent execution.
- [x] Зафиксировать memory scope matrix.
- [x] Ввести generic LLM gateway через OpenRouter.

### Phase C — MVP-4 Functional AI
- [x] Довести `AgronomAgent` до реального AI-backed expert agent.
- [x] Довести `EconomistAgent` до реального AI-backed economics agent.
- [x] Довести `KnowledgeAgent` до реального RAG synthesis agent.
- [x] Довести `MonitoringAgent` до governed AI-backed monitoring agent.
- [x] Перевести runtime к каноническому `hybrid agent-first` режиму.
- [x] Зафиксировать backend-first contract, по которому можно проектировать бэкофис.

### Phase D — Expansion Layer
- [x] Описать manifest-путь для новых агентов.
- [x] Описать connector policy для новых доменов.
- [x] Описать low-friction onboarding path для новых агентов.
- [x] Подготовить platform contract для будущих:
  - marketer
  - strategist
  - finance advisor
  - legal advisor
...
- [x] Зафиксировать template structure для новых агентов.
- [x] Зафиксировать registration path.
...
Статус: `в основном закрыто` для live `/api/rai/chat` smoke по `agronomist`, `economist`, `knowledge`, `monitoring`.
...
Статус: `закрыто` на уровне kernel types, registry/runtime contracts, OpenRouter gateway, persisted bindings и adapter-based runtime dispatch.
...
- `marketer` уже проходит `onboarding/validate -> change request -> canary -> promote -> persisted config`.
- Следующий незакрытый слой: исполнение future role через `/api/rai/chat` без добавления новой hardcoded runtime ветки.
