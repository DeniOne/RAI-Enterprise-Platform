...
> Состояние репозитория: `2026-03-20`
>
...
## Подтвержденные функциональные срезы
...
- `integrity`
- `level-f/gateway`, `level-f/crypto`, `level-f/worm`, `level-f/snapshot`, `level-f/certification`
...
| --- | --- |
| Backend | NestJS 10, REST c global prefix `/api`, Swagger в non-production, GraphQL через Apollo, BullMQ, EventEmitter |
...
Примечание: API сейчас стартует на `4000` в [`apps/api/src/main.ts`](apps/api/src/main.ts), поэтому любые старые упоминания `3001` считаются устаревшими.
...
## Команды, на которые можно опираться
...
| `pnpm lint:docs:matrix:strict` | Жесткая проверка docs matrix и версий |
...
Документация в репозитории переведена в docs-as-code режим. Точки входа:
...
Если `README`, отдельный документ и код спорят между собой, правильный порядок решения спора такой:
...
Этот файл должен отвечать только на четыре вопроса:
