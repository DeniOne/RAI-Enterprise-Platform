...
| Privacy/legal контур был только архивным выводом | есть active operator/privacy register, transborder/deployment matrix, subject-rights runbook, external evidence request packet, metadata register и acceptance runbook | новые docs в `docs/05_OPERATIONS` | legal backlog больше не разрозненный и переведён в criteria-driven closeout queue с owner-routing |
