...
| Privacy/legal контур был только архивным выводом | есть active operator/privacy register, transborder/deployment matrix, subject-rights runbook и external evidence request packet | новые docs в `docs/05_OPERATIONS` | legal backlog больше не разрозненный и переведён в criteria-driven closeout packet |
...
- Улучшилось: runtime quality baseline, invariant hygiene, reproducible audit/secret/schema/license/SBOM controls, active ops/compliance packet, explicit legal closeout packet, expanded CODEOWNERS.
