...
| Privacy/legal контур был только архивным выводом | есть active operator/privacy register, transborder/deployment matrix, subject-rights runbook, external evidence request packet, metadata register, acceptance runbook, intake command, lifecycle transition command и reproducible status gate | новые docs в `docs/05_OPERATIONS` + `scripts/legal-evidence-*.cjs` | legal backlog больше не разрозненный и переведён в criteria-driven closeout queue с owner-routing и full lifecycle machine-check |
