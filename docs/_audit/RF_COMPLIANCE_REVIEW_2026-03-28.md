...
Причина: теперь в проекте уже есть активный legal/privacy packet, отдельный external evidence request packet, seeded metadata register, acceptance runbook и машинный verdict-report, но ключевые внешние доказательства всё ещё отсутствуют:
...
`pnpm legal:evidence:verdict` на текущем baseline детерминированно подтверждает тот же результат: `NO-GO`.
...
- Создан active acceptance runbook с named owners и verdict-upgrade criteria.
- Добавлен машинный verdict report с автоматическим расчётом blockers до `CONDITIONAL GO` и `GO`.
...
|---|---|---|---|---|---|---|
| 152-ФЗ `О персональных данных` | `применимо` | operator roles, lawful basis, цели, состав данных, меры защиты | active privacy register, external evidence request packet, metadata register, acceptance runbook, verdict report, data-flow map, subject-rights runbook, code-backed PII controls | юридическое лицо оператора и final lawful basis pack не подтверждены | юридический + организационный + технический | прикрепить реальные артефакты к `reference_id` из `docs/05_OPERATIONS/EXTERNAL_LEGAL_EVIDENCE_METADATA_REGISTER.md`, прогнать через acceptance runbook и пересчитать verdict |
...
Собрать внешний пакет подтверждений по `docs/05_OPERATIONS/EXTERNAL_LEGAL_EVIDENCE_REQUEST_PACKET.md`, seeded `docs/05_OPERATIONS/EXTERNAL_LEGAL_EVIDENCE_METADATA_REGISTER.md` и `docs/05_OPERATIONS/WORKFLOWS/EXTERNAL_LEGAL_EVIDENCE_ACCEPTANCE_RUNBOOK.md`, затем пересчитать `pnpm legal:evidence:verdict`:
- кто оператор;
