...
| `front_office` | входящие и исходящие сообщения, диалоги, классификация общения, task/process detection, эскалации | `front-office`, `telegram`, `task`, `advisory`, `/front-office` | `front_office_agent` | `knowledge` | `crm_agent`, `personal_assistant` | `front_office_agent`, `knowledge`, `crm_agent` по клиентскому контексту | `front_office_agent`, `crm_agent`, `personal_assistant` | communicator log, thread state, escalation/task records своего домена; outbound replies только через governed communication layer | `READ_ONLY_SUPPORT` | `HIGH` | `CANONICAL FIRST WAVE` | Owner коммуникационного ingress уже реализован как canonical role первой волны; safe informational direct replies хозяйству разрешены через owner-agent и orchestration spine, а responsible actions остаются human-owned. |
...
- `front_office_agent` уже замыкает production ingress для safe informational replies, но legal/template routing и часть future-role handoff всё ещё не production-enabled.
