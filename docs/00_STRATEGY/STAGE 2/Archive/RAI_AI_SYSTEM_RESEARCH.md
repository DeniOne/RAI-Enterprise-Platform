...
> **Изменения v2:** Добавлены секции 13-15 (Deterministic Core API, Observability Gaps, PII/Compliance Analysis)
...
**⚠️ Архитектурная проблема (v2):** SupervisorAgent смешивает 5 ответственностей в одном классе: маршрутизацию (IntentRouter), управление жизненным циклом агентов (AgentRuntime), управление бюджетом токенов (BudgetController), координацию памяти (MemoryCoordinator) и композицию ответов (ResponseComposer). При росте системы это станет точкой отказа и регрессий.
...
**⚠️ Архитектурная проблема (v2):** Единый плоский реестр. При росте до 20+ инструментов превратится в свалку. Отсутствует capability-based access (агент видит ВСЕ тулы, а не только свои).
...
## 13. Инвентаризация детерминированного ядра (Deterministic Core)

> **v2 — Дополнение №6:** Фиксация границы «что считает код, а что — LLM»

### 13.1 Существующие калькуляторы

В `apps/api/src/modules/tech-map/calculators/` уже реализованы:

| Калькулятор | Метод | Формула / Логика |
|------------|-------|-----------------|
| `SeedingRateCalculator` | `compute(params)` | `(targetPlantDensity / fieldGermination) × thousandSeedWeight / 1000` |
| `FertilizerDoseCalculator` | `compute(params)` | Балансовый метод: `(targetYield × unitRemoval - soilAvailable) / utilizationCoefficient` |
| `GDDWindowCalculator` | `compute(params)` | GDD-аккумуляция: `Σmax(0, Tavg - Tbase)`, окно `[startGDD, endGDD]` |

### 13.2 Существующие валидаторы

| Валидатор | Класс | Назначение |
|-----------|-------|-----------|
| `DAGValidationService` | Service | Топологическая сортировка + CPM критический путь |
| `TechMapValidationEngine` | Service | 7 классов ошибок (HARD_STOP / WARNING) |
| `TankMixCompatibilityService` | Service | Проверка совместимости баковых смесей |
| `AgroEventsMustValidator` | Guard | MUST-gate валидация перед commit |

### 13.3 Gap-анализ: что отсутствует

| Функция | Статус | Необходимость |
|---------|--------|--------------|
| `validate_pesticide_dose(registry, dose)` | ❌ Нет | Критично — проверка по справочнику регистрации |
| `compute_bbch_window(hybrid, gddAccumulated)` | Частично (`HybridPhenologyService`) | Нужен `explain` |
| `compute_water_balance(precipitation, evapotranspiration)` | ❌ Нет | Для irrigation scheduling |
| Единый фасад `AgroDeterministicEngine` | ❌ Нет | Сейчас калькуляторы разрозненные |
| `explain` в ответах калькуляторов | ❌ Нет | Для explainability UI и аудита |

### 13.4 Рекомендация

Собрать все калькуляторы и валидаторы в единый фасад `AgroDeterministicEngine`:

```typescript
interface AgroDeterministicEngine {
  computeSeedingRate(params): ExplainableResult<SeedingRate>;
  computeFertilizerDose(params): ExplainableResult<FertilizerDose>;
  computeGDDWindow(params): ExplainableResult<GDDWindow>;
  computeBBCHStage(hybrid, gddAccumulated): ExplainableResult<BBCHStage>;
  validatePesticideDose(registry, dose): ExplainableResult<ValidationVerdict>;
  validateTankMix(mix): ExplainableResult<CompatibilityVerdict>;
  validateDAG(operations): ExplainableResult<DAGVerdict>;
}

interface ExplainableResult<T> {
  value: T;
  explain: {
    formula: string;           // Формула или алгоритм
    inputParams: Record<string, unknown>; // Входные параметры
    constraints: string[];     // Ограничения
    source: string;            // Источник данных/нормативов
  };
}
```
...
## 14. Gap-анализ AI-наблюдаемости (Observability Gaps)

> **v2 — Дополнение №10:** Связка AI traces с GenerativeEngine / Explainability

### 14.1 Текущее состояние трассировки

| Слой | Трассировка | Проблема |
|------|------------|----------|
| `SupervisorAgent` | `traceId` в актор-контексте | Есть, но не передаётся в GenerativeEngine |
| `RaiToolsRegistry` | Forensic-логирование всех вызовов | Есть, но не связано с `GenerationRecord` |
| `GenerativeEngine` | `GenerationRecordService` записывает поколения | Есть, но нет `traceId` из AI-контекста |
| `ExplainabilityModule` | Level 1/2/3 объяснения | Есть, но не привязано к tool calls |
| `AuditLog` | Общий аудит-лог | Есть, но нет AI-специфичной аналитики |

### 14.2 Целевая модель: единая трассировка

```
traceId ДОЛЖЕН связывать:
  ├── SupervisorAgent.orchestrate (intent, agent selection)
  ├── Agent.think (LLM call, tokensUsed, confidence)
  ├── RaiToolsRegistry.execute[] (tool calls с latency)
  ├── GenerationRecord (если агент генерировал artефакт)
  ├── ExplainabilityPanel (Level 1/2/3 для пользователя)
  ├── DivergenceRecord (если Level C обнаружил расхождение)
  └── AuditLog entry (финальная запись)
```

### 14.3 Разрыв: Agent Evaluation

Система **не имеет** контура оценки качества агентов. Нет:
- `AgentScoreCard` — агрегированные метрики по агенту (correction-rate, rejection-reasons, unsafe-suggestion-rate, tool-failure-rate, hallucination-flag-rate)
- `GoldenTestSet` — эталонные E2E-сценарии для регрессионного тестирования промтов
- `PromptVersion → EvalRun` — привязка метрик качества к конкретной версии промта/модели
- Процесс `PromptChange RFC` — формальный процесс изменения промтов (цель → риск → eval → rollout plan → rollback)

---

## 15. Gap-анализ PII и compliance (Sensitive Data)

> **v2 — Дополнение №7:** Идентификация точек утечки и требований к маскированию

### 15.1 Что проходит через AI-контекст

| Тип данных | Где появляется | Уровень риска |
|-----------|---------------|--------------|
| `companyId` | Каждый запрос | Низкий (UUID) |
| ФИО агронома | MemoryProfile, UserProfile | Средний (ПДн) |
| ИНН компании | Финансовые данные через tool calls | Высокий (реквизиты) |
| Банковские реквизиты | Через EconomyService (теоретически) | Критический |
| Паспортные данные | HR-модуль (PulseSurvey context) | Критический |
| Координаты полей | Field.geometry (GeoJSON) | Средний (коммерческая тайна) |
| API-ключи | Переменные окружения | Критический (но изолированы) |
| Номера телефонов | CRM, HR | Высокий (ПДн) |

### 15.2 Текущие защиты

- ✅ `companyId` enforcement в `RaiToolsRegistry`
- ✅ `Memory Isolation` по `companyId + userId`
- ✅ Denylist секретов в Memory Adapter
- ❌ **Нет формальной `SensitiveDataPolicy`** — нет определения «что считается PII»
- ❌ **Нет output-фильтрации** — ответы AI не проходят через PII-маскер
- ❌ **Нет unit-тестов** на маскирование
- ❌ **Нет красной команды** — нет набора prompt-injection / exfiltration тестов

### 15.3 Рекомендация

Реализовать `SensitiveDataPolicy`:
1. **Справочник PII-паттернов:** ИНН (12 цифр), Р/С (20 цифр), паспорт (серия-номер), телефон (+7xxx), email
2. **Output-фильтр:** Regex-маскер на выходе каждого агента
3. **Red-team набор:** 20+ промт-инъекций + exfiltration-паттернов → автоматические тесты
4. **Периодический аудит:** Ежемесячная проверка логов на утечки
