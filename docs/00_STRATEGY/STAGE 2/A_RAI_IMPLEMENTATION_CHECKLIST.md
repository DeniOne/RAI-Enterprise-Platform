...
- [x] **Cost Decomposition & Workload Hotspots** — отслеживание расхода бюджета (LLM vs DB) и самых "дорогих/долгих" сессий. | **AG-ARAI-F4-013** | DONE
...
- [x] **Governance Counters** — счетчики Tenant Isolation Sentinel (кросс-тенант попытки) и SensitiveDataFilter. | **AG-ARAI-F4-011** | DONE
...
## 🏁 Финализация Phase 4 (Текущий фокус) [DONE]
1. [x] **Agent Registry & Configurator UI (F4.18)** — DONE.
2. [x] **Governance & Security Control UI (F4.19)** — DONE.
3. [x] **E2E Валидация всей Фазы 4** — DONE.
   **Эффект:** Инфраструктура доверия (Trust Infrastructure) полностью готова к эксплуатации. Проверена связка "Инцидент → Алерт → Трейс → Реплей → Резолв".