...
## TM-UX — Сквозной продуктовый поток TechMap

**Цель**: Сшить путь `агент / карточка поля / сезон / план / техкарта / execution` в единый штатный UX без backstage-экранов.

### TM-UX.1 Временный маршрут проверки
- [x] Добавлен временный backstage-маршрут `/consulting/techmaps/new` для честной проверки реального backend-flow.
- [x] После создания поля выполняется прямой переход в этот маршрут с сохранением контекста `accountId + fieldId`.

### TM-UX.2 Удаление временного маршрута
- [ ] Перенести основной вход `Подготовить техкарту` в штатные экраны поля/сезона/плана и в сценарий агента.
- [ ] Проверить, что агент по созданию техкарт проводит пользователя по цепочке без отдельного backstage-мастера.
- [ ] Удалить `/consulting/techmaps/new` после подтверждения, что UX сшит в единый поток.

Ожидаемый эффект: проверочный маршрут не закрепится как постоянный обходной UX, а продукт перейдёт на естественный бизнес-процесс с агентом и рабочими окнами.
