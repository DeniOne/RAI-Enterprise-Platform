...
Цель: единый исполняемый контур работы для двух сред (Cursor ↔ Antigravity) через общую папку в репозитории.
...
Структура и правила определены в:
- `interagency/README.md`
- `interagency/INDEX.md`
- `interagency/templates/*`

### Алгоритм исполнения (жёстко, без интерпретаций)
1) **Получение задачи**
   - Оркестратор берёт входные задачи ТОЛЬКО из `interagency/prompts/`.
   - USER кладёт новый промт в `interagency/prompts/YYYY-MM-DD_<slug>.md`.
   - Оркестратор обязан открыть и прочитать промт целиком перед любыми действиями.

2) **Планирование**
   - На каждый промт оркестратор обязан создать план:
     - путь: `interagency/plans/YYYY-MM-DD_<slug>.md`
     - формат: `interagency/templates/PLAN_TEMPLATE.md`
   - План обязан содержать: границы, риски, DoD, порядок работ.

3) **Исполнение**
   - CODER реализует задачу строго по плану и промту.
   - Любые отклонения от промта/DoD фиксируются в отчёте как “не получилось / почему”.

4) **Отчёт**
   - По завершении работы оркестратор обязан создать отчёт:
     - путь: `interagency/reports/YYYY-MM-DD_<slug>.md`
     - формат: `interagency/templates/REPORT_TEMPLATE.md`
   - Отчёт обязан содержать (минимум):
     - что было целью,
     - что сделано (факты),
     - изменённые файлы (список),
     - проверки/прогоны (что запускали и результат),
     - что сломалось/не получилось,
     - следующий шаг.

5) **Обновление индекса**
   - Оркестратор обязан обновить `interagency/INDEX.md`:
     - добавить ссылку на промт,
     - добавить ссылку на план,
     - добавить ссылку на отчёт,
     - пометить статус (active/done).

### Запреты (блокирующие)
- Запрещено выполнять задачи, которые не оформлены файлом в `interagency/prompts/`.
- Запрещено начинать реализацию без плана в `interagency/plans/`.
- Запрещено объявлять задачу “готовой” без отчёта в `interagency/reports/` и без проверок из тест-плана промта.
