...
3) **Исполнение (Antigravity)**
   - CODER реализует задачу по плану/промту
   - затем выполняет `REVIEW PACKET PROMPT` и ставит `READY_FOR_REVIEW` в `interagency/INDEX.md`

4) **Ревью и финализация (Cursor)**
   - Cursor-агент выполняет `REVIEW & FINALIZE PROMPT`
   - только после APPROVED разрешены: чеклисты, memory-bank, commit, push (если USER сказал "пушь")
...
7. CODER реализует и подготавливает пакет на ревью (см. `docs/ANTIGRAVITY SOFTWARE FACTORY — REVIEW PACKET PROMPT.md`)
8. TECHLEAD/ревьюер (Cursor) проверяет и финализирует (см. `docs/CURSOR SOFTWARE FACTORY — REVIEW & FINALIZE PROMPT.md`)
...
**Правило:** Checkbox marked = только после внешнего ревью (APPROVED) и зафиксированной интеграции в git (commit; push — если разрешён USER).
