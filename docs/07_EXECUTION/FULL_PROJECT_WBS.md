...
- [x] **[Backend]** **Episodes/Profile Runtime Integration (S5.5)**: `MemoryEpisode` и `MemoryProfile` реально пишутся/читаются; `SupervisorAgent` использует profile context.
Evidence: `apps/api/src/shared/memory/default-memory-adapter.service.ts`, `apps/api/src/modules/rai-chat/supervisor-agent.service.ts`, `interagency/reports/2026-03-03_s5-5_episodes-profile-integration.md`
