...
Канон терминов и границ сущностей: `docs/01_ARCHITECTURE/PARTY_ACCOUNT_MODEL_CANON.md`.
...
5. Маршрут `/consulting/techmaps/new` считать только backstage-инструментом проверки UX-сшивки, а не постоянной продуктовой точкой входа.
...
### 8.3.1 Временный backstage-маршрут

- `/consulting/techmaps/new` разрешён только как временный проверочный экран, пока проход `агент -> карточка сущности -> техкарта` не сшит в один штатный UX.
- Критерий удаления: агент умеет провести пользователя по подготовке техкарты, а карточки поля/сезона/плана содержат естественные входы без отдельного мастера.

Ожидаемый эффект: временная сборка помогает быстро выявить разрывы UI, но не превращается в постоянный обходной продуктовый путь.
