...
**Фазы 4, 5 и 6 закрыты.** API Gateway готов.

**Остаток по архитектуре перед Pilot:**
1. **HSM Integration**: Подключить реальный HashiCorp Vault / AWS KMS вместо локальных Ed25519 ключей.
2. **M-of-N Governance**: Докрутить мультисиг (например, 3-of-7) для подтверждения `Panic Halt` или смены весов.
3. **Deployment**: Накатить NGINX Docker на боевые серваки.
