...
> - [ ] `GovernanceBar` корректно адаптируется под флаги `Authority`.
> - [ ] В компонентах отсутствуют проверки ролей (только флаги полномочий).
> - [ ] `DomainTree` фильтруется на основе `capability`.
> - [ ] Механизм переключения Mock-ролей работает корректно.
...
> - [ ] `EscalationBanner` используется во всех точках эскалации.
> - [ ] Прогресс кворума визуально понятен и обновляется.
> - [ ] Флоу подписи нельзя обойти (Bypass-proof).
> - [ ] `Override` требует обязательного ввода причины (Reason).
> - [ ] Флаг `canSign` корректно влияет на доступность кнопок подписи.

---

## PHASE 4: Risk Triage & Causal Loops
**Срок:** Неделя 4  
**Цель:** Слой аналитики рисков и кросс-доменных связей.

### Задачи:
*   **4.1 Risk Stratification (R1–R4):**
