...
Документ фиксирует профиль будущего `front_office_agent` как owner-agent для коммуникационного ingress-контура.
...
`front_office_agent` должен стать отдельным owner-agent для:
