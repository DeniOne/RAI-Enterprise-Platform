...
## Контракт Плановых Claim-Документов
- Не каждый `claim` описывает текущее runtime-поведение.
- Активные strategy / business / frontend / execution / instruction-документы тоже могут быть `claim-managed`, если они являются каноническими точками входа, governance-источниками или обязательными плановыми документами.
- Для таких документов `verified_by: manual` допустим и означает: документ проверен как действующий источник intent, навигации или planning-логики.
- Такой `claim` подтверждает роль документа в системе знаний, но не заменяет сверку runtime-тезисов по `code/tests/gates`.
...
## Каноническое Активное Планирование
- Если документ нужен агенту и человеку как обязательная действующая точка входа, его нужно переводить в `claim-managed` статус даже тогда, когда он описывает будущую реализацию, а не уже подтверждённый код.
- Это нужно, чтобы активные планы, governance и карты системы не терялись между operational docs и архивом.
