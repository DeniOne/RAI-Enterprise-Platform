...
async function main() {
  ...
  const rollout = await call(token, "/api/advisory/rollout/status");
  const ops = await call(token, "/api/advisory/ops/metrics?windowHours=24");
  const killSwitch = await call(token, "/api/advisory/incident/kill-switch");
...
main().catch((err) => {
  console.error(`[advisory-s3-monitoring] ${err.message}`);
  process.exit(1);
});
