...
import { Season, RapeseedType } from '@prisma/client';
...
export class SeasonBusinessRulesService {
    ...
    async validateRapeseedSeason(season: any): Promise<void> {
        const rapeseed = await this.prisma.rapeseed.findUnique({
            where: { id: season.rapeseedId }
        });
        ...
        await this.validateCropRotation(season.fieldId, season.year, season.rapeseedId);
    }

    /**
     * Validates sowing dates for Rapeseed.
     */
    validateRapeseedSeasonDates(startDate: Date | null, type: RapeseedType): void {
        if (!startDate) return;
     ...
     */
    async validateCropRotation(fieldId: string, currentYear: number, rapeseedId: string): Promise<void> {
        ...
        if (previousSeasons.length > 0) {
            ...
            this._logViolation(message, { fieldId, currentYear, previousYears: previousSeasons.map(s => s.year) });
            throw new BadRequestException(message);
        }
    }
    ...
    validateYieldTarget(expectedYield: number | null): void {
        if (expectedYield && (expectedYield < 1 || expectedYield > 6)) {
            throw new BadRequestException('Целевая урожайность рапса должна быть в диапазоне 1-6 т/га');
        }
    }
}
