...
describe('TaskService', () => {
    ...
    describe('createTasksFromSeason (Idempotency)', () => {
        it('should create tasks if they do not exist', async () => {
            ...
            const result = await service.createTasksFromSeason('season-1', 'company-1');
        ...
        it('should throw if season is locked', async () => {
            ...
            await expect(service.createTasksFromSeason('season-1', 'company-1'))
    ...
    describe('Task Lifecycle & Guards', () => {
        ...
        it('should transition PENDING -> IN_PROGRESS', async () => {
            prismaMock.task.update.mockResolvedValue({ ...mockTask, status: TaskStatus.IN_PROGRESS });

            const result = await service.startTask('task-1', mockUser, 'company-1');

            expect(result.status).toBe(TaskStatus.IN_PROGRESS);
            expect(auditMock.log).toHaveBeenCalledWith('TASK_STARTED', expect.anything(), expect.anything());
        });
        ...
        it('should block COMPLETED -> IN_PROGRESS (Terminal guard)', async () => {
            ...
            await expect(service.startTask('task-1', mockUser, 'company-1'))
                .rejects.toThrow(BadRequestException);
        });
        ...
        it('should enforce tenant isolation', async () => {
            ...
            await expect(service.startTask('task-1', mockUser, 'wrong-company'))
