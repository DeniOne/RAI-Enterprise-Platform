        ...
        for (const run of runningRuns) {
            ...
            try {
                ...
                if (status !== MLJobStatus.RUNNING) {
                    this.logger.log(`📍 Syncing status for ${run.id}: RUNNING -> ${status}`);
                ...
                if (e.response && e.response.statusCode === 404) {
                    this.logger.error(`Job ${jobName} not found in K8s. Marking TrainingRun ${run.id} as QUARANTINED.`);
                    await this.prisma.trainingRun.update({
                        where: { id: run.id },
                    ...
                    this.logger.warn(`⚠️ Could not reconcile job ${jobName}: ${e.message}`);
                }
            }
        }
    }

    async createTrainingJob(companyId: string, trainingRunId: string, image: string, envVars: Record<string, string>) {
        ...
        if (monthlyUsage >= MAX_MONTHLY_RUNS) {
            this.logger.error(`💸 Tenant ${companyId} exceeded monthly training budget (${monthlyUsage}/${MAX_MONTHLY_RUNS})`);
            throw new InternalServerErrorException('Monthly training budget exceeded for this tenant.');
        }

        // 2. Проверка Global Concurrency Cap
        const activeJobsStr = await this.redis.getClient().get('rai:total_active_jobs');
        const activeJobs = parseInt(activeJobsStr || '0');
        if (activeJobs >= this.MAX_CONCURRENT_JOBS) {
            this.logger.warn(`⏳ Global Job Quota exceeded (${this.MAX_CONCURRENT_JOBS}). Job deferred.`);
            throw new InternalServerErrorException('Global compute quota exceeded. Try again later.');
        }
                        ...
                        containers: [
                            {
                                name: 'ml-trainer',
                                image: image,
                                env: Object.entries(envVars).map(([name, value]) => ({ name: String(name), value: String(value) })),
                                resources: {
                                    limits: {
                                        'nvidia.com/gpu': '1',
                                        memory: '8Gi',
