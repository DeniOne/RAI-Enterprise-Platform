...
export class ModelRegistryService {
  ...
  async registerModel(companyId: string, dto: RegisterModelDto) {
    this.logger.log(
      `🏗️ Registering model ${dto.name} v${dto.version} for company ${companyId}`,
    );

    // Проверка целостности артефакта в S3
    const isArtifactValid = await this.s3.validateObjectIntegrity(
      dto.artifactPath,
      dto.hash,
    );
    if (!isArtifactValid) {
      throw new BadRequestException(
        "Artifact integrity check failed (S3 existence or hash mismatch).",
      );
    }
