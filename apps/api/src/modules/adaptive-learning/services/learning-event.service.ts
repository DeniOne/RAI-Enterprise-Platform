...
export class LearningEventService {
  ...
  async ingestEvent(companyId: string, dto: CreateLearningEventDto) {
    this.logger.log(
      `📥 Ingesting learning event for ${dto.featureId} (Company: ${companyId})`,
    );
    ...
    if (!isValid) {
      this.logger.error(
        `❌ Invalid signature for learning event: ${dto.featureId}`,
      );
      throw new BadRequestException(
        "Invalid cryptographic signature for learning event.",
      );
    }
