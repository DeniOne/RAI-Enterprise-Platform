...
export class RetrainingOrchestrator {
   ...
   */
  async handleIncomingMetrics(
    companyId: string,
    featureId: string,
    metrics: any,
  ) {
    this.logger.log(
      `🧬 RetrainingOrchestrator: Processing metrics for ${featureId} (${companyId})`,
    );
    ...
    if (!acquired) {
      this.logger.warn(
        `⏳ Orchestration already in progress for ${featureId}. Skipping.`,
      );
      return;
    }
    ...
    try {
      ...
      if (await this.cooldown.isUnderCooldown(companyId, featureId)) {
        this.logger.warn(
          `🧊 Feature ${featureId} is under cooldown. Skipping retraining.`,
        );
        return;
      }

      // 3. Анализ дрейфа (Shadow Mode baseline)
      const latestModel = await this.modelRegistry.getLatestActiveModel(
        companyId,
        featureId,
      );
      if (!latestModel) {
        this.logger.warn(
          `⚠️ No active model found for ${featureId}. Manual initialization required.`,
        );
        return;
      }
      ...
      const driftReport = await this.driftAnalysis.analyzeDrift(
        companyId,
        latestModel.id,
        metrics,
        driftConfig,
      ...
      if (driftReport.status === "CRITICAL") {
        this.logger.error(
          `🚨 CRITICAL DRIFT detected for ${featureId}. Initiating SHADOW retraining pipeline.`,
        );
        await this.startShadowTraining(
          companyId,
          featureId,
  ...
  private async startShadowTraining(
    companyId: string,
    featureId: string,
    baseArtifact: string,
  ) {
    ...
    try {
      ...
      await this.k8sJob.createTrainingJob(
        companyId,
        run.id,
        "rai/ml-trainer:latest",
        {
      ...
      this.logger.error(
        `❌ Failed to start K8s Job for ${run.id}: ${e.message}`,
      );
      await this.prisma.trainingRun.update({
        where: { id: run.id },
        data: { status: "FAILED" },
