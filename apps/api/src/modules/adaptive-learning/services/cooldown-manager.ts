...
export class CooldownManager {
     ...
     */
    async setCooldown(companyId: string, featureId: string, durationSeconds: number = this.DEFAULT_COOLDOWN) {
        const key = `rai:cooldown:${companyId}:${featureId}`;
        this.logger.warn(`🛑 Setting cooldown for ${featureId} (${companyId}): ${durationSeconds}s`);
     ...
     */
    async isUnderCooldown(companyId: string, featureId: string): Promise<boolean> {
        const key = `rai:cooldown:${companyId}:${featureId}`;
