import { Controller, Post, Body, Headers, BadRequestException, Logger, Param } from '@nestjs/common';
...
import { GovernanceService, ApprovalAction } from './services/governance.service';
...
export class AdaptiveLearningController {
    ...
    async handleMLCallback(
        @Body() payload: { trainingRunId: string, modelHash: string, artifactPath: string, mae: number, companyId: string },
        @Headers('x-rai-signature') signature: string,
        @Headers('x-rai-nonce') nonce: string,
        @Headers('x-rai-timestamp') timestamp: string,
    ) {
        this.logger.log(`📞 Received ML Callback for TrainingRun: ${payload.trainingRunId}`);
        ...
        if (Math.abs(Date.now() - ts) > 300000) { // 5 минут окно
        ...
        return { status: 'ACCEPTED', modelId: model.id, nextStep: 'WAITING_FOR_GOVERNANCE_APPROVAL' };
    }
    ...
    async submitForApproval(@Param('id') modelId: string, @Body() body: { userId: string, notes: string }) {
    ...
    async reviewModel(
        ...
        return this.governance.reviewModel(requestId, body.userId, body.action, body.comment);
    }
   ...
   */
    ...
    async canaryCheck(@Param('id') id: string, @Body() metrics: { companyId: string, mae: number, baselineMae: number, sampleSize: number }) {
        return this.canary.evaluateCanaryPerformance(metrics.companyId, id, metrics.mae, metrics.baselineMae, metrics.sampleSize);
    }
}
