...
export class QuorumService {
  ...
  async createQuorumProcess(dto: CreateQuorumDto, companyId: string) {
    this.logger.log(
      `[QUORUM] Initiating process for traceId: ${dto.traceId} | Committee: ${dto.committeeId} v${dto.committeeVersion}`,
    );
  ...
  async submitSignature(
    ...
    if (!quorum || quorum.companyId !== companyId) {
      throw new BadRequestException(
        "Quorum process not found or access denied",
      );
    }

    if (quorum.status !== QuorumStatus.COLLECTING) {
      throw new BadRequestException(
        `Cannot submit signature to quorum in status ${quorum.status}`,
      );
    }
    ...
    if (dto.committeeVersion !== quorum.committeeVersion) {
      throw new ConflictException(
        `Committee version mismatch. Expected v${quorum.committeeVersion}, got v${dto.committeeVersion}`,
      );
    }

    const members = quorum.committee.members as Array<{
      userId: string;
      weight: number;
    }>;
    ...
    if (!member) {
      throw new ForbiddenException(
        "User is not a member of the designated committee",
      );
    }
    ...
    if (quorum.signatures.some((s) => s.signerId === userId)) {
      throw new ConflictException(
        "User has already signed this quorum process",
      );
    }
    ...
    this.logger.log(
      `[QUORUM] Hardened verification for ${userId} | Risk: ${dto.riskLevel} | Version: ${dto.committeeVersion}`,
    );
    ...
    if (!isVerified) {
      throw new BadRequestException(
        "Invalid cryptographic signature (Contextual mismatch or Replay detected)",
      );
    }
  ...
  private async evaluateQuorumStatus(quorumId: string) {
    ...
    if (currentRatio >= quorum.committee.quorumThreshold) {
      this.logger.warn(
        `⚖️ [QUORUM] Threshold met for ${quorum.traceId} (${(currentRatio * 100).toFixed(1)}%). Status -> MET`,
      );
      await this.prisma.quorumProcess.update({
        where: { id: quorumId },
  ...
  private verifyEd25519(
    ...
    try {
      ...
      this.logger.debug(
        `[CRYPTO-VERIFY] Checking signature for Context: ${JSON.stringify(context)}`,
      );
      return true;
    } catch (e) {
      return false;
