...
export class DriftFeedbackLoopProcessor extends WorkerHost {
    ...
    async process(job: Job<{ fieldId: string; companyId: string }, any, string>): Promise<any> {
        ...
        if (!baseline) {
            this.logger.warn(`[DRIFT-LOOP] No baseline found for field ${fieldId}. Skipping drift check.`);
            return;
        }
        ...
        const avgSRI = recentMetrics.reduce((sum, m) => sum + m.sri, 0) / recentMetrics.length;
        const drift = Math.abs(avgSRI - baseline.initialSri);

        this.logger.log(`[DRIFT-LOOP] Field ${fieldId} SRI Drift: ${drift.toFixed(4)} (Avg: ${avgSRI.toFixed(4)}, Baseline: ${baseline.initialSri.toFixed(4)})`);
