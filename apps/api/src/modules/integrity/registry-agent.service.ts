...
export class RegistryAgentService {
    ...
    async processObservation(observation: FieldObservation) {
        this.logger.log(`[REGISTRY-AGENT] Analyzing potential asset from observation ${observation.id}`);
        ...
        try {
            ...
            if (!recognitionResult || recognitionResult.confidence < 0.7) {
                this.logger.log(`[REGISTRY-AGENT] No confident asset in observation ${observation.id} (Confidence: ${recognitionResult?.confidence})`);
                return;
            }
    ...
    private generateIdempotencyKey(observation: FieldObservation): string {
        const source = observation.photoUrl || observation.voiceUrl || observation.content || observation.id;
        return crypto.createHash("sha256").update(source).digest("hex");
    }

    private async recognizeAsset(observation: FieldObservation): Promise<RegistryRecognitionResult | null> {
    ...
    private async handleMachinery(metadata: any, observation: FieldObservation, idempotencyKey: string) {
        ...
        if (existingByHash) {
            if (existingByHash.status === AssetStatus.REJECTED) {
                this.logger.warn(`[REGISTRY-AGENT] Machinery REJECTED by hash. Skipping.`);
            } else {
                this.logger.log(`[REGISTRY-AGENT] Machinery idempotency hit (${existingByHash.status}). Skipping.`);
            }
            return;
        }
    ...
    private async handleStock(metadata: any, observation: FieldObservation, idempotencyKey: string) {
        ...
        if (existingByHash) {
            if (existingByHash.status === AssetStatus.REJECTED) {
                this.logger.warn(`[REGISTRY-AGENT] StockItem REJECTED by hash. Skipping.`);
            } else {
                this.logger.log(`[REGISTRY-AGENT] StockItem idempotency hit (${existingByHash.status}). Skipping.`);
            }
            return;
        }
