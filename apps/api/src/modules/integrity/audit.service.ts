...
export class AuditService {
    ...
    async recordGovernanceEvent(data: {
        ...
        const eventHash = crypto.createHash('sha256').update(`${data.fieldId}-${data.eventType}-${payload}-${Date.now()}`).digest('hex');

        this.logger.log(`[AUDIT] Recording Governance Event: ${data.eventType} with Hash: ${eventHash.substring(0, 8)}`);
