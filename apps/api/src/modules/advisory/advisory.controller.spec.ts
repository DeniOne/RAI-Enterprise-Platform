...
describe("AdvisoryController (HTTP)", () => {
  ...
  it("GET /api/advisory/pilot/status", async () => {
    advisoryServiceMock.getPilotStatus.mockResolvedValue({
      enabled: true,
      scope: "COMPANY",
      companyId: "c-test",
    });
  ...
  it("GET /api/advisory/pilot/cohort", async () => {
    advisoryServiceMock.getPilotCohort.mockResolvedValue([
      { userId: "u-1", enabled: true, updatedAt: "2026-02-08T00:00:00.000Z" },
    ]);

    const response = await request(app.getHttpServer())
      .get("/api/advisory/pilot/cohort")
      .expect(200);
  ...
  it("POST /api/advisory/tuning/thresholds", async () => {
    advisoryServiceMock.updateTuningThresholds.mockResolvedValue({
  ...
  it("GET /api/advisory/ops/metrics", async () => {
    advisoryServiceMock.getOpsMetrics.mockResolvedValue({
      windowHours: 24,
      shadowEvaluated: 10,
      accepted: 5,
      rejected: 3,
      ...
      rejectRate: 0.3,
      feedbackRate: 0.2,
      decisionLagAvgMinutes: 7.5,
    });

    const response = await request(app.getHttpServer())
      .get("/api/advisory/ops/metrics?windowHours=24")
      .expect(200);
  ...
  it("POST /api/advisory/incident/kill-switch/enable", async () => {
    advisoryServiceMock.enableKillSwitch.mockResolvedValue({
      status: "ENABLED",
    });
  ...
  it("GET /api/advisory/rollout/status", async () => {
    advisoryServiceMock.getRolloutStatus.mockResolvedValue({
      stage: "S2",
      percentage: 25,
  ...
  it("POST /api/advisory/rollout/config", async () => {
    advisoryServiceMock.configureRollout.mockResolvedValue({
      stage: "S1",
      percentage: 10,
      autoStopEnabled: true,
    });
  ...
  it("POST /api/advisory/recommendations/:traceId/accept", async () => {
    advisoryServiceMock.acceptRecommendation.mockResolvedValue({
      traceId: "t-accept",
      status: "ACCEPTED",
    });
