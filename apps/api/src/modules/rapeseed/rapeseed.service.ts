...
export class RapeseedService {
    ...
    async create(input: CreateRapeseedInput, user: User, companyId: string): Promise<Rapeseed> {
        // Check uniqueness
        ...
        if (existing) {
            throw new ConflictException(`Rapeseed with name ${input.name} already exists for this company`);
        }
        ...
        await this.auditService.log(
            AgriculturalAuditEvent.RAPESEED_CREATED,
            user,
            { rapeseedId: rapeseed.id, name: rapeseed.name },
        );

        return rapeseed;
    }

    async update(input: UpdateRapeseedInput, user: User, companyId: string): Promise<Rapeseed> {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (!currentRapeseed.isLatest) {
                throw new BadRequestException('You can only update the latest version of a Rapeseed');
            }
            ...
            const newRapeseed = await tx.rapeseed.create({
                data: {
                    ...
                    oilContent: updateData.oilContent ?? (currentRapeseed as any).oilContent,
                    erucicAcid: updateData.erucicAcid ?? (currentRapeseed as any).erucicAcid,
                    glucosinolates: updateData.glucosinolates ?? (currentRapeseed as any).glucosinolates,
                    vegetationPeriod: updateData.vegetationPeriod ?? currentRapeseed.vegetationPeriod,
                    sowingNormMin: updateData.sowingNormMin ?? currentRapeseed.sowingNormMin,
                    sowingNormMax: updateData.sowingNormMax ?? currentRapeseed.sowingNormMax,
                    sowingDepthMin: updateData.sowingDepthMin ?? currentRapeseed.sowingDepthMin,
                    sowingDepthMax: updateData.sowingDepthMax ?? currentRapeseed.sowingDepthMax,
            ...
            for (const key of Object.keys(updateData)) {
                // @ts-ignore
                if (updateData[key] !== currentRapeseed[key] && updateData[key] !== undefined) {
                    // @ts-ignore
                    diff[key] = { from: currentRapeseed[key], to: updateData[key] };
                }
            }

            await this.auditService.logRapeseedChange(newRapeseed.id, newVersion, user.id, diff);
    ...
    async getRapeseedVersionHistory(rapeseedId: string, companyId: string): Promise<Rapeseed[]> {
     ...
     */
    async calculateOilYield(rapeseedId: string, area: number): Promise<number> {
        const rapeseed = await this.prisma.rapeseed.findUnique({ where: { id: rapeseedId } });
    ...
    validateRapeseedParameters(input: CreateRapeseedInput | UpdateRapeseedInput): void {
        if (input.sowingNormMin && input.sowingNormMax && input.sowingNormMin > input.sowingNormMax) {
            throw new BadRequestException('Min sowing norm cannot be greater than Max');
        }
        if (input.sowingDepthMin && input.sowingDepthMax && input.sowingDepthMin > input.sowingDepthMax) {
            throw new BadRequestException('Min sowing depth cannot be greater than Max');
        }
    }
}
