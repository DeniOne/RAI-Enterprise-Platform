...
describe('RapeseedService', () => {
    ...
    it('should create a Rapeseed', async () => {
        ...
        const result = await service.create(
            { name: 'Winter Rapeseed', type: RapeseedType.SPRING, vegetationPeriod: 120 },
            mockUser,
            'company-1',
        );

        expect(prismaMock.rapeseed.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should update rapeseed with versioning (transactional)', async () => {
        ...
        expect(prismaMock.rapeseed.update).toHaveBeenCalledWith(expect.objectContaining({
