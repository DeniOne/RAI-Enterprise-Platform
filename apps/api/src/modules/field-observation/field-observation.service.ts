...
export class FieldObservationService {
  ...
  async createObservation(data: {
    ...
    this.logger.log(
      `[FO] Creating observation: ${data.type} (Intent: ${data.intent}) for field ${data.fieldId}`,
    );
    ...
    this.integrityGate.processObservation(observation).catch((err) => {
      this.logger.error(
        `[FO] Integrity Gate processing FAILED for observation ${observation.id}: ${err.message}`,
      );
    });
    ...
    this.shadowAdvisory
      ...
      .catch((err) => {
        this.logger.warn(
          `[FO] Shadow advisory skipped for observation ${observation.id}: ${err.message}`,
        );
      });
