...
export class ProgressService implements OnModuleInit {
    ...
    public getProgressStats() {
        try {
            ...
            const alphaProgress = total > 0 ? Math.round((completed / total) * 100) : 0;
    ...
    private async broadcastProgress(stats: any) {
        ...
        for (const user of users) {
            try {
                await this.bot.telegram.sendMessage(user.telegramId!, report, { parse_mode: 'Markdown' });
            } catch (e) {
                this.logger.error(`❌ Failed to send push to ${user.telegramId}: ${e.message}`);
            }
        }
    }
}
