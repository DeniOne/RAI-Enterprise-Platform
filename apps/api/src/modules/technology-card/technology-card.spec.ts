...
describe("TechnologyCardService", () => {
  ...
  it("should create a technology card with operations and resources", async () => {
    ...
    (prisma.technologyCard.create as jest.Mock).mockResolvedValue({
      id: "tc1",
      ...input,
    });
  ...
  it("should apply card to season successfully", async () => {
    (prisma.season.findFirst as jest.Mock).mockResolvedValue({
      id: "s1",
      companyId: "c1",
    });
    (prisma.technologyCard.findFirst as jest.Mock).mockResolvedValue({
      id: "tc1",
      companyId: "c1",
    });
  ...
  it("should throw if season belongs to another company", async () => {
    ...
    await expect(
      service.applyToSeason("s1", "tc1", mockUser, "c1"),
