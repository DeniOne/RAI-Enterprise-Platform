...
export class ClientRegistryController {
    ...
    @Post('holdings')
    async createHolding(@Body() body: { name: string; description?: string; companyId: string }) {
    ...
    async getHolding(@Param('id') id: string, @Param('companyId') companyId: string) {
        return this.registryService.findOneHolding(id, companyId);
    }

    @Delete('holdings/:id/:companyId')
    async deleteHolding(@Param('id') id: string, @Param('companyId') companyId: string) {
    ...
    async linkToHolding(
        ...
        return this.registryService.updateClientHolding(id, body.holdingId, body.companyId);
    }
}
