...
export class ClientRegistryService {
    ...
    async createHolding(data: { name: string; description?: string }, companyId: string): Promise<Holding> {
    ...
    async deleteHolding(id: string, companyId: string): Promise<void> {
        const holding = await this.findOneHolding(id, companyId);
        ...
        if (clientsCount > 0) {
            throw new ConflictException(`Cannot delete holding ${id} because it has active clients linked to it`);
        }
    ...
    async updateClientHolding(clientId: string, holdingId: string | null, companyId: string): Promise<Client> {
        ...
        if (holdingId) {
            ...
            if (!holding) {
                throw new ForbiddenException(`Holding ${holdingId} does not belong to your company`);
            }
        }
    ...
    async findClientsByHolding(holdingId: string, companyId: string): Promise<Client[]> {
