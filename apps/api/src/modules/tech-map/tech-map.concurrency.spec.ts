...
describe('TechMapService Concurrency (Track 1)', () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: IntegrityGateService, useValue: { validateTechMapAdmission: jest.fn() } },
                TechMapStateMachine,
            ],
        }).compile();
    ...
    it('should throw TechMapActiveConflictError on P2002 duplicate active map', async () => {
        ...
        await expect(
            service.transitionStatus('map-1', TechMapStatus.ACTIVE, 'comp-1', UserRole.CEO, 'user-1')
    ...
    it('should reset activeTechMapId when archiving an ACTIVE map', async () => {
        ...
        jest.spyOn(prisma.techMap, 'findFirst').mockResolvedValue(mockMap as any);
        jest.spyOn(prisma.techMap, 'update').mockResolvedValue({ ...mockMap, status: TechMapStatus.ARCHIVED } as any);

        await service.transitionStatus('map-1', TechMapStatus.ARCHIVED, 'comp-1', UserRole.CEO, 'user-1');
