...
describe('ReconciliationJob alert path', () => {
    const build = () => {
        ...
        const outbox = {
            createEvent: jest.fn().mockReturnValue({ type: 'finance.reconciliation.alert', payload: {} }),
        } as any;
    ...
    it('creates outbox alert when event has no ledger entries', async () => {
        ...
        prisma.economicEvent.findMany
            .mockResolvedValueOnce([
                { id: 'evt-1', companyId: 'c1', type: 'COST_INCURRED', createdAt: new Date('2026-02-16T12:00:00.000Z') },
            ])
            .mockResolvedValueOnce([]);
