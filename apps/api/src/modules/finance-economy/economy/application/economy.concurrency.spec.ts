...
process.env.DATABASE_URL =
  "postgresql://rai_admin:secret@localhost:5432/rai_platform?schema=public";
...
const mockOutboxService = {
  createEvent: jest
    .fn()
    .mockImplementation((aggId, aggType, type, payload) => ({
      aggregateId: aggId,
...
describe("EconomyService Final Stress Simulation", () => {
  ...
  beforeAll(async () => {
    ...
    attributionSpy = jest
      .spyOn(CostAttributionRules, "getAttributions")
  ...
  const generateEvent = (
    companyId: string,
    type: EconomicEventType = EconomicEventType.REVENUE_RECOGNIZED,
  ...
  it("Scenario A: 200 Concurrent Same-Tenant Ingest (No Deadlocks)", async () => {
    ...
    const events = Array.from({ length: concurrency }).map(() =>
      generateEvent(companyId),
    );

    console.time("Scenario A");
    const results = await Promise.allSettled(
      events.map((e) => service.ingestEvent(e)),
    );
    ...
    if (failed.length > 0) {
      ...
      const deadlocks = reasons.filter(
        (e) => e.code === "40P01" || e.message?.includes("deadlock"),
      );
      expect(deadlocks.length).toBe(0);

      console.log(
        `Scenario A: ${successful} success, ${failed.length} failed (likely timeouts). Deadlocks: ${deadlocks.length}`,
      );
    } else {
  ...
  it("Scenario B: 200 Concurrent Solvency Attacks (Panic, No Leaks)", async () => {
    ...
    const events = Array.from({ length: concurrency }).map(() =>
      generateEvent(
        companyId,
        EconomicEventType.OBLIGATION_SETTLED,
        "SOLVENCY_ATTACK",
      ),
    );
    ...
    const integrityErrors = failed.filter((r) => {
      ...
      return (
        ...
        msg.includes("математической ошибки целостности")
      ); // Russian localized message
    });

    console.log(
      `Scenario B: ${integrityErrors.length} caught solveny/integrity violations.`,
    );
    expect(integrityErrors.length).toBeGreaterThan(0);

    // Verify Tenant is READ_ONLY
    const tenant = await prisma.tenantState.findUnique({
      where: { companyId },
    });
  ...
  it("Scenario C: Mixed Workload (Isolation)", async () => {
    ...
    const workload = [
      ...Array.from({ length: 100 }).map(() =>
        generateEvent(
          companyIdValid,
          EconomicEventType.REVENUE_RECOGNIZED,
          "VALID",
        ),
      ),
      ...Array.from({ length: 100 }).map(() =>
    ...
    const results = await Promise.allSettled(
      workload.map((e) => service.ingestEvent(e)),
    );
    console.timeEnd("Scenario C");

    // Check Valid Company is still ACTIVE
    const validState = await prisma.tenantState.findUnique({
      where: { companyId: companyIdValid },
    });
    expect(validState?.mode).toBe("ACTIVE");

    // Check Invalid Company is READ_ONLY
    const invalidState = await prisma.tenantState.findUnique({
      where: { companyId: companyIdInvalid },
    });
    expect(invalidState?.mode).toBe("READ_ONLY");

    // Check Cross Company is ACTIVE
    const crossState = await prisma.tenantState.findUnique({
      where: { companyId: companyIdCross },
    });
  ...
  it("Scenario D: Long Transaction + Burst (No Global Freeze)", async () => {
    ...
    const longTxPromise = prisma.$transaction(
      async (tx) => {
        await tx.$executeRaw(
          Prisma.sql`SELECT pg_advisory_xact_lock(hashtext(${lockEventId}))`,
        );
    ...
    const burstEvents = Array.from({ length: 50 }).map(() =>
      generateEvent(companyIdBurst),
    );

    console.time("Scenario D: Burst");
    const results = await Promise.allSettled(
      burstEvents.map((e) => service.ingestEvent(e)),
    );
    ...
    if (failed.length > 0) {
      console.error(
        "Scenario D Failed Reason Sample:",
