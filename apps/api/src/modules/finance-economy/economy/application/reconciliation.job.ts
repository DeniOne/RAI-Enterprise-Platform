...
export class ReconciliationJob {
  ...
  async run(): Promise<void> {
    const lookbackHours = Number(
      process.env.FINANCE_RECON_LOOKBACK_HOURS || 24,
    );
    const from = new Date(
    ...
    for (const event of eventsWithoutLedger) {
      this.logger.error(
        `[RECON] MISSING_LEDGER_ENTRIES: eventId=${event.id} type=${event.type} company=${event.companyId}`,
      );
      alertPayloads.push(
        this.prepareAlert(event.companyId, "MISSING_LEDGER_ENTRIES", {
    ...
    for (const event of candidates) {
      ...
      if (Math.abs(debit - credit) > 0.0001) {
        alertPayloads.push(
          this.prepareAlert(event.companyId, "DOUBLE_ENTRY_MISMATCH", {
            economicEventId: event.id,
            debit,
            credit,
  ...
  private prepareAlert(
    ...
    this.logger.error(
      `[RECON] ${anomalyType} company=${companyId} details=${JSON.stringify(details)}`,
    );
