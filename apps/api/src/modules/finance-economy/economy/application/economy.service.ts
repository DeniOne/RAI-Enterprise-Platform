...
export class EconomyService {
  ...
  async ingestEvent(dto: IngestEconomicEventDto) {
    ...
    if (InvariantMetrics.shouldTriggerFinancialPanic(panicThreshold)) {
      ...
      throw new ServiceUnavailableException(
        "Режим финансовой паники активен. Операции временно заблокированы.",
      );
    }
    ...
    if (!idempotencyKey && this.config.get("requireIdempotency")) {
      ...
      throw new BadRequestException(
        "Ключ идемпотентности обязателен для финансовых операций.",
      );
    }
    ...
    const scale = await this.getCurrencyScale(
      dto.currency || this.config.get("defaultCurrency"),
    );
    ...
    this.logger.log(
      `Регистрация экономического события: ${dto.type} для компании ${dto.companyId}`,
    );
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        const tenantState = await (tx as any).tenantState.findUnique({
          where: { companyId: dto.companyId },
        });
        if (tenantState && tenantState.mode === "HALTED") {
          throw new ServiceUnavailableException(
            `Компания ${dto.companyId} ОСТАНОВЛЕНА. Финансовые операции заблокированы.`,
          );
        }
      ...
      if (this.isUniqueConflict(error, "replayKey")) {
        ...
        if (existing) {
          ...
          if (entryCount === 0) {
            this.logger.error(
              `КРИТИЧЕСКАЯ ОШИБКА: Обнаружено событие-фантом ${existing.id} без проводок в леджере. Попытка аварийного восстановления...`,
            );
            ...
            return await this.prisma.$transaction(async (tx) => {
              ...
              await this.generateLedgerEntries(
                existing,
                normalizedAmount,
                tx,
                scale,
      ...
      if (this.isIntegrityViolation(error)) {
        ...
        (InvariantMetrics as any).increment(
          "financial_integrity_violations_total",
        );
        ...
        throw new ServiceUnavailableException(
          "Транзакция заблокирована из-за математической ошибки целостности. Тенант изолирован.",
        );
      }
  ...
  private async triggerTenantPanic(companyId: string, reason: string) {
    try {
      ...
      if (result.count > 0) {
        this.logger.warn(
          `Тенант ${companyId} переведен в режим READ_ONLY по причине: ${reason}`,
        );
      } else {
      ...
      console.error(
        `Критический сбой: Не удалось заблокировать тенант ${companyId} во время паники!`,
        panicError,
      );
      this.logger.error(
        `Критический сбой: Не удалось заблокировать тенант ${companyId} во время паники!`,
  ...
  private async generateLedgerEntries(
    ...
    const attributions =
      CostAttributionRules.getAttributions({
        type: event.type,
    ...
    if (attributions.length === 0) {
      ...
      if (isSettlement) {
        this.logger.error(
          `КРИТИЧЕСКОЕ НАРУШЕНИЕ: Расчетное событие ${event.type} не породило проводок! Транзакция прервана.`,
        );
        InvariantMetrics.increment("financial_invariant_failures_total");
        throw new Error(
          `Integrity Violation: Settlement event ${event.type} must have attributions.`,
        );
      }
    ...
    if (
      !executionId &&
      event.type !== EconomicEventType.OTHER &&
      event.type !== EconomicEventType.BOOTSTRAP
    ) {
      this.logger.warn(
        `Экономическое событие ${event.id} получено без executionId. Прослеживаемость ограничена.`,
      );
    }
    ...
    if (cashImpact) {
      if (!cashAccountId || !cashDirection) {
        InvariantMetrics.increment("financial_invariant_failures_total");
        throw new Error(
          `Нарушение целостности: cashImpact=true требует наличия cashAccountId и cashDirection (Событие ${event.id})`,
        );
      }
    } else {
      if (cashAccountId || cashDirection) {
        InvariantMetrics.increment("financial_invariant_failures_total");
        throw new Error(
          `Нарушение целостности: cashImpact=false требует отсутствия cashAccountId и cashDirection (Событие ${event.id})`,
        );
      }
    }
    ...
    const normalizedPostings = attributions.map((attr: any) => ({
      ...
      amount: new Prisma.Decimal(
        new Prisma.Decimal(attr.amount).toFixed(
          scale,
    ...
    await (tx as any).$executeRaw(
      Prisma.sql`SELECT pg_advisory_xact_lock(hashtext(${event.id}))`,
    );
    ...
    let startSeq =
      existingEntries.length > 0 ? existingEntries[0].sequenceNumber : 0;
  ...
  private validateContractCompatibility(
    ...
    const supported = contractVersion
      ? (FINANCE_INGEST_SUPPORTED_VERSIONS as readonly string[]).includes(
          contractVersion,
        )
      : false;
