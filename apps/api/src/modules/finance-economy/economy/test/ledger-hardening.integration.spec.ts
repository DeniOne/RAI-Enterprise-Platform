...
describe('Ledger Architectural Hardening (G1-G4 Verification)', () => {
    ...
    it('G1: Injects RLS context into database session', async () => {
        ...
        ((prisma as any).tenantState.findUnique as jest.Mock).mockResolvedValue({ mode: 'ACTIVE' });
    ...
    it('G2: Triggers Autonomous Panic (READ_ONLY) when Integrity Violation occurs', async () => {
        const integrityError = new Prisma.PrismaClientKnownRequestError('Imbalanced', {
            code: 'P2002',
            clientVersion: '5.x',
        ...
        expect((prisma as any).tenantState.upsert).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('G1 (Fail-Closed): Blocks ingestion when tenant is HALTED (P0003)', async () => {
        ...
        ((prisma as any).tenantState.findUnique as jest.Mock).mockResolvedValue({ mode: 'HALTED' });
    ...
    it('G3: Handles Replay/Duplicate with target check (P2002 replayKey)', async () => {
        ...
        ((prisma as any).economicEvent.findFirst as jest.Mock).mockResolvedValue({ id: 'existing_evt' });
