...
export type JournalPhase = 'ACCRUAL' | 'SETTLEMENT' | 'ADJUSTMENT' | 'BOOTSTRAP' | 'OTHER';
...
export function resolveSettlementRef(type: EconomicEventType, metadata: Record<string, unknown> | undefined): string | null {
...
export function assertBalancedPostings(postings: JournalPosting[]): void {
    ...
    if (Math.abs(delta) > 0) {
        throw new Error(`Unbalanced journal postings: debit=${debit}, credit=${credit}, delta=${delta}`);
    }
}
