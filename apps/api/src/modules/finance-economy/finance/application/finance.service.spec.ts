...
describe('FinanceService optimistic locking', () => {
    ...
    it('throws NotFound when account is missing', async () => {
        ...
        await expect(service.updateBalance('acc-1', 100, 'c1')).rejects.toBeInstanceOf(NotFoundException);
    });

    it('throws ConflictException on version mismatch', async () => {
        const { service, prisma } = makeService();
        prisma.cashAccount.findFirst.mockResolvedValue({ id: 'acc-1', companyId: 'c1', version: 2 });
        prisma.cashAccount.updateMany.mockResolvedValue({ count: 0 });

        await expect(service.updateBalance('acc-1', 100, 'c1')).rejects.toBeInstanceOf(ConflictException);
    });
    ...
    it('increments version and returns updated account', async () => {
        ...
        prisma.cashAccount.findFirstOrThrow.mockResolvedValue({ id: 'acc-1', version: 3 });
