...
export const BUDGET_STATE_METADATA: Record<
  BudgetStatus,
  StateMetadata<BudgetStatus>
> = {
  [BudgetStatus.DRAFT]: {
...
const BUDGET_TRANSITIONS: TransitionDef<BudgetStatus, BudgetEvent>[] = [
  {
    ...
    event: BudgetEvent.APPROVE,
    to: BudgetStatus.APPROVED,
  },
  {
    from: BudgetStatus.APPROVED,
    event: BudgetEvent.ACTIVATE,
    to: BudgetStatus.ACTIVE,
  },

  {
    from: BudgetStatus.ACTIVE,
    event: BudgetEvent.EXHAUST,
    to: BudgetStatus.EXHAUSTED,
  },
  {
    from: BudgetStatus.ACTIVE,
    event: BudgetEvent.BLOCK,
    to: BudgetStatus.BLOCKED,
  },
  {
    from: BudgetStatus.ACTIVE,
    event: BudgetEvent.CLOSE,
    to: BudgetStatus.CLOSED,
  },

  {
    from: BudgetStatus.EXHAUSTED,
    event: BudgetEvent.REPLENISH,
    to: BudgetStatus.ACTIVE,
  },
  {
    from: BudgetStatus.EXHAUSTED,
  ...
  {
    from: BudgetStatus.BLOCKED,
    event: BudgetEvent.UNBLOCK,
    to: BudgetStatus.ACTIVE,
  },
  {
    from: BudgetStatus.BLOCKED,
...
class BudgetStateMachineImpl implements StateMachine<
  BudgetEntity,
  BudgetStatus,
  BudgetEvent
> {
  ...
  getAvailableEvents(state: BudgetStatus): BudgetEvent[] {
    return BUDGET_TRANSITIONS.filter((t) => t.from === state).map(
      (t) => t.event,
    );
  }
