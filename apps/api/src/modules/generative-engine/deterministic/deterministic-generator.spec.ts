...
describe("DeterministicGenerator", () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DeterministicGenerator,
        {
          ...
          useValue: {
            computeFixedTimestamp: jest
              .fn()
              .mockReturnValue("2026-01-01T00:00:00Z"),
            wrapDeterministicContext: jest
              .fn()
    ...
    constraintPropagator =
      module.get<ConstraintPropagator>(ConstraintPropagator);
    integrityGate = module.get<IntegrityGateGenerative>(
  ...
  it("должен оркестровать детерминированный пайплайн корректно", async () => {
    ...
    // 2. Resolve seed
    expect(seedManager.resolveSeed).toHaveBeenCalledWith(
      expect.any(String),
      999,
    );

    // 3. Timestamp
    expect(entropyController.computeFixedTimestamp).toHaveBeenCalledWith(
      "12345",
    );
    ...
    // 6. Post-validation
    expect(integrityGate.validateGeneratedDraft).toHaveBeenCalledWith(
      mockDraft,
      expect.any(String),
    );
