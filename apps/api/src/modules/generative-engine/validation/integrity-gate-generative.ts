...
export class IntegrityGateGenerative {
  ...
  validateGenerationInput(params: GenerationParams): void {
    ...
    if (params.explicitSeed !== undefined) {
      if (
        !Number.isInteger(params.explicitSeed) ||
        params.explicitSeed < 0 ||
        params.explicitSeed > 0xffffffff
      ) {
  ...
  validateGeneratedDraft(
    ...
    if (draft.status !== "GENERATED_DRAFT") {
      errors.push(
        `status должен быть GENERATED_DRAFT, получен: ${draft.status}`,
      );
    }
  ...
  validateRecordIntegrity(record: {
    ...
    const canonical = this.canonicalSorter.canonicalize(
      record.canonicalizedPayload,
    );
