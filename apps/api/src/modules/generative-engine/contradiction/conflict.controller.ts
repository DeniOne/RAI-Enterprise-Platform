...
export class ConflictController {
  ...
  async confirmOverride(
    ...
    const disResult = this.conflictMatrix.calculate({
      ...
      humanOperationCount: (
        (dto.humanAction["operations"] as any[]) ||
        (draftSnapshot["operations"] as any[]) ||
        []
      ).length,
    ...
    const recordId = await this.divergenceTracker.recordDivergence({
      ...
      deltaRisk: riskResult.deltaRisk,
      conflictVector: disResult.conflictVector as unknown as Record<
        string,
        unknown
      >,
      humanAction: dto.humanAction,
  ...
  async getDivergence(@Param("id") id: string) {
    ...
    if (!record) {
      throw new BadRequestException(`DivergenceRecord id="${id}" не найден.`);
    }
    return record;
  }
