...
export class ConflictMatrixService {
    ...
    calculate(input: ConflictMatrixInput): ConflictMatrixResult {
        ...
        // f1: Yield Divergence
        const yieldDenom = Math.max(Math.abs(input.aiYield), ConflictMatrixService.EPSILON);
        let f1: number;
        ...
        if (totalOps < ConflictMatrixService.EPSILON) {
            ...
            f4 = Math.abs(input.humanOperationCount - input.aiOperationCount) / totalOps;
        }
        ...
        const disScore = roundHalfToEven(
            Math.min(Math.max(rawDIS, 0), 1),
        );
