...
describe('DraftFactory', () => {
    ...
    describe('createDraft', () => {
        ...
        const templates: any[] = [
            { stageName: 'Stage B', stageSequence: 2, name: 'Op 2', sequence: 1, resources: [] },
            { stageName: 'Stage A', stageSequence: 1, name: 'Op 1', sequence: 1, resources: [] }
        ];
    ...
    describe('assertHumanApproval (I17)', () => {
        it('должен позволять AGRONOMIST утверждать', () => {
            expect(() => factory.assertHumanApproval('AGRONOMIST' as any)).not.toThrow();
        });

        it('должен запрещать SYSTEM утверждать', () => {
            expect(() => factory.assertHumanApproval('SYSTEM' as any)).toThrow(/Human Override/);
        });
    });
});
