import { DraftStateManager, GovernanceContext } from '../fsm/draft-state-manager';
...
describe('E2E Override Pipeline', () => {
            ...
            stages: [
                {
                    ...
                    operations: [
                        ...
                        { type: 'fertilizing', product: 'nh4no3', dose: 100, unit: 'kg/ha' },
                    ],
                },
            ],
        },
    ...
    it('полный pipeline: simulation → DIS → governance → FSM transition', () => {
        ...
        expect(['ACCEPT', 'REVIEW', 'REJECT']).toContain(explanation.recommendation);
    ...
    it('hash sensitivity: разные policyVersion → разный hash', () => {
        const hash1 = engine.simulate({ ...CF_INPUT, policyVersion: 'v1.0.0' }).simulationHash;
        const hash2 = engine.simulate({ ...CF_INPUT, policyVersion: 'v2.0.0' }).simulationHash;
