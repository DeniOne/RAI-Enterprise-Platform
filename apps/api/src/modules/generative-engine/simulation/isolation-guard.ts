...
export class IsolationGuard {
    ...
    assertIsolation(contextType: string): void {
        if (contextType === 'SIMULATION') {
            this.logger.debug('[I22] IsolationGuard: SIMULATION context active. Write operations restricted.');
    ...
    blockProductionWrite(tableName: string): void {
        throw new Error(`[I22] Isolation Violation: Attempt to write to ${tableName} during simulation`);
    }
}
