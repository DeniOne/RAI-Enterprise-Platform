...
describe('ConfidenceIntervalValidator', () => {
    ...
    beforeEach(async () => {
        ...
        validator = module.get<ConfidenceIntervalValidator>(ConfidenceIntervalValidator);
    });
    ...
    it('должен кидать ошибку на некорректный уровень доверия', () => {
        expect(() => validator.validateInterval(10, 20, 0.5)).toThrow(BadRequestException);
        expect(() => validator.validateInterval(10, 20, 1.0)).toThrow(BadRequestException);
    });

    it('должен кидать ошибку если lower >= upper', () => {
        expect(() => validator.validateInterval(20, 10, 0.95)).toThrow(BadRequestException);
    });
});
