...
export class YieldForecastService {
  ...
  constructor(private readonly engine: DeterministicForecastEngine) {}
  ...
  async predict(draft: GeneratedDraft): Promise<any> {
    this.logger.log(
      `Starting yield prediction for draft ${draft.generationMetadata.hash.substring(0, 8)}`,
    );
