...
describe('DraftStateManager (Level C + Governance Guards)', () => {
    ...
    describe('Level B: GENERATED_DRAFT transitions', () => {
        it('GENERATED_DRAFT → DRAFT: разрешён для ADMIN', () => {
            expect(
                fsm.canTransition(TechMapStatus.GENERATED_DRAFT, TechMapStatus.DRAFT, UserRole.ADMIN),
            ).toBe(true);
        });

        it('GENERATED_DRAFT → ARCHIVED: разрешён для AGRONOMIST', () => {
            expect(
                fsm.canTransition(TechMapStatus.GENERATED_DRAFT, TechMapStatus.ARCHIVED, UserRole.AGRONOMIST),
            ).toBe(true);
        });

        it('GENERATED_DRAFT → REVIEW: запрещён', () => {
            expect(
                fsm.canTransition(TechMapStatus.GENERATED_DRAFT, TechMapStatus.REVIEW, UserRole.ADMIN),
            ).toBe(false);
        });
    });
    ...
    describe('Level C: OVERRIDE_ANALYSIS basic transitions', () => {
        it('DRAFT → OVERRIDE_ANALYSIS: разрешён (вход в анализ)', () => {
            expect(
                fsm.canTransition(TechMapStatus.DRAFT, TechMapStatus.OVERRIDE_ANALYSIS, UserRole.ADMIN),
            ).toBe(true);
        });

        it('OVERRIDE_ANALYSIS → ARCHIVED: разрешён (reject)', () => {
            expect(
                fsm.canTransition(TechMapStatus.OVERRIDE_ANALYSIS, TechMapStatus.ARCHIVED, UserRole.MANAGER),
            ).toBe(true);
        });

        it('OVERRIDE_ANALYSIS → ACTIVE: запрещён (shortcut)', () => {
            expect(
                fsm.canTransition(TechMapStatus.OVERRIDE_ANALYSIS, TechMapStatus.ACTIVE, UserRole.ADMIN),
            ).toBe(false);
        });
    });
        ...
        it('DIS > 0.7 С пустой justification → ЗАПРЕЩЁН', () => {
            expect(
                fsm.canTransition(
                    ...
                    TechMapStatus.DRAFT,
                    UserRole.ADMIN,
                    { divergenceRecordId: 'clxyz123', disScore: 0.85, justification: '   ' },
                ),
            ).toBe(false);
        });
            ...
            expect(
                fsm.canTransition(
                    ...
                    UserRole.ADMIN,
                    {
                        ...
                        disScore: 0.85,
                        justification: 'Агроном подтверждает: изменение обосновано погодными условиями',
                    },
                ),
            ).toBe(true);
        });
