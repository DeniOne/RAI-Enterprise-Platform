...
export class DraftStateManager {
    ...
    canTransition(
        ...
        if (rule.requiresJustification && governance?.disScore !== undefined) {
            if (governance.disScore > DraftStateManager.HIGH_RISK_DIS_THRESHOLD) {
                if (!governance.justification || governance.justification.trim().length === 0) {
                    return false;
                }
            }
        }
    ...
    validate(
        ...
        if (!this.canTransition(currentStatus, targetStatus, userRole, governance)) {
            ...
            if (!rule) {
                const allowed = DraftStateManager.TRANSITION_MATRIX
    ...
    getAvailableTransitions(
        ...
        return DraftStateManager.TRANSITION_MATRIX
            .filter((r) => r.from === currentStatus)
