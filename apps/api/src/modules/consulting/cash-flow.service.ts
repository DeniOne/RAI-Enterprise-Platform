...
export class CashFlowService {
    ...
    async getCashPosition(context: UserContext, asOfDate: Date = new Date()) {
        this.logger.debug(`Calculating cash position for company ${context.companyId} as of ${asOfDate.toISOString()}`);
     ...
     */
    async getProjectedCashFlow(context: UserContext, startDate: Date, endDate: Date) {
        this.logger.debug(`Calculating projected cash flow for company ${context.companyId} from ${startDate.toISOString()} to ${endDate.toISOString()}`);
        ...
        let currentBalance = (await this.getCashPosition(context, startDate)).netPosition;

        // Инициализируем карту дат
        const dateMap = new Map<string, { inflow: Prisma.Decimal, outflow: Prisma.Decimal }>();

        entries.forEach(entry => {
            const dateStr = entry.dueDate!.toISOString().split('T')[0];
            const current = dateMap.get(dateStr) || { inflow: this.d(0), outflow: this.d(0) };
