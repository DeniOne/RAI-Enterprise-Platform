...
export class YieldService {
  ...
  async createOrUpdateHarvestResult(
    dto: SaveHarvestResultDto,
    context: UserContext,
  ): Promise<HarvestResult> {
    this.logger.log(
      `[YIELD-SERVICE] Processing harvest result for plan ${dto.planId}`,
    );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!seasonId) {
        throw new BadRequestException(
          "Для сохранения результата сбора необходима привязка к сезону через техкарту",
        );
      }
      ...
      this.logger.log(
        `[YIELD-SERVICE] Harvest result recorded successfully for plan ${dto.planId}`,
      );
      return savedResult;
    });
  }

  async getHarvestResultByPlan(
    planId: string,
    context: UserContext,
  ): Promise<HarvestResult> {
    const result = await this.repository.findByPlanId(
      planId,
      context.companyId,
    );
