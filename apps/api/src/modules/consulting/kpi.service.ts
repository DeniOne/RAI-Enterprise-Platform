...
export class KpiService {
  ...
  async calculatePlanKPI(planId: string, context: UserContext) {
    ...
    const totalActualCost = ledgerSummary._sum.amount
      ? this.d(ledgerSummary._sum.amount)
      : this.d(result?.costSnapshot);
    ...
    return {
      ...
      plannedYield: this.round(
        this.d(result?.plannedYield || plan.optValue),
        2,
      ),
