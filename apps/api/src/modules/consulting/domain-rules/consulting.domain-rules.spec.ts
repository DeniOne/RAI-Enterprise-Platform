...
describe('ConsultingDomainRules', () => {
    ...
    describe('canActivate', () => {
        ...
        it('ЗАПРЕЩАЕТ активацию при открытых Deviation (DETECTED)', async () => {
            ...
            await expect(service.canActivate('plan-1')).rejects.toThrow(BadRequestException);
            await expect(service.canActivate('plan-1')).rejects.toThrow('незакрытых отклонений');
        });
    });
    ...
    describe('canCreateDeviation', () => {
        it('РАЗРЕШАЕТ создание Deviation для ACTIVE плана', async () => {
            ...
            await expect(service.canCreateDeviation('plan-1', 'comp-1')).resolves.toBeUndefined();
        });
        ...
        it('ЗАПРЕЩАЕТ создание Deviation для DRAFT плана', async () => {
            ...
            await expect(service.canCreateDeviation('plan-1', 'comp-1')).rejects.toThrow(
                'ACTIVE планов',
            );
        });
        ...
        it('ЗАПРЕЩАЕТ создание Deviation для несуществующего плана', async () => {
            ...
            await expect(service.canCreateDeviation('no-plan', 'comp-1')).rejects.toThrow(
    ...
    describe('canArchive', () => {
        ...
        it('ЗАПРЕЩАЕТ архивацию при незакрытых Deviation', async () => {
            ...
            await expect(service.canArchive('plan-1')).rejects.toThrow('незакрытых отклонений');
        });
    });
});
