...
export class ConsultingDomainRules {
  ...
  async canActivate(planId: string, companyId?: string): Promise<void> {
    ...
    if (
      !planWithBudget?.activeBudgetPlanId ||
      planWithBudget.activeBudgetPlan?.status !== "LOCKED"
    ) {
      throw new BadRequestException(
  ...
  async canEditHarvestResult(
    ...
    if (plan.status === "ARCHIVE") {
      throw new BadRequestException(
        "Невозможно редактировать урожай для архивного плана",
      );
    }
  }
}
