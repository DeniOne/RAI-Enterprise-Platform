...
export class StrategicViewService {
    ...
    async getStrategicDashboard(seasonId: string, context: UserContext) {
        this.logger.log(`[STRATEGIC] Generating dashboard for season ${seasonId}, company ${context.companyId}`);

        // 1. Получаем агрегированные финансовые KPI (Pure Projection from Ledger)
        const financialKPIs = await this.kpiService.calculateStrategicKPIs(seasonId, context);
        ...
        const managementMetrics = {
            totalOpenDeviations: deviations.filter(d => d.status !== 'CLOSED').length,
            unresolvedRiskCount: deviations.filter(d => !d.decisions || d.decisions.length === 0).length,
            confirmedDecisionsCount: deviations.flatMap(d => d.decisions || []).filter(dec => dec.status === 'CONFIRMED').length,
            criticalDeviations: deviations.filter(d => d.severity === 'HIGH').length
        };
        ...
        return {
            header: {
                ...
                status: financialKPIs.activeReportingCount > 0 ? 'STABLE' : 'INITIALIZING'
            },
            // Финансовый слой
            ...
            deviationJournal: deviations.map(d => {
                ...
                return {
                    ...
                    status: d.status,
                    hasConfirmedDecision: d.decisions?.some(dec => dec.status === 'CONFIRMED') || false,
                    ...
                    budgetContext: d.budgetItem ? {
                        ...
                        deviation: Number(d.budgetItem.actualAmount) - Number(d.budgetItem.plannedAmount)
                    } : null
                };
            })
        };
