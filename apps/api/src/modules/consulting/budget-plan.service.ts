...
export class BudgetPlanService {
  ...
  async createBudget(harvestPlanId: string, context: UserContext) {
      ...
      include: {
        activeTechMap: {
          include: {
            ...
            stages: {
              include: { operations: { include: { resources: true } } },
            },
          },
        },
      },
    ...
    if (!plan.activeTechMap) {
      throw new BadRequestException(
        "Для создания бюджета необходима активная технологическая карта",
      );
    }
    ...
    const resources = plan.activeTechMap.stages.flatMap((s) =>
    ...
    const derivationHash = createHash("sha256")
      .update(this.deterministicStringify(hashInput))
      .digest("hex");
    ...
    const totalPlannedAmount = Array.from(categoryMap.values()).reduce(
      (sum, val) => sum + val,
      0,
    );
    ...
    return this.prisma.budgetPlan.create({
      data: {
        ...
        seasonId: plan.activeTechMap.seasonId,
        items: {
          create: Array.from(categoryMap.entries()).map(
            ([category, amount]) => ({
              category,
  ...
  async transitionStatus(
    ...
    switch (event) {
      case BudgetTransitionEvent.APPROVE:
        if (currentStatus !== BudgetStatus.DRAFT)
          throw new BadRequestException(
            "Бюджет должен быть в статусе DRAFT для утверждения",
          );
      ...
      case BudgetTransitionEvent.LOCK:
        if (currentStatus !== BudgetStatus.APPROVED)
          throw new BadRequestException(
            "Бюджет должен быть APPROVED для блокировки (LOCK)",
          );
        targetStatus = BudgetStatus.LOCKED;
        break;
      case BudgetTransitionEvent.CLOSE:
        if (currentStatus !== BudgetStatus.LOCKED)
          throw new BadRequestException("Только LOCKED бюджет можно закрыть");
  ...
  async updateBudget(budgetId: string, data: any, context: UserContext) {
    ...
    if (budget.status !== BudgetStatus.DRAFT) {
      throw new ForbiddenException(
        "Integrity Check Failed: Cannot update IMMUTABLE Budget (Status != DRAFT). Create a new version instead.",
      );
    }
  ...
  async syncActuals(budgetId: string, context: UserContext) {
    return withOptimisticLock(async () => {
      ...
      if (budget.status !== BudgetStatus.LOCKED) {
        throw new BadRequestException(
          "Синхронизация факта возможна только для LOCKED бюджета",
        );
      }
      ...
      if (hasOverflow) {
        ...
        await this.deviationService.createReview({
          ...
          aiImpactAssessment:
            "Финансовые риски: превышение запланированных лимитов производства. Требуется анализ эффективности операций.",
        });
      }
  ...
  async validateIntegrity(budgetId: string) {
    ...
    if (!budget.derivationHash) {
      throw new Error(
        `Integrity Violation: Budget ${budgetId} is missing derivationHash!`,
      );
    }
  }
