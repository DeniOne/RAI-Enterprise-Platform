...
import { TechMap, TechMapStatus, BudgetType, BudgetStatus } from '@rai/prisma-client';
...
export class BudgetGeneratorService {
    ...
    async generateOperationalBudget(techMapId: string, userId: string, companyId: string) {
        ...
        if (techMap.status !== TechMapStatus.ACTIVE) {
            throw new BadRequestException('Budget can only be generated from an ACTIVE TechMap');
        }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (existing) {
                ...
                throw new BadRequestException(`Operational Budget already exists for TechMap v${techMap.version}`);
            }
     ...
     */
    private calculateDerivationHash(mapId: string, version: number, lines: any[]): string {
