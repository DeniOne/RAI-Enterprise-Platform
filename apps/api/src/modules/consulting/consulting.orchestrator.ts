...
export class ConsultingOrchestrator {
  ...
  async handleOperationCompleted(event: ConsultingOperationCompletedEvent) {
    this.logger.log(
      `[ORCHESTRATOR] Handling completion of operation ${event.operationId} in execution ${event.executionId}`,
    );

    // 1. Создаем запись в логе (START)
    const orchestratorLog = await this.prisma.executionOrchestrationLog.create({
      // tenant-lint:ignore tenant scope is inherited via executionId relation
      data: {
    ...
    try {
      ...
      if (!budgetPlan) {
        ...
        await this.prisma.$transaction(async (tx) => {
          ...
          for (const transaction of transactions) {
            const category = this.mapResourceToCategory(
              (transaction as any).resourceType,
            );
            const budgetItem = budgetPlan.items.find(
            ...
            if (budgetItem) {
              await tx.budgetItem.update({
                ...
                data: {
                  actualAmount: {
                    increment: (transaction as any).totalCost || 0,
                  },
              ...
              outcome.warnings.push(
                `Категория ${category} не найдена в бюджете ${budgetPlan.id}`,
              );
            }
            ...
            try {
              ...
              this.logger.error(
                `Failed to ingest economic event for tx ${transaction.id}: ${e.message}`,
              );
              outcome.warnings.push(
                `Failed to create Ledger Entry for ${transaction.id}`,
              );
            }
          }
        });
      ...
      this.logger.error(
        `[ORCHESTRATOR] Failure during processing execution ${event.executionId}: ${error.message}`,
        error.stack,
      );
    } finally {
      // 5. Финализируем лог
      try {
        await this.prisma.executionOrchestrationLog.update({
          // tenant-lint:ignore tenant scope is inherited via orchestration log -> execution relation
        ...
        this.logger.error(
          `[CRITICAL] Failed to finalize orchestration log: ${logError.message}`,
        );
      }
    }
  }
