import { Controller, Get, Post, Body, Param, Patch, UseGuards, Query, ForbiddenException } from '@nestjs/common';
...
export class ConsultingController {
    ...
    private ensureManagementAccess(role: UserRole) {
        ...
        if (!allowed.includes(role)) {
            throw new ForbiddenException('Доступ разрешен только руководству (CEO/CFO)');
        }
    }
    ...
    private ensureStrategicAccess(role: UserRole) {
        const allowed: UserRole[] = [UserRole.CEO, UserRole.CFO, UserRole.ADMIN];
        if (!allowed.includes(role)) {
            throw new ForbiddenException('Доступ разрешен только стратегическому руководству (CEO/CFO/ADMIN)');
        }
    }
    ...
    async createGoal(@Body() dto: any, @CurrentUser() user: any) {
        ...
        const context: UserContext = { userId: user.userId, role: user.role, companyId: user.companyId };
    ...
    async simulate(@Param('id') id: string, @Body() dto: any, @CurrentUser() user: any) {
        this.ensureStrategicAccess(user.role);
        const context: UserContext = { userId: user.userId, role: user.role, companyId: user.companyId };
    ...
    async getAdvisory(@Param('seasonId') seasonId: string, @CurrentUser() user: any) {
        this.ensureStrategicAccess(user.role);
        const context: UserContext = { userId: user.userId, role: user.role, companyId: user.companyId };
    ...
    async getCashPosition(@Query('asOfDate') asOfDate: string, @CurrentUser() user: any) {
        this.ensureStrategicAccess(user.role);
        const context: UserContext = { userId: user.userId, role: user.role, companyId: user.companyId };
    ...
    async getCashProjection(
        ...
        return this.cashFlowService.getProjectedCashFlow(context, startDate, endDate);
    }

    @Get('cashflow/liquidity-risk/:seasonId')
    async getLiquidityRisk(@Param('seasonId') seasonId: string, @CurrentUser() user: any) {
        this.ensureStrategicAccess(user.role);
        const context: UserContext = { userId: user.userId, role: user.role, companyId: user.companyId };
    ...
    async supersedeDecision(@Param('id') id: string, @Body() dto: any, @CurrentUser() user: any) {
        this.ensureManagementAccess(user.role);
        const context: UserContext = { userId: user.userId, role: user.role, companyId: user.companyId };
        return this.decisionService.supersede(id, dto.description, dto.expectedEffect, context);
    }
    ...
    async startOperation(@Param('operationId') operationId: string, @CurrentUser() user: any) {
        return this.executionService.startOperation(operationId, { userId: user.userId, companyId: user.companyId });
    }

    @Post('execution/complete')
    async completeOperation(@Body() dto: CompleteOperationDto, @CurrentUser() user: any) {
        return this.executionService.completeOperation(dto, { userId: user.userId, companyId: user.companyId });
    }

    @Get('execution/operations')
    async getActiveOperations(@CurrentUser() user: any) {
        return this.executionService.getActiveOperations({ userId: user.userId, companyId: user.companyId });
    }
    ...
    async getStrategicDashboard(@Param('seasonId') seasonId: string, @CurrentUser() user: any) {
        this.ensureManagementAccess(user.role);
        const context: UserContext = { userId: user.userId, role: user.role, companyId: user.companyId };
    ...
    async createDecisionDraft(@Body() dto: any, @CurrentUser() user: any) {
        this.ensureManagementAccess(user.role);
        const context: UserContext = { userId: user.userId, role: user.role, companyId: user.companyId };
        return this.decisionService.createDraft(dto.deviationId, dto.description, dto.expectedEffect, context);
    }
}
