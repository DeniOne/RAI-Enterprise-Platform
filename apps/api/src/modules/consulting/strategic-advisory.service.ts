...
export class StrategicAdvisoryService {
    ...
    async getAdvisory(seasonId: string, context: UserContext) {
        const baseline = await this.kpiService.calculateStrategicKPIs(seasonId, context);
        const risks: AdvisoryRisk[] = [];

        // 1. Анализ чувствительности (Yield Sensitivity)
        // Что если урожайность упадет на 10%?
        const yieldStress = await this.simulationService.simulateSeason(seasonId, { yieldDeviationPercent: -10 }, context);
        if (yieldStress.simulated.roi < 5) {
            risks.push({
                score: 75,
                level: 'HIGH',
                reason: 'Высокая чувствительность к урожайности. Падение на 10% делает проект нерентабельным (ROI < 5%)',
                linkedKpi: 'ROI',
                recommendation: 'Рассмотреть страхование урожая или пересмотр структуры затрат.'
            });
        }

        // 2. АНАЛИЗ ЛИКВИДНОСТИ (Phase 5: Financial Stability)
        const liquidity = await this.liquidityRisk.analyzeLiquidityRisk(context, seasonId);
        ...
        const budget = await this.prisma.budgetPlan.findFirst({
            where: { seasonId, companyId: context.companyId, status: BudgetStatus.LOCKED },
            include: { items: true }
        });
        ...
        if (budget) {
            ...
            if (actualCost.gt(plannedCost.mul(1.05))) {
                risks.push({
                    ...
                    linkedKpi: 'Cost',
                    recommendation: 'Срочный аудит Ledger-записей по категории Fertilizer/Fuel.'
                });
            }
        }

        // 4. Анализ рыночной волатильности
        const priceStress = await this.simulationService.simulateSeason(seasonId, { priceDeviationPercent: -15 }, context);
        if (priceStress.delta.ebitda < -1000000) { // Порог 1 млн (условный)
            risks.push({
                score: 60,
                level: 'MEDIUM',
                reason: 'Прогноз значительного падения прибыли при коррекции рыночных цен на 15%',
                linkedKpi: 'EBITDA',
                recommendation: 'Рекомендуется фиксация цены через форвардные контракты.'
            });
        }
