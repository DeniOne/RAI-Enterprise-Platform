...
export class ConsultingService {
  ...
  async updateDraftPlan(
    ...
    if (plan.status !== HarvestPlanStatus.DRAFT) {
      throw new ForbiddenException(
        "Р РµРґР°РєС‚РёСЂРѕРІР°С‚СЊ РјРѕР¶РЅРѕ С‚РѕР»СЊРєРѕ DRAFT-РїР»Р°РЅС‹",
      );
    }
  ...
  async transitionPlanStatus(
    id: string,
    targetStatus: HarvestPlanStatus,
    context: UserContext,
  ) {
    const plan = await this.prisma.harvestPlan.findFirst({
      where: { id, companyId: context.companyId },
    ...
    try {
      ...
      if (updateResult.count !== 1) {
        throw new ConflictException(
          `РЎС‚Р°С‚СѓСЃ РїР»Р°РЅР° Р±С‹Р» РёР·РјРµРЅРµРЅ РґСЂСѓРіРёРј РїСЂРѕС†РµСЃСЃРѕРј. РўРµРєСѓС‰РёР№ СЃС‚Р°С‚СѓСЃ: ${currentStatus}`,
        );
      }
    ...
    this.logger.log(
      `[CONSULTING] Plan ${id}: ${currentStatus} в†’ ${targetStatus} by ${context.role}`,
    );
    return updatedPlan;
  }
  ...
  private async validateTransition(
    ...
    if (
      current === HarvestPlanStatus.DRAFT &&
      target === HarvestPlanStatus.REVIEW
    ) {
      return;
    }
    ...
    if (
      current === HarvestPlanStatus.REVIEW &&
      target === HarvestPlanStatus.APPROVED
    ) {
      if (context.role !== UserRole.CEO && context.role !== UserRole.ADMIN) {
        throw new ForbiddenException(
          "РўРѕР»СЊРєРѕ CEO РјРѕР¶РµС‚ СѓС‚РІРµСЂР¶РґР°С‚СЊ РїР»Р°РЅС‹",
        );
      }
      return;
    }
    ...
    if (
      current === HarvestPlanStatus.APPROVED &&
      target === HarvestPlanStatus.ACTIVE
    ) {
      if (context.role !== UserRole.CEO && context.role !== UserRole.ADMIN) {
        throw new ForbiddenException(
          "РўРѕР»СЊРєРѕ CEO РјРѕР¶РµС‚ Р°РєС‚РёРІРёСЂРѕРІР°С‚СЊ РїР»Р°РЅС‹",
        );
      }
    ...
    if (
      current === HarvestPlanStatus.ACTIVE &&
      target === HarvestPlanStatus.DONE
    ) {
      return;
    }
    ...
    if (
      current === HarvestPlanStatus.DONE &&
      target === HarvestPlanStatus.ARCHIVE
    ) {
    ...
    throw new BadRequestException(
      `РќРµРґРѕРїСѓСЃС‚РёРјС‹Р№ РїРµСЂРµС…РѕРґ РёР· ${current} РІ ${target}`,
    );
  }
  ...
  async openConsultationThread(deviationReviewId: string, companyId: string) {
    this.logger.log(
      `[CONSULTING] РћС‚РєСЂС‹С‚РёРµ РєРѕРЅС‚РµРєСЃС‚РЅРѕРіРѕ С‚СЂРµРґР° РґР»СЏ deviation ${deviationReviewId}`,
    );
    ...
    if (review.telegramThreadId) {
      this.logger.warn(
        `[CONSULTING] РўСЂРµРґ СѓР¶Рµ СЃСѓС‰РµСЃС‚РІСѓРµС‚ РґР»СЏ deviation ${deviationReviewId}`,
      );
    ...
    if (updated.count !== 1) {
      throw new ConflictException(
        `DeviationReview ${deviationReviewId} update conflict`,
      );
    }

    this.logger.log(
      `[CONSULTING] РљРѕРЅС‚РµРєСЃС‚РЅС‹Р№ С‚СЂРµРґ СЃРѕР·РґР°РЅ: ${virtualThreadId}`,
    );
  ...
  async logMessage(
    ...
    if (!review) {
      this.logger.error(
        `[LAW-VIOLATION] РЎРѕРѕР±С‰РµРЅРёРµ Р±РµР· РІР°Р»РёРґРЅРѕРіРѕ РєРѕРЅС‚РµРєСЃС‚РЅРѕРіРѕ С‚СЂРµРґР°: ${threadId}`,
      );
      return;
    }

    this.logger.log(
      `[CONSULTING] РўСЂР°СЃСЃРёСЂРѕРІРєР°: РђРІС‚РѕСЂ ${authorId} в†’ Review ${review.id}`,
    );
  }
}
