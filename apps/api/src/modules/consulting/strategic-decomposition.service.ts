...
export class StrategicDecompositionService {
  ...
  async decomposeGoal(goalId: string, context: UserContext) {
    ...
    const baselineKpi = await this.kpiService.calculateStrategicKPIs(
      goal.seasonId,
      context,
    );
  ...
  private getCurrentValue(type: GoalType, kpi: any): number {
    switch (type) {
      case GoalType.EBITDA_TARGET:
        return kpi.ebitda;
      case GoalType.ROI_TARGET:
        return kpi.roi;
      case GoalType.COST_LIMIT:
        return kpi.actualCost;
      case GoalType.YIELD_TARGET:
        return 0; // Требует агрегации по культурам
      default:
        return 0;
    }
  }
