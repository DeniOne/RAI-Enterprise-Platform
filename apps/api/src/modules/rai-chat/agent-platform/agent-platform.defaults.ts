...
const GOVERNANCE_POLICIES: Record<CanonicalAgentRuntimeRole, AgentGovernancePolicy> = {
  ...
  contracts_agent: {
    ...
    criticalActionRules: ["deny_unreviewed_financial_posting_actions"],
    auditRequirements: ["trace", "evidence", "validation", "gate_status"],
...
export function buildDefaultRuntimeProfile(
  ...
  return {
    ...
    temperature:
      role === "monitoring"
        ? 0.1
        : role === "crm_agent" || role === "front_office_agent" || role === "contracts_agent"
          ? 0.15
          : 0.2,
    timeoutMs: 15_000,
...
export function buildDefaultDefinition(
  ...
  return {
    ...
    kind:
      baseRole === "monitoring" ||
      baseRole === "crm_agent" ||
      baseRole === "front_office_agent" ||
      baseRole === "contracts_agent"
        ? "worker_hybrid"
        : "domain_advisor",
    ownerDomain,
    description,
