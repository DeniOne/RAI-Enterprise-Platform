...
export class SemanticRouterService {
  ...
  private isCrmWorkspaceReviewQuery(
    ...
    if (
      /(обнови|измени|удали|создай|добавь|зарегистр|заре[гп]|оформи|логируй|зафиксируй|завед(и|ите|ём|ем))/i.test(
        message,
      )
    ) {
      return false;
  ...
  private isCrmInnLookupQuery(message: string): boolean {
    if (
      /(созд(ай|ать)|зарегистр|заре[гп]|добавь|обнови|измени|удали|оформи|завед(и|ите|ём|ем))/i.test(
        message,
      )
    ) {
      return false;
