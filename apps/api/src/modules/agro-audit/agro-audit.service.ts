            ...
            try {
                ...
                if (attempt < maxRetries - 1) {
                    try {
                        ...
                        this.logger.error('CRITICAL: Failed to log audit failure itself', failLogErr);
                    }
                }
            }
        }

        throw new Error(`Failed to log audit event after ${maxRetries} attempts: ${lastError?.message}`);
    }
