...
export class WormStorageService {
    ...
    private initS3() {
        ...
        if (bucket && region) {
            this.logger.log(`Initialized WORM S3 Client for bucket: ${bucket} in ${region}`);
    ...
    async uploadImmutableObject(
        ...
        this.logger.log(`Uploading to WORM Storage -> Key: ${key}, Retention: ${retentionYears} years`);
