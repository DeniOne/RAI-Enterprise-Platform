...
export class RatingEngineService {
  ...
  public evaluateSnapshot(payload: SnapshotPayload): RatingResult {
    ...
    if (!fenceResult.passed) {
      this.logger.warn(
        `Snapshot Fences Failed. Assigning Grade D for company ${payload.companyId}`,
      );
      return {
    ...
    this.logger.log(
      `Certification Complete for company ${payload.companyId}: Grade ${grade} (Score: ${aggregatedScore})`,
    );
