...
import {
  JwtMinterService,
  LevelFCertificatePayload,
} from "./jwt-minter.service";
...
export class ReproducibilityCheckerService {
  ...
  public async rebuildAndVerifyCertificate(
    ...
    this.logger.log(
      `Rebuilding Certificate ${existingJwtPayload.jti} for company ${companyId}`,
    );
    ...
    try {
      ...
      if (!isGradeMatch || !isScoreMatch) {
        this.logger.error(
          `Reproducibility Failed: Diff found in grades/scores`,
        );
      ...
      this.logger.log(
        `Deterministic Rebuild Successful. Exact match confirmed.`,
      );
