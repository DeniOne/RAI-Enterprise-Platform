...
export class AssertionFencesService {
    ...
    public evaluateFences(payload: SnapshotPayload): FenceResult {
        ...
        this.logger.debug(`Evaluating Assertion Fences for Snapshot: ${payload.previousHash}`);
        ...
        if (!payload.rawSource || payload.rawSource.length === 0) {
            violations.push('I40: Payload lacks rawSource (Data Availability Requirement)');
        ...
        if (aggregateMetrics.financialRisk > this.MAX_FR_RISK) {
            violations.push(`FR_OVERFLOW: Financial Risk (${aggregateMetrics.financialRisk}) exceeds threshold (${this.MAX_FR_RISK})`);
        }

        // Fence 2: Калибровка Риска (Level C Regret bounds)
        if (aggregateMetrics.riskCalibrationScore < this.MIN_RCS) {
            violations.push(`RCS_UNDERFLOW: Risk Calibration Score (${aggregateMetrics.riskCalibrationScore}) is below minimum (${this.MIN_RCS})`);
        }
