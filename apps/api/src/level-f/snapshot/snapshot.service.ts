...
export class SnapshotService {
  ...
  async createSnapshot(params: {
    ...
    this.logger.log(
      `Building Snapshot for company ${params.companyId} [${params.startDate.toISOString()} - ${params.endDate.toISOString()}]`,
    );
  ...
  validateIncomingSnapshot(
    ...
    if (skew > maxSkewMs) {
      this.logger.error(
        `Temporal Skew Error: ${skew}ms exceeds max ${maxSkewMs}ms`,
      );
    ...
    if (node.payload.previousHash !== this.dagHeadHash) {
      this.logger.error(
        `DAG Continuity Error: Invalid previous hash. Received: ${node.payload.previousHash}, Expected: ${this.dagHeadHash}`,
      );
    ...
    if (computedHash !== node.hash) {
      this.logger.error(
        `Hash Mismatch. Received: ${node.hash}, Computed: ${computedHash}`,
      );
      return false;
    }
