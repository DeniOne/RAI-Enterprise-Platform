    ...
    return next.handle().pipe(
      tap(() => {
        ...
        if (executionTime > this.SLO_LATENCY_BUDGET_MS) {
          this.logger.warn(
            `SLO Violation [Latency]: ${req.method} ${req.url} took ${executionTime}ms (Budget: ${this.SLO_LATENCY_BUDGET_MS}ms)`,
          );
      ...
      catchError((err) => {
        ...
        this.logger.error(
          `SLO Violation [Error]: ${req.method} ${req.url} failed after ${executionTime}ms`,
        );
