...
export class AnchorService {
    ...
    async publishDailyRootHash() {
        ...
        this.logger.log(`Starting daily Anchoring process... Active Provider: ${activeProvider}`);

        // Заглушка получения свежего HeadHash из SnapshotService
        const mockMerkleRootHash = '0x' + Buffer.from(Date.now().toString()).toString('hex').padEnd(64, '0').slice(0, 64);
        ...
        try {
            if (activeProvider === AnchorProvider.PRIMARY_L1_EVM) {
                ...
                this.logger.log(`[EVM L1] Successfully anchored daily root: ${mockMerkleRootHash}`);
            } else {
                // Secondary Hyperledger/Consortium API:
                // await fabricService.invokeTransaction('anchorSnapshot', mockMerkleRootHash)
                this.logger.log(`[Consortium L2] Successfully anchored fallback daily root: ${mockMerkleRootHash}`);
            }
        } catch (error) {
