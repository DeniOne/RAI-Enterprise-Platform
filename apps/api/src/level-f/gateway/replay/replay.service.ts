...
export class ReplayService {
     ...
     */
    async verifyReplay(recordedHash: string, payload: any): Promise<{ isMatch: boolean, replayedHash: string }> {
        ...
        const replayedHash = crypto.createHash('sha256').update(canonicalJson).digest('hex');

        // Шаг 3: Сверка хешей
        const isMatch = (recordedHash === replayedHash);

        if (isMatch) {
            this.logger.log(`Dispute resolved. Replay MATCH for hash: ${recordedHash}`);
        } else {
            this.logger.warn(`Dispute DIVERGENCE! Recorded: ${recordedHash} != Replayed: ${replayedHash}`);
        }
