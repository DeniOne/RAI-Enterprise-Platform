...
export class HsmService {
    ...
    constructor() {
        if (this.IS_DEV_MODE) {
            this.logger.warn(`[HSM] Running in DEV mode. Using local in-memory Ed25519 keypair instead of Vault.`);
            const { privateKey, publicKey } = crypto.generateKeyPairSync('ed25519');
            this.devPrivateKey = privateKey;
            this.devPublicKey = publicKey;
        } else {
            this.logger.log(`[HSM] Connected to HashiCorp Vault (Transit Engine). Keys remain strictly in enclave.`);
        }
    }
     ...
     */
    async signEd25519(payload: string, keyName: string = 'institutional-jwt-mstr'): Promise<string> {
        ...
        throw new Error('HSM Proxy not fully implemented for Production (Missing Vault Token)');
    }

    /**
     * Извлечение Публичного ключа для передачи в цепочку сертификатов
     */
    async exportPublicKey(keyName: string = 'institutional-jwt-mstr'): Promise<string> {
        if (this.IS_DEV_MODE) {
            return this.devPublicKey.export({ format: 'pem', type: 'spki' }).toString();
        }
