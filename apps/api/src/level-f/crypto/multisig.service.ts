...
export class MultisigService {
  ...
  createSession(action: GovernanceAction, payload: any): MultisigSession {
    ...
    this.logger.warn(
      `[M-of-N] New GOVERNANCE Session created: ${sessionId} | Action: ${action} | Threshold: ${session.targetThreshold}/${session.totalSigners}`,
    );
    return session;
  }
  ...
  provideSignature(
    ...
    if (session.signatures.has(signerId)) {
      throw new HttpException(
        "Signer already approved this session",
        HttpStatus.CONFLICT,
      );
    }
    ...
    this.logger.log(
      `[M-of-N] Session ${sessionId}: Signature added by ${signerId} (${session.signatures.size}/${session.targetThreshold} required)`,
    );
  ...
  isActionExecutable(sessionId: string): boolean {
    const session = this.getSession(sessionId);
    return (
      session.signatures.size >= session.targetThreshold && !session.executed
    );
  }
  ...
  markExecuted(sessionId: string) {
    ...
    if (!this.isActionExecutable(sessionId)) {
      throw new HttpException(
        "Cannot execute: threshold not met",
        HttpStatus.FORBIDDEN,
      );
    }
    session.executed = true;
    this.logger.warn(
      `[M-of-N] Session ${sessionId} EXECUTED! Activating strictly gated logic.`,
    );
  }
