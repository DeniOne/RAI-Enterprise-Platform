...
export class OutboxRelay implements OnApplicationBootstrap {
    ...
    private readonly retryBaseDelayMs = Number(process.env.OUTBOX_RETRY_BASE_DELAY_MS || 1000);
    private readonly orderingDeferMs = Number(process.env.OUTBOX_ORDERING_DEFER_MS || 1000);
    private readonly deliveryMode = (process.env.OUTBOX_DELIVERY_MODE || 'local_only').toLowerCase();
    private readonly enforceEventContract = (process.env.OUTBOX_EVENT_CONTRACT_ENFORCE || 'true').toLowerCase() !== 'false';
            ...
            for (const msg of messages) {
                try {
                    ...
                    if (!this.hasTenantContract(msg)) {
                        InvariantMetrics.incrementTenantViolation('UNKNOWN_TENANT', 'OutboxRelay');
                    ...
                    if (!this.hasValidEventContract(msg)) {
                        ...
                        if (this.enforceEventContract) {
                            await this.prisma.outboxMessage.update({
                                where: { id: msg.id },
                                data: { status: OutboxStatus.FAILED, error: err, deadLetterAt: new Date() },
                            });
                            continue;
                        }
                    ...
                    if (attempts >= this.maxRetries) {
                        this.logger.error(`Failed to process message ${msg.id}: ${errorMessage} (DLQ)`);
                        await (this.prisma as any).outboxMessage.update({
                            where: { id: msg.id },
                            data: {
    ...
    private async publishToBroker(msg: any) {
        ...
        this.logger.log(`[Broker] Published: ${msg.type} (Agg: ${msg.aggregateId || 'N/A'})`);
    }
