...
export class S3Service implements OnModuleInit {
     ...
     */
    async validateObjectIntegrity(path: string, expectedHash: string): Promise<boolean> {
        try {
            ...
            this.logger.debug(`🔍 Validating S3 object: bucket=${bucket}, key=${key}`);

            const info = await this.client.statObject(bucket, key);

            // Проверка на атомарность / полноту загрузки
            if (info.size === 0) {
                this.logger.error(`❌ S3 object ${key} is empty (0 bytes)! Partial upload suspected.`);
                return false;
            }

            this.logger.debug(`✅ S3 object validated: size=${info.size}, etag=${info.etag}`);

            // Сравнение хеша (ETag)
            if (expectedHash && info.etag !== expectedHash && !info.etag.includes(expectedHash)) {
                this.logger.warn(`⚠️ Hash mismatch for ${key}. Expected: ${expectedHash}, Got: ${info.etag}`);
                return false;
            }

            return true;
        } catch (error) {
            this.logger.error(`❌ S3 Integrity Check failed for ${path}: ${error.message}`);
            return false;
        }
    }
}
