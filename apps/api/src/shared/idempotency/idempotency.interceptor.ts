...
export class IdempotencyInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
        ...
        if (!idempotencyKey) {
            ...
            this.logger.warn(`Missing Idempotency-Key or jti for ${request.method} ${request.url}`);
        ...
        if (cachedState) {
            ...
            if (stateObj.status === 'IN_PROGRESS') {
                throw new HttpException('Request is already in progress', HttpStatus.CONFLICT);
            }
            ...
            if (stateObj.status === 'ERROR') {
                throw new HttpException(stateObj.error.message, stateObj.error.status || HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }
        ...
        if (!acquired) {
            throw new HttpException('Request is already in progress (concurrent)', HttpStatus.CONFLICT);
        }
        ...
        return next.handle().pipe(
            ...
            catchError(async (err) => {
                ...
                const httpError = err instanceof HttpException ? err : new HttpException('Internal Error', 500);

                await this.redisService.set(
                    cacheKey,
                    JSON.stringify({
                        status: 'ERROR',
                        error: { message: httpError.message, status: httpError.getStatus() }
                    }),
                    86400 // 24 hours
                );
