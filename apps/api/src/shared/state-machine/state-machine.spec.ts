...
describe("TaskStateMachine", () => {
  describe("canTransition", () => {
    it("should allow START from PENDING", () => {
      expect(
        TaskStateMachine.canTransition(TaskStatus.PENDING, TaskEvent.START),
      ).toBe(true);
    });

    it("should allow COMPLETE from IN_PROGRESS", () => {
      expect(
        TaskStateMachine.canTransition(
    ...
    it("should NOT allow START from COMPLETED", () => {
      expect(
        TaskStateMachine.canTransition(TaskStatus.COMPLETED, TaskEvent.START),
      ).toBe(false);
    });

    it("should NOT allow COMPLETE from PENDING", () => {
      expect(
        TaskStateMachine.canTransition(TaskStatus.PENDING, TaskEvent.COMPLETE),
      ).toBe(false);
    });
  });
  ...
  describe("transition", () => {
    ...
    it("should throw on invalid transition", () => {
      ...
      expect(() =>
        TaskStateMachine.transition(entity, TaskEvent.START),
...
describe("AplStateMachine", () => {
  describe("canTransition", () => {
    ...
    it("should NOT allow ADVANCE from POST_HARVEST (terminal)", () => {
      expect(
        AplStateMachine.canTransition(AplStage.POST_HARVEST, AplEvent.ADVANCE),
      ).toBe(false);
    });
  ...
  describe("transition", () => {
    it("should advance SOIL_PREPARATION → FERTILIZATION_BASE", () => {
      const entity: SeasonEntity = {
        id: "season-1",
        currentStageId: AplStage.SOIL_PREPARATION,
      };
    ...
    it("should skip desiccation FULL_MATURITY → HARVEST", () => {
      const entity: SeasonEntity = {
        id: "season-2",
        currentStageId: AplStage.FULL_MATURITY,
      };
    ...
    it("should throw on invalid transition", () => {
      const entity: SeasonEntity = {
        id: "season-3",
        currentStageId: AplStage.POST_HARVEST,
      };

      expect(() =>
        AplStateMachine.transition(entity, AplEvent.ADVANCE),
      ).toThrow(InvalidTransitionError);
    });
  ...
  describe("getAvailableEvents", () => {
    it("should return ADVANCE for SOIL_PREPARATION", () => {
      const events = AplStateMachine.getAvailableEvents(
        AplStage.SOIL_PREPARATION,
      );
