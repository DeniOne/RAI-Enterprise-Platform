...
export function isTransitionAllowed(policy: FsmPolicyKey, state: string, event: string): boolean {
    const matrix = FSM_TRANSITION_POLICIES[policy];
    const allowedEvents = matrix[state] || [];
    return allowedEvents.includes(event);
}

export function assertTransitionAllowed(policy: FsmPolicyKey, state: string, event: string): void {
