import { assertTransitionAllowed, isTransitionAllowed } from './fsm-transition-policy';
...
describe('FSM transition policy registry', () => {
    ...
    it('blocks invalid task transitions', () => {
        ...
        expect(() => assertTransitionAllowed('TASK', 'COMPLETED', 'START')).toThrow(InvalidTransitionError);
    });
    ...
    it('blocks invalid budget transitions', () => {
        ...
        expect(() => assertTransitionAllowed('BUDGET', 'APPROVED', 'CLOSE')).toThrow(InvalidTransitionError);
    });
});
