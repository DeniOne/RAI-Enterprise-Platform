    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
      <Card>
        ...
        <div className="mt-6 space-y-3 text-sm text-gray-600">
          <p>Статус: {formatStatusLabel(task.status)}</p>
          <p>Поле: {task.field?.name ?? "—"}</p>
          <p>Сезон: {task.season?.year ?? "—"}</p>
          <p>Факт. ресурсы: {task.actualResources?.length ?? 0}</p>
        </div>
      </Card>
      <Card>
        <h3 className="text-lg font-medium text-gray-900">Подтверждения и наблюдения</h3>
        <div className="mt-4 space-y-3">
          {observations.length === 0 ? (
            <p className="text-sm text-gray-500">По задаче пока нет наблюдений.</p>
          ) : (
            observations.map((item: any) => (
              <div key={item.id} className="rounded-2xl border border-black/5 p-4">
                <p className="text-sm font-medium text-gray-900">
                  {formatObservationTypeLabel(item.type)} • {formatObservationIntentLabel(item.intent)}
                </p>
                <p className="mt-1 text-xs text-gray-500">{formatEvidenceIntegrityLabel(item.integrityStatus)}</p>
