                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <ExplanationLayer
                        reasons={data.risk?.explanation?.signals?.map((s: any) => ({
                            type: s.source,
                            title: s.reasonCode, // Or map to a friendly title if available
                            description: s.description,
                            ref: s.refId,
                            version: '1.0' // Placeholder or from signal
                        })) || []}
                    />
                    ...
                    <div className="p-8 rounded-xl bg-gradient-to-br from-white/[0.03] to-transparent border border-white/5 flex flex-col justify-center items-center text-center space-y-4">
                        ...
                        <div className={`text-4xl font-light ${data.risk?.verdict === 'BLOCKED' ? 'text-red-500' : ''}`}>
                        ...
                        {data.risk?.explanation?.previous && (
                            <div className="text-[9px] uppercase tracking-widest opacity-20">
                                Previously: {data.risk.explanation.previous}
                            </div>
                        )}
                    </div>
                </div>
