...
function assert(condition, message) {
  if (!condition) {
    console.error(`[phase-e-pilot-transition] ${message}`);
    process.exit(1);
  }
}
...
function printUsage() {
  console.log(
    "usage: node scripts/phase-e-pilot-transition.cjs --reference=E-H-01 --status=reviewed|accepted|expired [--at=2026-04-01] [--dry-run]",
  );
}
...
function main() {
  ...
  console.log(`[phase-e-pilot-transition] reference=${referenceId}`);
  console.log(`[phase-e-pilot-transition] current_status=${currentStatus}`);
  console.log(`[phase-e-pilot-transition] target_status=${targetStatus}`);
  console.log(`[phase-e-pilot-transition] at=${at}`);
  console.log(`[phase-e-pilot-transition] metadata_file=${metadataFile.replace(/\\/g, "/")}`);
  console.log(`[phase-e-pilot-transition] dry_run=${dryRun ? "yes" : "no"}`);

  writeText(metadataFile, metadataContent, dryRun);

  console.log(`[phase-e-pilot-transition] status=${currentStatus}->${targetStatus}`);
}
