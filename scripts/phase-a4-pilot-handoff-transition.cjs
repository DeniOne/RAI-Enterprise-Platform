...
const RESTRICTED_ROOT = process.env.PHASE_A4_HANDOFF_ROOT
  ? path.resolve(process.env.PHASE_A4_HANDOFF_ROOT)
  : path.resolve(ROOT, "..", "RAI_EP_RESTRICTED_EVIDENCE", "pilot-handoffs", "2026-03-31");
...
function assert(condition, message) {
  if (!condition) {
    console.error(`[phase-a4-handoff-transition] ${message}`);
    process.exit(1);
  }
}
...
function printUsage() {
  console.log(
    "usage: node scripts/phase-a4-pilot-handoff-transition.cjs --reference=A4-H-01 --status=reviewed|accepted|expired [--at=2026-03-31] [--dry-run]",
  );
}
...
function main() {
  ...
  assert(/^A4-H-\d{2}$/.test(referenceId), "reference должен быть в формате A4-H-01");
  ...
  console.log(`[phase-a4-handoff-transition] reference=${referenceId}`);
  console.log(`[phase-a4-handoff-transition] current_status=${currentStatus}`);
  console.log(`[phase-a4-handoff-transition] target_status=${targetStatus}`);
  console.log(`[phase-a4-handoff-transition] at=${at}`);
  console.log(`[phase-a4-handoff-transition] metadata_file=${metadataFile.replace(/\\/g, "/")}`);
  console.log(`[phase-a4-handoff-transition] dry_run=${dryRun ? "yes" : "no"}`);

  writeText(metadataFile, metadataContent, dryRun);

  console.log(`[phase-a4-handoff-transition] status=${currentStatus}->${targetStatus}`);
}
