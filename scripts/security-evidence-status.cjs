...
const REPORT_JSON = path.join(OUTPUT_DIR, "security-evidence-status.json");
const REPORT_MD = path.join(OUTPUT_DIR, "security-evidence-status.md");
const METADATA_DIR = process.env.SECURITY_EVIDENCE_METADATA_DIR
  ? path.resolve(process.env.SECURITY_EVIDENCE_METADATA_DIR)
  : path.resolve(ROOT, "..", "RAI_EP_RESTRICTED_EVIDENCE", "security", "2026-03-31", "metadata");
...
function parseMetadataFile(filePath) {
  ...
  const refMatch = basename.match(/^(A2-S-\d{2})-/);
...
function main() {
  ...
  if (!fs.existsSync(METADATA_DIR)) {
    console.error(`[security-evidence-status] отсутствует metadata dir: ${METADATA_DIR}`);
    process.exit(1);
  }
  if (!fs.existsSync(INDEX_FILE)) {
    console.error(`[security-evidence-status] отсутствует index file: ${INDEX_FILE}`);
    process.exit(1);
  }

  const indexItems = parseIndex(readText(INDEX_FILE));
  const indexMap = new Map(indexItems.map((item) => [item.referenceId, item]));
  const metadataFiles = fs
    .readdirSync(METADATA_DIR)
    .filter((name) => /^A2-S-\d{2}.*\.md$/.test(name))
    .sort();
  ...
  for (const item of metadataItems) {
    ...
    if (
      ...
      fields.artifact_path !== "pending" &&
      !fs.existsSync(fields.artifact_path)
    ) {
  ...
  console.log("[security-evidence-status] summary");
  ...
  if (mode === "enforce" && (issues.length > 0 || counts.overdue > 0)) {
    process.exit(1);
  }
}
