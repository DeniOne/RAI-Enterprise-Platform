...
const REPORT_JSON = path.join(OUTPUT_DIR, "phase-a4-pilot-handoff-status.json");
const REPORT_MD = path.join(OUTPUT_DIR, "phase-a4-pilot-handoff-status.md");
...
function parseMetadataFile(filePath) {
  ...
  const refMatch = basename.match(/^(A4-H-\d{2})-/);
...
function main() {
  ...
  if (!fs.existsSync(METADATA_DIR)) {
    console.error(`[phase-a4-handoff-status] отсутствует metadata dir: ${METADATA_DIR}`);
    process.exit(1);
  }
  if (!fs.existsSync(INDEX_FILE)) {
    console.error(`[phase-a4-handoff-status] отсутствует index file: ${INDEX_FILE}`);
    process.exit(1);
  }
  ...
  const metadataFiles = fs
    ...
    .filter((name) => /^A4-H-\d{2}.*\.md$/.test(name))
    .sort();
  ...
  console.log("[phase-a4-handoff-status] summary");
