...
function assert(condition, message) {
  if (!condition) {
    console.error(`[security-evidence-transition] ${message}`);
    process.exit(1);
  }
}
...
function printUsage() {
  console.log(
    "usage: node scripts/security-evidence-transition.cjs --reference=A2-S-01 --status=reviewed|accepted|expired [--at=2026-03-31] [--dry-run]",
  );
}
...
function main() {
  ...
  console.log(`[security-evidence-transition] reference=${referenceId}`);
  console.log(`[security-evidence-transition] current_status=${currentStatus}`);
  console.log(`[security-evidence-transition] target_status=${targetStatus}`);
  console.log(`[security-evidence-transition] at=${at}`);
  console.log(`[security-evidence-transition] metadata_file=${metadataFile.replace(/\\/g, "/")}`);
  console.log(`[security-evidence-transition] dry_run=${dryRun ? "yes" : "no"}`);

  writeText(metadataFile, metadataContent, dryRun);

  console.log(`[security-evidence-transition] status=${currentStatus}->${targetStatus}`);
}
