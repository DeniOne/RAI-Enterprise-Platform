...
import { PrismaClient, TechMapStatus } from '@rai/prisma-client';
import { UnitNormalizationService } from '../apps/api/src/modules/consulting/unit-normalization.service';
import { TechMapValidator } from '../apps/api/src/modules/consulting/tech-map.validator';

const prisma = new PrismaClient();
const unitService = new UnitNormalizationService();
const validator = new TechMapValidator(unitService);
...
async function main() {
    ...
    try {
        ...
        if (tonToKg.value !== 1500 || tonToKg.unit !== 'kg') throw new Error('Ton -> Kg failed');
        console.log('✅ Ton -> Kg: OK (1.5t -> 1500kg)');

        const haToHa = unitService.normalize(100, 'ha');
        if (haToHa.value !== 100 || haToHa.unit !== 'ha') throw new Error('Ha Identity failed');
