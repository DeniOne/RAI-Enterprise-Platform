...
function assert(condition, message) {
  if (!condition) {
    console.error(`[phase-a4-handoff-intake] ${message}`);
    process.exit(1);
  }
}
...
function printUsage() {
  console.log(
    "usage: node scripts/phase-a4-pilot-handoff-intake.cjs --reference=A4-H-01 --source=/abs/path/file.md [--received-at=2026-03-31] [--copy-name=custom.md] [--dry-run]",
  );
}
...
function main() {
  ...
  console.log(`[phase-a4-handoff-intake] reference=${referenceId}`);
  console.log(`[phase-a4-handoff-intake] source=${source}`);
  console.log(`[phase-a4-handoff-intake] target=${targetFile.replace(/\\/g, "/")}`);
  console.log(`[phase-a4-handoff-intake] metadata_file=${metadataFile.replace(/\\/g, "/")}`);
  console.log(`[phase-a4-handoff-intake] dry_run=${dryRun ? "yes" : "no"}`);
  ...
  console.log("[phase-a4-handoff-intake] status=requested->received");
  console.log("[phase-a4-handoff-intake] next_action=owner review");
}
