...
const REPORT_JSON = path.join(OUTPUT_DIR, "phase-e-pilot-status.json");
const REPORT_MD = path.join(OUTPUT_DIR, "phase-e-pilot-status.md");
...
function parseMetadataFile(filePath) {
  ...
  const refMatch = basename.match(/^(E-H-\d{2})-/);
...
function computeVerdict(status) {
  return status === "done" ? "pilot_ready_tier2" : "pilot_blocked_tier2";
}
...
function main() {
  ...
  const metadataFiles = fs.existsSync(METADATA_DIR)
    ? fs
      ...
      .filter((name) => /^E-H-\d{2}.*\.md$/.test(name))
      .sort()
    : [];
  ...
  for (const item of metadataItems) {
    ...
    rows.push({
      ...
      owner: fields.owner || "",
      reviewDue,
      cohortId: fields.cohort_id || "",
      successMetricRef: fields.success_metric_ref || "",
  ...
  const nextAction =
    status === "done"
      ? "удерживать managed pilot evidence и не допускать scope drift"
      : "перевести минимум один `E-H-XX` reference в `accepted` и закрыть pending review";
  ...
  const md = [
    ...
    "## Items",
    "",
    "| Reference | Status | Owner | Cohort | Success metric | Review due | Draft | Artifact |",
    "|---|---|---|---|---|---|---|---|",
    ...rows.map(
      (row) =>
        `| \`${row.referenceId}\` | \`${row.status || "missing"}\` | ${row.owner || "-"} | \`${row.cohortId || "-"}\` | \`${row.successMetricRef || "-"}\` | \`${row.reviewDue || "-"}\` | \`${row.draftPath || "-"}\` | \`${row.artifactPath || "-"}\` |`,
    ),
    "",
    "## Issues",
    "",
  ...
  console.log("[phase-e-pilot-status] summary");
