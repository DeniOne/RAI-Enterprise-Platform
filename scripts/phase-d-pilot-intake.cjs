...
function assert(condition, message) {
  if (!condition) {
    console.error(`[phase-d-pilot-intake] ${message}`);
    process.exit(1);
  }
}
...
function printUsage() {
  console.log(
    "usage: node scripts/phase-d-pilot-intake.cjs --reference=D-H-01 --source=/abs/path/file.md [--received-at=2026-04-01] [--copy-name=custom.md] [--dry-run]",
  );
}
...
function main() {
  ...
  console.log(`[phase-d-pilot-intake] reference=${referenceId}`);
  console.log(`[phase-d-pilot-intake] source=${source}`);
  console.log(`[phase-d-pilot-intake] target=${targetFile.replace(/\\/g, "/")}`);
  console.log(`[phase-d-pilot-intake] metadata_file=${metadataFile.replace(/\\/g, "/")}`);
  console.log(`[phase-d-pilot-intake] dry_run=${dryRun ? "yes" : "no"}`);
  ...
  console.log("[phase-d-pilot-intake] status=requested->received");
  console.log("[phase-d-pilot-intake] next_action=owner review");
}
