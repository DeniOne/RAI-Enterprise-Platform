...
class AuthService {
    ...
    async refresh(refreshToken) {
        try {
            ...
            const scopes = this.getScopesForStatus(user.admission_status);
    ...
    generateAuthResponse(user) {
        const scopes = this.getScopesForStatus(user.foundation_status);
    ...
    mapToUserResponse(user) {
        try {
            const response = {
                ...
                foundationStatus: user.foundation_status || client_1.FoundationStatus.NOT_STARTED,
                admissionStatus: user.admission_status,
            };
            return response;
        }
        catch (error) {
            logger_1.logger.error('CRITICAL ERROR in mapToUserResponse:', {
                error: error.message,
                userId: user.id
            });
            throw error;
    ...
    getScopesForStatus(foundationStatus) {
        switch (foundationStatus) {
            case client_1.FoundationStatus.ACCEPTED:
                return ['*'];
            case client_1.FoundationStatus.READY_TO_ACCEPT:
                return ['foundation:read', 'foundation:accept'];
            case client_1.FoundationStatus.READING:
            case client_1.FoundationStatus.NOT_STARTED:
            default:
