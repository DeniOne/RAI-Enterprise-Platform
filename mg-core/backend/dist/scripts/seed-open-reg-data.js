...
async function main() {
    ...
    // Check if exists
    const existingLocs = await prisma.$queryRaw `SELECT id FROM locations WHERE name = ${locationName}`;
    ...
    for (const posName of positions) {
        ...
        const existingPos = await prisma.$queryRaw `SELECT id FROM positions WHERE name = ${posName}`;
