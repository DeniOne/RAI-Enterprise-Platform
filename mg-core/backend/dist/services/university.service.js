...
class UniversityService {
     ...
     */
    async getAcademies() {
        const academies = await prisma_1.prisma.academy.findMany({
    ...
    async createAcademy(data) {
        return await prisma_1.prisma.academy.create({
            data: {
                name: data.name,
    ...
    async getAvailableCourses(userId) {
        ...
        return courses
            ...
            if (course.type === 'APPLIED') {
                const hasAccess = acceptance?.decision === 'ACCEPTED' && acceptance?.version === foundation_constants_1.FOUNDATION_VERSION;
                if (!hasAccess) {
                    isLocked = true;
    ...
    async getCourseById(id) {
        ...
        const totalDuration = course.modules.reduce((sum, module) => sum + (module.material?.duration_minutes || 0), 0);
        return {
            id: course.id,
            title: course.title,
            description: course.description,
    ...
    async getCourseDetails(id, userId) {
        ...
        if (courseDetails.type === 'APPLIED' && userId) {
            ...
            isLocked = !acceptance || acceptance.decision !== 'ACCEPTED' || acceptance.version !== foundation_constants_1.FOUNDATION_VERSION;
    ...
    async createCourse(data) {
        return await prisma_1.prisma.course.create({
            data: {
                ...
                required_grade: data.requiredGrade,
    ...
    async getRecommendedCourses(userId) {
        ...
        const weakMetrics = [];
        if (photocompanyMetrics.okk < 80)
            weakMetrics.push('OKK');
        if (photocompanyMetrics.ck < 70)
            weakMetrics.push('CK');
        if (photocompanyMetrics.conversion < 60)
            weakMetrics.push('CONVERSION');
        if (photocompanyMetrics.quality < 90)
            weakMetrics.push('QUALITY');
        if (photocompanyMetrics.retouchTime > 40)
            weakMetrics.push('RETOUCH_TIME');
     ...
     */
    async calculateProgressToNext(userId) {
        const userGrade = await prisma_1.prisma.userGrade.findUnique({
            where: { user_id: userId },
        });
        if (!userGrade) {
     ...
     */
    async getMyLearningStatus(userId) {
     ...
     */
    async explainRecommendation(courseId, userId) {
        const course = await prisma_1.prisma.course.findUnique({
            where: { id: courseId },
        });
        if (!course) {
        ...
        const metricData = thresholds[targetMetric];
        if (!metricData) {
            return {
                courseId,
                targetMetric,
    ...
    async getAnalyticsOverview() {
        const [academiesCount, coursesCount, totalEnrollments, completedEnrollments, activeMentorships, securitySignalsCount] = await Promise.all([
            prisma_1.prisma.academy.count({ where: { is_active: true } }),
            prisma_1.prisma.course.count({ where: { is_active: true } }),
            prisma_1.prisma.enrollment.count(),
            prisma_1.prisma.enrollment.count({ where: { status: 'COMPLETED' } }),
            prisma_1.prisma.mentorshipPeriod.count({ where: { status: 'ACTIVE' } }),
            prisma_1.prisma.antiFraudSignal.count()
        ]);
        return {
            infrastructure: {
