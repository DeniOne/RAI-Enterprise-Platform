...
class EnrollmentService {
     ...
     */
    async enrollInCourse(userId, courseId, assignedBy) {
        ...
        if (course.type === 'APPLIED') {
            await foundation_service_1.foundationService.assertFoundationAccessForApplied(userId);
        }
        ...
        const courseModules = await prisma_1.prisma.courseModule.findMany({
        ...
        await prisma_1.prisma.moduleProgress.createMany({
     ...
     */
    async withdrawFromCourse(userId, enrollmentId) {
     ...
     */
    async updateModuleProgress(enrollmentId, moduleId, status, score) {
        const progress = await prisma_1.prisma.moduleProgress.findFirst({
            where: {
        ...
        const updated = await prisma_1.prisma.moduleProgress.update({
    ...
    async completeCourse(userId, courseId) {
        ...
        const allModules = await prisma_1.prisma.courseModule.findMany({
            where: { course_id: courseId },
        });
        const progress = await prisma_1.prisma.moduleProgress.findMany({
            where: {
                enrollment_id: enrollment.id,
            },
        });
        const requiredModules = allModules.filter((m) => m.is_required);
        const completedRequired = progress.filter((p) => p.status === 'COMPLETED' && requiredModules.some((rm) => rm.id === p.module_id));
        if (completedRequired.length < requiredModules.length) {
            throw new Error('Not all required modules completed');
        }
        // Update enrollment status
        const completed = await prisma_1.prisma.enrollment.update({
        ...
        try {
            const { antiFraudDetector } = await Promise.resolve().then(() => __importStar(require('./anti-fraud-detector.service')));
            const { antiFraudSignalWriter } = await Promise.resolve().then(() => __importStar(require('./anti-fraud-signal-writer.service')));
            // Pure detection
            const signals = antiFraudDetector.detectSignals('Course', courseId, {
                userId,
                courseId,
                enrollmentData: { ...enrollment, user: await prisma_1.prisma.user.findUnique({ where: { id: userId }, include: { erp_role: true } }) },
        ...
        try {
            ...
            await telegramService.sendCourseCompletedNotification(userId, enrollment.course.title, enrollment.course.recognition_mc);
        }
        catch (error) {
            // Log error but DO NOT block completion
    ...
    async updateEnrollmentProgress(enrollmentId) {
        const allModules = await prisma_1.prisma.moduleProgress.findMany({
    ...
    async registerRecognition(userId, course) {
        ...
        if (course.recognition_mc > 0) {
            await rewardService.registerEligibility(userId, 'COURSE_COMPLETED', course.id, course.recognition_mc);
        }
