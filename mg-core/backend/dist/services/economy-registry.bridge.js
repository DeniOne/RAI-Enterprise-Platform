...
exports.economyRegistryBridge = exports.EconomyRegistryBridgeService = void 0;
 ...
 */
class EconomyRegistryBridgeService {
    static instance;
    // Registry Definitions
    DEFINITION_OWNS_WALLET = 'urn:mg:def:economy:owns_wallet'; // Relationship: User -> Wallet
    TYPE_WALLET = 'urn:mg:type:economy:wallet'; // Entity Type for Wallet
    constructor() { }
    static getInstance() {
     ...
     */
    async createWalletStructure(userId, walletId, force = false, reason) {
        ...
        const existing = await prisma_1.prisma.registryEntity.findUnique({ where: { urn: walletUrn } });
        if (!existing) {
        ...
        const rel = await registry_relationship_service_1.registryRelationshipService.createRelationship({
     ...
     */
    async transferWalletOwnership(walletId, newOwnerId, force = false, reason) {
        const walletUrn = this.getWalletUrn(walletId);
        const newOwnerUrn = this.getUserUrn(newOwnerId);
        // Find existing owner relationship
        const rels = await registry_relationship_service_1.registryRelationshipService.getRelationships(this.DEFINITION_OWNS_WALLET, undefined, // from_urn unknown (current owner)
        walletUrn);
        if (rels.length === 0) {
...
exports.EconomyRegistryBridgeService = EconomyRegistryBridgeService;
exports.economyRegistryBridge = EconomyRegistryBridgeService.getInstance();
