...
class DepartmentService {
    ...
    async getDepartmentKPI(departmentId, period) {
        return {
            departmentId,
            period,
            metrics: {
    ...
    async getDepartments(includeInactive = false, format = 'flat') {
        const cacheKey = format === 'tree' ? cache_1.CacheKeys.departmentTree() : cache_1.CacheKeys.departments();
        if (includeInactive) {
            return this.fetchDepartments(includeInactive, format);
        }
        return cache_1.cache.getOrSet(cacheKey, () => this.fetchDepartments(includeInactive, format), cache_1.CacheTTL.MEDIUM);
    }
    /**
     * Fetch departments from database
     */
    async fetchDepartments(includeInactive, format) {
    ...
    async updateDepartment(id, data) {
        ...
        if (data.functions !== undefined) {
            updates.push(`functions = $${paramIndex++}::text[]`);
            values.push(data.functions);
        }
        if (data.kpis !== undefined) {
            updates.push(`kpis = $${paramIndex++}::jsonb`);
            values.push(JSON.stringify(data.kpis));
        }
        if (data.annual_goals !== undefined) {
            updates.push(`annual_goals = $${paramIndex++}`);
            values.push(data.annual_goals);
        }
        if (data.budget_annual !== undefined) {
            updates.push(`budget_annual = $${paramIndex++}`);
            values.push(data.budget_annual);
        }
        ...
        const result = await prisma_1.prisma.$queryRawUnsafe(query, ...values);
        await this.invalidateCache();
        return result[0];
    }
    /**
     ...
     */
    async deleteDepartment(id, softDelete = true) {
        if (softDelete) {
            return this.updateDepartment(id, { is_active: false });
        }
        else {
            await prisma_1.prisma.$executeRaw `DELETE FROM departments WHERE id = ${id}`;
            await this.invalidateCache();
            return { success: true };
        }
    }
    ...
    async getDepartmentsByHierarchy() {
        ...
        const departments = await prisma_1.prisma.$queryRaw `
    ...
    async invalidateCache() {
        await cache_1.cache.del(cache_1.CacheKeys.departments());
        await cache_1.cache.del(cache_1.CacheKeys.departmentTree());
    }
    mapToResponse(dept) {
        return {
            id: dept.id,
...
exports.default = DepartmentService.getInstance();
