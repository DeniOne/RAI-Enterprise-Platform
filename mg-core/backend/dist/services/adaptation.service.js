...
exports.adaptationService = exports.AdaptationService = void 0;
const prisma_1 = require("../config/prisma");
class AdaptationService {
    /**
     * Get mentorship and 1-on-1 status for a specific user
     */
    async getMyAdaptationStatus(userId) {
        const mentorship = await prisma_1.prisma.mentorship.findUnique({
        ...
        const next1on1 = await prisma_1.prisma.oneOnOne.findFirst({
            where: {
                employee_id: userId,
     ...
     */
    async create1on1(data) {
        return await prisma_1.prisma.oneOnOne.create({
            data: {
     ...
     */
    async complete1on1(id, notes, actionItems, mood) {
        return await prisma_1.prisma.oneOnOne.update({
            where: { id },
            data: {
     ...
     */
    async getTeamStatus(managerId) {
        // 1. Find all mentees
        const mentees = await prisma_1.prisma.mentorship.findMany({
        ...
        const recentSessions = await prisma_1.prisma.oneOnOne.findMany({
            where: {
        ...
        return {
            mentees: mentees.map((m) => m.mentee),
            teamHappiness: {
                average: avgMood ? Math.round(avgMood * 10) / 10 : null,
                label: "Advisory Metric - Not a KPI",
                sessionCount: recentSessions.length
            },
            pendingMeetings: await prisma_1.prisma.oneOnOne.findMany({
                where: {
                    manager_id: managerId,
                    completed_at: null,
                    scheduled_at: { gte: new Date() }
...
exports.adaptationService = new AdaptationService();
