...
exports.AIAnalyst = exports.AIAnalystError = void 0;
const ai_guardrails_1 = require("../ai-guardrails");
// =============================================================================
// AI ANALYST ERROR
// =============================================================================
class AIAnalystError extends Error {
    constructor(message) {
        super(message);
        this.name = 'AIAnalystError';
    }
}
exports.AIAnalystError = AIAnalystError;
// =============================================================================
// PROMPT TEMPLATES
// =============================================================================
...
class AIAnalyst {
     ...
     */
    async explainKPIChanges(input) {
        const prompt = this.buildExplainKPIPrompt(input);
        return this.executeWithGuardrails(prompt, 'explainKPIChanges');
    }
    buildExplainKPIPrompt(input) {
        const kpiList = input.kpi_records.map((k) => `- ${k.kpi_name}: ${k.value} ${k.unit} (период: ${k.period_start.toISOString().split('T')[0]} - ${k.period_end.toISOString().split('T')[0]})`).join('\n');
        const eventsList = input.events.slice(0, 10).map((e) => `- [${e.type}] ${e.summary} (${e.timestamp.toISOString().split('T')[0]})`).join('\n');
...
Объясни, что повлияло на KPI. НЕ давай советов.`;
     ...
     */
    async describeTrends(input) {
        const prompt = this.buildDescribeTrendsPrompt(input);
        return this.executeWithGuardrails(prompt, 'describeTrends');
    }
    buildDescribeTrendsPrompt(input) {
        const kpiList = input.kpi_history.map((k) => `- ${k.kpi_name}: ${k.value} ${k.unit} (${k.period_start.toISOString().split('T')[0]})`).join('\n');
        return `${SYSTEM_CONTEXT}
...
Опиши наблюдаемые тренды. НЕ делай прогнозов. НЕ давай советов.`;
     ...
     */
    async highlightRisks(input) {
        const prompt = this.buildHighlightRisksPrompt(input);
        return this.executeWithGuardrails(prompt, 'highlightRisks');
    }
    buildHighlightRisksPrompt(input) {
        const eventsList = input.events.map((e) => `- [${e.type}] ${e.summary} (${e.timestamp.toISOString().split('T')[0]})`).join('\n');
        ...
        const kpiInfo = input.recent_kpis
            ? input.recent_kpis.map((k) => `- ${k.kpi_name}: ${k.value} ${k.unit}`).join('\n')
            : 'Нет данных о KPI';
        return `${SYSTEM_CONTEXT}
...
Подсветь риски БЕЗ советов по их устранению.`;
     ...
     */
    async summarizeState(input) {
        const prompt = this.buildSummarizeStatePrompt(input);
        return this.executeWithGuardrails(prompt, 'summarizeState');
    }
    buildSummarizeStatePrompt(input) {
        const kpiList = input.recent_kpis.map((k) => `- ${k.kpi_name}: ${k.value} ${k.unit}`).join('\n');
        const rewardsList = input.recent_rewards.map((r) => `- ${r.reward_type}: ${r.amount} (${r.reason})`).join('\n');
...
Опиши текущее состояние ФАКТАМИ. НЕ оценивай человека. НЕ давай советов.`;
    ...
    async executeWithGuardrails(prompt, methodName) {
        ...
        if (!guardrailCheck.allowed) {
            throw new AIAnalystError(ai_guardrails_1.REFUSAL_MESSAGE);
        }
        try {
    ...
    validateFactType(type) {
        ...
        return validTypes.includes(String(type)) ? type : null;
    }
    parseRisks(risks) {
    ...
    validateSeverity(severity) {
        ...
        return validSeverities.includes(String(severity)) ? severity : null;
    }
