...
exports.mesService = exports.MesService = void 0;
...
class MesService {
     ...
     */
    getShiftWindow(date = new Date()) {
        const start = new Date(date);
        start.setHours(mes_rates_1.SHIFT_CONFIG.START_HOUR, 0, 0, 0);
        const end = new Date(date);
        end.setHours(mes_rates_1.SHIFT_CONFIG.END_HOUR, 0, 0, 0);
        return { start, end };
    }
    /**
     * Get real-time shift progress for a specific employee
     */
    async getMyShiftProgress(userId) {
        ...
        const soldCount = orders.filter(o => o.status === client_1.ProductionOrderStatus.COMPLETED).length;
        // 3. Calculate Earnings (Deterministic Logic)
        let totalEarnings = 0;
        for (const order of orders) {
            const rates = mes_rates_1.PRODUCT_RATES[order.product_type] || mes_rates_1.PRODUCT_RATES['DEFAULT'];
            ...
            if (latestCheck && latestCheck.result === client_1.QualityResult.FAIL) {
                orderEarnings *= mes_rates_1.QUALITY_MODIFIERS.FAIL;
            }
            totalEarnings += orderEarnings;
        }
        // 4. Get active work orders (pending retouching)
        const activeWorkOrdersCount = await prisma_1.prisma.workOrder.count({
            where: {
     ...
     */
    async getEarningsForecast(userId) {
    ...
    async getMonthlyEarnings(userId) {
        ...
        const orders = await prisma_1.prisma.productionOrder.findMany({
            where: {
                ...
                status: client_1.ProductionOrderStatus.COMPLETED // Only completed count for monthly progress
            },
            include: {
                quality_checks: {
                    orderBy: { created_at: 'desc' },
...
exports.mesService = new MesService();
