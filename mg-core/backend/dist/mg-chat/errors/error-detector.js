...
const error_router_1 = require("./error-router");
// =============================================================================
// CONFIGURATION
// =============================================================================
 ...
 */
function detectError(message, context = {}) {
    // 1. Empty message
    if (message.trim().length === 0) {
        return {
            matched: true,
            match: (0, error_router_1.routeError)('empty_message')
        };
    }
    const normalizedMessage = message.toLowerCase().trim();
    // 2. Spam repetition (same message >= 3 times)
    if (context.recentMessages && context.recentMessages.length > 0) {
        const count = context.recentMessages.filter(msg => msg.toLowerCase().trim() === normalizedMessage).length;
        if (count >= SPAM_THRESHOLD) {
            return {
                matched: true,
                match: (0, error_router_1.routeError)('spam_repetition')
            };
        }
    }
    ...
    if (context.messageTimestamps && context.messageTimestamps.length > 0) {
        const now = Date.now();
        const recentCount = context.messageTimestamps.filter(ts => now - ts < FLOOD_WINDOW_MS).length;
        if (recentCount > FLOOD_THRESHOLD) {
            return {
                matched: true,
                match: (0, error_router_1.routeError)('flooding')
            };
        }
    }
    // 4. Aggression detection (profanity wordlist)
    const hasAggression = AGGRESSION_KEYWORDS.some(keyword => normalizedMessage.includes(keyword));
    if (hasAggression) {
        return {
            matched: true,
            match: (0, error_router_1.routeError)('aggression_detected')
        };
    }
    // 5. Emotional overload
    const hasEmotionalOverload = EMOTIONAL_OVERLOAD_PHRASES.some(phrase => normalizedMessage.includes(phrase));
    if (hasEmotionalOverload) {
        return {
            matched: true,
            match: (0, error_router_1.routeError)('emotional_overload')
        };
    }
    // No error detected
    return {
