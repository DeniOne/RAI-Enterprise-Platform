...
const scenario_router_1 = require("./scenarios/scenario-router");
...
async function runTests() {
    ...
    await prisma_1.prisma.mentorship.deleteMany({ where: { OR: [{ mentee_id: testUserId }, { mentor_id: testManagerId }] } });
    await prisma_1.prisma.wallet.deleteMany({ where: { user_id: { in: [testUserId, testManagerId] } } });
    await prisma_1.prisma.user.deleteMany({ where: { id: { in: [testUserId, testManagerId] } } });
    // Create Employee
    ...
    await prisma_1.prisma.mentorship.create({
        data: {
    ...
    const initialWallet = await prisma_1.prisma.wallet.findUnique({ where: { id: 'w-11' } });
    const intentEmp = {
    ...
    const respEmp = await (0, scenario_router_1.routeScenario)(intentEmp);
    console.log('Response:', respEmp.text.split('\n')[0]);
    const finalWallet = await prisma_1.prisma.wallet.findUnique({ where: { id: 'w-11' } });
    // VERIFY: No changes
    ...
    const respMgr = await (0, scenario_router_1.routeScenario)(intentMgr);
    ...
    const respWrong = await (0, scenario_router_1.routeScenario)(intentWrong);
    ...
    // Cleanup
    await prisma_1.prisma.mentorship.deleteMany({ where: { mentor_id: testManagerId } });
    await prisma_1.prisma.wallet.deleteMany({ where: { user_id: testUserId } });
    await prisma_1.prisma.user.deleteMany({ where: { id: { in: [testUserId, testManagerId] } } });
