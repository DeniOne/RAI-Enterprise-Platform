...
exports.handleManagerScenario = handleManagerScenario;
 ...
 */
async function handleManagerScenario(action, intent) {
    ...
    switch (action) {
        ...
        case 'one_on_one_schedule':
            const status = await adaptation_service_1.adaptationService.getMyAdaptationStatus(managerId); // Reuse logic or add specific manager method if needed
            // For managers, we likely want "My scheduled meetings as Manager"
            // adaptationService.getTeamStatus gives pendingMeetings
            const teamStatus = await adaptation_service_1.adaptationService.getTeamStatus(managerId);
        ...
        case 'team_happiness':
            const happinessData = await adaptation_service_1.adaptationService.getTeamStatus(managerId);
        ...
        case 'mentee_list':
            const data = await adaptation_service_1.adaptationService.getTeamStatus(managerId);
