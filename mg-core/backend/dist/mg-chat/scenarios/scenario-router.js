...
const employee_scenario_1 = require("./employee.scenario");
const manager_scenario_1 = require("./manager.scenario");
/**
 * Route intent to appropriate scenario.
 *
 * This is where Intent Namespaces become technical reality.
 */
async function routeScenario(intent) {
    const [namespace, action] = intent.intentId.split('.');
    switch (namespace) {
        case 'employee':
            return (0, employee_scenario_1.handleEmployeeScenario)(action, intent);
        case 'manager':
            return (0, manager_scenario_1.handleManagerScenario)(action, intent);
        case 'exec':
 ...
 */
function handleExecutiveScenario(action, intent) {
    switch (action) {
