...
exports.handleEmployeeScenario = handleEmployeeScenario;
const mes_service_1 = require("../../mes/services/mes.service");
 ...
 */
async function handleEmployeeScenario(action, intent) {
    switch (action) {
        // ==========================================
        ...
        case 'show_my_earnings':
            const earningsForecast = await mes_service_1.mesService.getEarningsForecast(intent.userId);
        ...
        case 'show_my_shift':
            const shift = await mes_service_1.mesService.getMyShiftProgress(intent.userId);
        ...
        case 'show_mc_balance':
            const wallet = await prisma_1.prisma.wallet.findUnique({ where: { user_id: intent.userId } });
        ...
        case 'show_my_training':
            ...
            try {
                const dashboard = await university_service_1.universityService.getStudentDashboard(intent.userId);
                const userGrade = await prisma_1.prisma.userGrade.findUnique({ where: { user_id: intent.userId } });
        ...
        case 'show_my_courses':
            ...
            try {
                const coursesObj = await enrollment_service_1.enrollmentService.getMyCourses(intent.userId);
        ...
        case 'show_my_qualification':
            ...
            try {
                const userGrade = await prisma_1.prisma.userGrade.findUnique({ where: { user_id: intent.userId } });
                const progress = await university_service_1.universityService.calculateProgressToNext(intent.userId);
                if (!userGrade) {
                    return {
        ...
        case 'growth_matrix':
            const pulse = await growth_matrix_service_1.growthMatrixService.getGrowthPulse(intent.userId);
