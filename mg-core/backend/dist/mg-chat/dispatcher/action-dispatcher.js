...
function dispatchAction(actionId) {
    ...
    const isDirectIntent = contracts.intents.intents.some(intent => intent.id === actionId);
    if (isDirectIntent) {
        return {
            status: 'ok',
            intent: actionId,
    ...
    const isErrorIntent = contracts.errors.error_intents.some(err => err.id === actionId);
    if (isErrorIntent) {
        return {
            status: 'ok',
            intent: actionId,
