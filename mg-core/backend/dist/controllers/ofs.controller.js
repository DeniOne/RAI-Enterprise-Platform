...
class OFSController {
    ...
    async getDepartments(req, res) {
        try {
            ...
            const departments = await ofs_service_1.default.getDepartments(include_inactive === 'true', format || 'flat');
            res.status(200).json({
                success: true,
                data: departments
            });
    ...
    async createDepartment(req, res) {
        try {
            ...
            const department = await ofs_service_1.default.createDepartment({ ...body, force, reason });
            res.status(201).json({
                success: true,
                data: department
            });
    ...
    async updateDepartment(req, res) {
        try {
            const { id } = req.params;
            const department = await ofs_service_1.default.updateDepartment(id, req.body);
            res.status(200).json({
                success: true,
                data: department
            });
    ...
    async deleteDepartment(req, res) {
        try {
            ...
            const result = await ofs_service_1.default.deleteDepartment(id, hard !== 'true', force === 'true', reason);
            res.status(200).json({
                success: true,
                data: result
            });
    ...
    async moveDepartment(req, res) {
        try {
            ...
            const department = await ofs_service_1.default.moveDepartment(id, new_parent_id, reason, userId, force);
            res.status(200).json({
                success: true,
                data: department
            });
    ...
    async getRoleMatrix(req, res) {
        try {
            ...
            const userRole = req.user?.role || common_enums_1.UserRole.EMPLOYEE;
            const matrix = await ofs_service_1.default.getRoleMatrix(department_id);
            const filtered = (0, ofs_acl_service_1.filterRoleMatrixByRole)(matrix, userRole);
            res.status(200).json({
                success: true,
                data: filtered
            });
    ...
    async createRole(req, res) {
        try {
            const role = await ofs_service_1.default.createRole(req.body);
            res.status(201).json({
                success: true,
                data: role
            });
    ...
    async updateRole(req, res) {
        try {
            const { id } = req.params;
            const role = await ofs_service_1.default.updateRole(id, req.body);
            res.status(200).json({
                success: true,
                data: role
            });
    ...
    async deleteRole(req, res) {
        try {
            const { id } = req.params;
            await ofs_service_1.default.deleteRole(id);
            res.status(200).json({
                success: true,
    ...
    async assignRole(req, res) {
        try {
            ...
            const userId = req.user?.id;
            const assignment = await ofs_service_1.default.assignRole({
                employee_id,
    ...
    async getEmployees(req, res) {
        try {
            ...
            const userRole = req.user?.role || common_enums_1.UserRole.EMPLOYEE;
            const userId = req.user?.id;
            const result = await ofs_service_1.default.getEmployees({
                department_id: department_id,
                role: role,
                status: status,
                has_competencies: has_competencies ? has_competencies.split(',') : undefined,
                page: page ? Number(page) : undefined,
                limit: limit ? Number(limit) : undefined
            });
            const filteredData = (0, ofs_acl_service_1.filterOFSArrayByRole)(result.data, userRole, userId);
            res.status(200).json({
                success: true,
                data: filteredData,
     ...
     */
    async updateEmployeeCompetencies(req, res) {
        try {
            const { id } = req.params;
            const employee = await ofs_service_1.default.updateEmployeeCompetencies(id, req.body);
            res.status(200).json({
                success: true,
                data: employee
            });
    ...
    async transferEmployee(req, res) {
        try {
            ...
            const userId = req.user?.id;
            const employee = await ofs_service_1.default.transferEmployee(id, {
                new_department_id,
                new_position,
     ...
     */
    async getReportingRelationships(req, res) {
        try {
            const { employeeId } = req.params;
            const relationships = await ofs_service_1.default.getReportingRelationships(employeeId);
            res.status(200).json({
                success: true,
                data: relationships
            });
     ...
     */
    async createReportingRelationship(req, res) {
        try {
            const { subordinate_id, supervisor_id, relationship_type, effective_from, reason } = req.body;
            const relationship = await ofs_service_1.default.createReportingRelationship({
                subordinate_id,
                supervisor_id,
                relationship_type,
                effective_from: new Date(effective_from),
    ...
    async getOrgChart(req, res) {
        try {
            ...
            const chart = await ofs_service_1.default.getOrgChart(department_id, depth ? Number(depth) : undefined, include_photos === 'true');
            res.status(200).json({
                success: true,
                data: chart
            });
    ...
    async getHistory(req, res) {
        try {
            const { entity_type, entity_id, action, date_from, date_to, changed_by, page, limit } = req.query;
            const userRole = req.user?.role || common_enums_1.UserRole.EMPLOYEE;
            const result = await ofs_service_1.default.getStructureHistory({
            ...
            const filteredData = (0, ofs_acl_service_1.filterHistoryByRole)(result.data, userRole);
            res.status(200).json({
                success: true,
                data: filteredData,
                pagination: result.pagination
     ...
     */
    async getStructureReport(req, res) {
        try {
            const stats = await ofs_service_1.default.getStructureStats();
            res.status(200).json({
                success: true,
                data: stats
            });
     ...
     */
    async getPyramidRoles(req, res) {
        try {
            const roles = await ofs_service_1.default.getPyramidRoles();
            res.status(200).json({
                success: true,
                data: roles
            });
     ...
     * Create pyramid role
     */
    async createPyramidRole(req, res) {
        try {
            const role = await ofs_service_1.default.createPyramidRole(req.body);
            res.status(201).json({
                success: true,
                data: role
            });
     ...
     */
    async getTriangleAssignments(req, res) {
        try {
            const { employee_id } = req.query;
            const assignments = await ofs_service_1.default.getTriangleAssignments(employee_id);
            res.status(200).json({
                success: true,
                data: assignments
            });
     ...
     */
    async assignTriangleRole(req, res) {
        try {
            const assignment = await ofs_service_1.default.assignTriangleRole(req.body);
            res.status(201).json({
                success: true,
                data: assignment
            });
     ...
     */
    async getTriangleStats(req, res) {
        try {
            const stats = await ofs_service_1.default.getTriangleStats();
            res.status(200).json({
                success: true,
                data: stats
            });
    ...
    async getHierarchyLevels(req, res) {
        try {
            const levels = await ofs_service_1.default.getHierarchyLevels();
            res.status(200).json({
                success: true,
                data: levels
            });
     ...
     */
    async getHierarchyStructure(req, res) {
        try {
            const structure = await ofs_service_1.default.getDepartmentsByHierarchy();
            res.status(200).json({
                success: true,
                data: structure
            });
     ...
     */
    async createRACIAssignment(req, res) {
        try {
            const assignment = await ofs_service_1.default.createRACIAssignment(req.body);
            res.status(201).json({
                success: true,
                data: assignment
            });
     ...
     */
    async getProjectRACI(req, res) {
        try {
            const { projectName } = req.params;
            const raci = await ofs_service_1.default.getProjectRACI(projectName);
            res.status(200).json({
                success: true,
                data: raci
            });
    ...
    async submitIdea(req, res) {
        try {
            const idea = await ofs_service_1.default.submitIdea(req.body);
            res.status(201).json({
                success: true,
                data: idea
            });
    ...
    async getIdeas(req, res) {
        try {
            ...
            const ideas = await ofs_service_1.default.getIdeas({
     ...
     */
    async logHybridInteraction(req, res) {
        try {
            const interaction = await ofs_service_1.default.logHybridInteraction(req.body);
            res.status(201).json({
                success: true,
                data: interaction
            });
     ...
     */
    async getHybridTeamStats(req, res) {
        try {
            const stats = await ofs_service_1.default.getHybridTeamStats();
            res.status(200).json({
                success: true,
                data: stats
            });
...
exports.default = new OFSController();
