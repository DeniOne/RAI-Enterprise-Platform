...
exports.entityCardGuard = exports.EntityCardGuard = void 0;
const entity_card_cache_1 = require("./entity-card.cache");
const logger_1 = require("../config/logger");
// =============================================================================
// ENTITY CARD GUARD
// =============================================================================
class EntityCardGuard {
    /**
    ...
    validate(entityType, data, operation, existingData) {
        ...
        let card;
        try {
            card = entity_card_cache_1.entityCardCache.get(entityType);
        }
        catch (error) {
            return {
                valid: false,
        ...
        if (!valid) {
            logger_1.logger.warn(`[EntityCardGuard] Validation failed for ${entityType}`, {
    ...
    // =========================================================================
    validateAttributes(card, data, operation, existingData, errors) {
    ...
    validateType(attr, value, errors) {
        switch (attr.type) {
            case 'INTEGER':
    ...
    validateEnum(attr, value, errors) {
    ...
    // =========================================================================
    validateRelations(card, data, operation, existingData, errors) {
    ...
    validateWithView(entityType, data, viewName) {
        ...
        const valid = errors.length === 0;
        if (!valid) {
            logger_1.logger.warn(`[EntityCardGuard] View Validation failed for ${entityType}:${viewName}`, {
                errors: errors.length
            });
        }
        return { valid, errors };
    }
    validateAttributesAgainstView(card, data, viewFields, // EntityCardFormField[] but avoiding circular dep issues if any
    errors) {
    ...
    // PRIVATE: Forbidden fields validation
    // =========================================================================
    validateForbiddenFields(card, data, errors) {
...
exports.EntityCardGuard = EntityCardGuard;
// =============================================================================
// SINGLETON
// =============================================================================
exports.entityCardGuard = new EntityCardGuard();
