...
const impact_guard_1 = require("./impact.guard");
...
class ImpactService {
     ...
     */
    getImpactReport(entityType, id, viewName) {
        // 1. Validation
        const rootCard = entity_card_cache_1.entityCardCache.get(entityType);
        const view = impact_guard_1.entityCardImpactGuard.validateImpactContext(rootCard, viewName);
        ...
        const graph = graph_service_1.graphService.getGraphByDef(entityType, id, transientGraphView);
        ...
        graph.edges.forEach(edge => {
            const sourceCard = entity_card_cache_1.entityCardCache.get(edge.source.split(':')[0]); // Assuming id format "type:id"
            ...
            if (relationDef && relationDef.impact) {
                ...
                if (view.include && !view.include.includes(relationDef.impact.type))
                    return;
    ...
    getAllEntityTypes() {
        return entity_card_cache_1.entityCardCache.getAll().map(c => c.entityType);
    }
}
