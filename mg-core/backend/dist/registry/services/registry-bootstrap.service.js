...
exports.registryBootstrapService = exports.RegistryBootstrapService = void 0;
const prisma_1 = require("../../config/prisma");
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const logger_1 = require("../../config/logger");
class RegistryBootstrapService {
    BOOTSTRAP_DIR = path_1.default.join(__dirname, '../bootstrap');
    REQUIRED_FILES = [
    ...
    async run() {
        logger_1.logger.info('Starting Registry Bootstrap...');
        // 1. Check if registry is empty
        const count = await prisma_1.prisma.registryEntity.count();
        if (count > 0) {
            logger_1.logger.warn('Registry is not empty. Bootstrap aborted.');
        ...
        await prisma_1.prisma.$transaction(async (tx) => {
            logger_1.logger.info('Inserting bootstrap entities...');
            // 3.1 Insert FSM (01)
        ...
        logger_1.logger.info('Registry Bootstrap completed successfully.');
    }
    async loadBootstrapData() {
        const data = {};
        for (const file of this.REQUIRED_FILES) {
            const filePath = path_1.default.join(this.BOOTSTRAP_DIR, file);
            try {
                const content = await promises_1.default.readFile(filePath, 'utf-8');
                data[file] = JSON.parse(content);
            }
            catch (error) {
                logger_1.logger.error(`Failed to load bootstrap file: ${file}`, error);
                throw new Error(`Failed to load bootstrap file: ${file}`);
            }
        }
        return data;
    }
    async insertEntity(tx, item, typeUrn) {
        ...
        if (typeUrn === 'fsm_definition')
            actualEntityTypeUrn = 'urn:mg:entity-type:fsm_definition:v1';
        else if (typeUrn === 'entity_type')
            actualEntityTypeUrn = 'urn:mg:entity-type:entity_type:v1';
        else if (typeUrn === 'attribute_definition')
            actualEntityTypeUrn = 'urn:mg:entity-type:attribute_definition:v1';
    ...
    async insertAttributes(tx, items, typeUrn) {
...
exports.RegistryBootstrapService = RegistryBootstrapService;
exports.registryBootstrapService = new RegistryBootstrapService();
