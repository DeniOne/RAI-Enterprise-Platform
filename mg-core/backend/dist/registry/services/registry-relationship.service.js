...
exports.registryRelationshipService = exports.RegistryRelationshipService = void 0;
const prisma_1 = require("../../config/prisma");
const registry_constraint_engine_1 = require("../core/registry-constraint.engine");
const registry_impact_engine_1 = require("../core/registry-impact.engine");
const impact_types_1 = require("../dto/impact.types");
class RegistryRelationshipService {
    async createRelationship(dto) {
        ...
        return await prisma_1.prisma.$transaction(async (tx) => {
            ...
            const impactReport = await registry_impact_engine_1.registryImpactAnalysisEngine.analyzeChange(tx, impact_types_1.ChangeType.RELATIONSHIP_CREATE, to_urn, { from_urn, to_urn, definition_urn });
            const blocking = impactReport.impacts.filter(i => i.level === impact_types_1.ImpactLevel.BLOCKING);
            if (blocking.length > 0) {
                await tx.registryAuditEvent.create({
                    data: {
                        ...
                        payload: { reason: 'Blocking impacts found', impacts: blocking }
                    }
                });
                throw new Error(`Conflict: Operation blocked by Impact Analysis. ${blocking.length} blocking issues found.`);
            }
            const warnings = impactReport.impacts.filter(i => i.level === impact_types_1.ImpactLevel.WARNING);
            if (warnings.length > 0) {
                ...
                await tx.registryAuditEvent.create({
                    data: {
                        ...
                        entity_urn: 'urn:mg:system:gate',
                        actor_urn: 'urn:mg:system:api',
                        payload: { reason: reason, warnings: warnings }
                    }
                });
            }
            // -----------------------------
            ...
            registry_constraint_engine_1.registryConstraintEngine.validateDefinitionState(definition);
            // 3. Validate Entities Exist
            const fromEntity = await tx.registryEntity.findUnique({ where: { urn: from_urn } });
            if (!fromEntity)
                throw new Error(`Source entity ${from_urn} not found`);
            const toEntity = await tx.registryEntity.findUnique({ where: { urn: to_urn } });
            if (!toEntity)
                throw new Error(`Target entity ${to_urn} not found`);
            // 3.1 Validate Entity Types against Definition
            const defAttrs = definition.attributes;
            ...
            if (cardinality === '1-1' || cardinality === '1-n') {
                ...
                if (cardinality === '1-1') {
                    ...
                    if (existingFrom)
                        throw new Error(`Cardinality violation 1-1: Source ${from_urn} already has a relationship`);
                    const existingTo = await tx.registryRelationship.findFirst({ where: { definition_urn, to_urn: to_urn } }); // Fix: to_urn: to_urn
                    if (existingTo)
                        throw new Error(`Cardinality violation 1-1: Target ${to_urn} already has a relationship`);
                }
                    ...
                    if (existingTo)
                        throw new Error(`Cardinality violation 1-n: Target ${to_urn} already has a parent/source`);
                }
            }
            // 5. Core DAG Enforcement (Invariant)
            // This throws 409 if cycle detected
            await registry_constraint_engine_1.registryConstraintEngine.enforceDAGConstraint(tx, from_urn, to_urn, definition_urn);
            // 6. Insert
    ...
    async updateRelationship(id, dto) {
        ...
        return await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (isMove) {
                ...
                const impactReport = await registry_impact_engine_1.registryImpactAnalysisEngine.analyzeChange(tx, impact_types_1.ChangeType.RELATIONSHIP_CREATE, newTo, { from_urn: newFrom, to_urn: newTo, definition_urn: currentRel.definition_urn });
                const blocking = impactReport.impacts.filter(i => i.level === impact_types_1.ImpactLevel.BLOCKING);
                if (blocking.length > 0) {
                    await tx.registryAuditEvent.create({
                        data: {
                            ...
                            entity_urn: 'urn:mg:system:gate',
                            actor_urn: 'urn:mg:system:api',
                            payload: { reason: 'Blocking impacts (Update/Move)', impacts: blocking }
                        }
                    });
                ...
                // DAG check (Strict)
                await registry_constraint_engine_1.registryConstraintEngine.enforceDAGConstraint(tx, newFrom, newTo, currentRel.definition_urn);
            }
            // Execute Update
    ...
    async deleteRelationship(id, force, reason) {
        return await prisma_1.prisma.$transaction(async (tx) => {
            const impactReport = await registry_impact_engine_1.registryImpactAnalysisEngine.analyzeChange(tx, impact_types_1.ChangeType.RELATIONSHIP_DELETE, id);
            const blocking = impactReport.impacts.filter(i => i.level === impact_types_1.ImpactLevel.BLOCKING);
            if (blocking.length > 0) {
    ...
    async getRelationships(definitionUrn, fromUrn, toUrn) {
        return prisma_1.prisma.registryRelationship.findMany({
            where: {
...
exports.RegistryRelationshipService = RegistryRelationshipService;
exports.registryRelationshipService = new RegistryRelationshipService();
