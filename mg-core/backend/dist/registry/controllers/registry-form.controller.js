...
exports.registryFormController = exports.RegistryFormController = void 0;
const registry_form_service_1 = require("../services/registry-form.service");
...
class RegistryFormController {
    ...
    async getProjection(req, res) {
        try {
            ...
            if (mode === registry_form_dto_1.FormMode.CREATE && !type) {
                res.status(400).json({ message: 'Missing type parameter for CREATE mode' });
                return;
            }
            if ((mode === registry_form_dto_1.FormMode.EDIT || mode === registry_form_dto_1.FormMode.VIEW) && !urn) {
            ...
            const user = req.user;
            const projection = await registry_form_service_1.registryFormService.getFormProjection(targetType, mode, user, urn);
            res.json(projection);
        }
        catch (error) {
...
exports.RegistryFormController = RegistryFormController;
exports.registryFormController = new RegistryFormController();
