...
exports.registryImpactAnalysisEngine = exports.RegistryImpactAnalysisEngine = void 0;
const crypto_1 = require("crypto");
const impact_types_1 = require("../dto/impact.types");
class RegistryImpactAnalysisEngine {
    /**
     * Analyze the impact of a proposed change.
     * This method is READ-ONLY regarding data modification, but may read extensively.
     */
    async analyzeChange(tx, changeType, targetUrn, proposedData) {
        const impacts = [];
        let visitedNodes = [];
        // 1. Initial State Fetch
        const targetEntity = await tx.registryEntity.findUnique({ where: { urn: targetUrn } });
        if (!targetEntity && changeType !== impact_types_1.ChangeType.RELATIONSHIP_CREATE) {
        ...
        if (changeType === impact_types_1.ChangeType.ENTITY_LIFECYCLE_TRANSITION) {
            await this.analyzeLifecycleTransition(tx, targetUrn, proposedData?.state, impacts, visitedNodes);
        }
        else if (changeType === impact_types_1.ChangeType.RELATIONSHIP_CREATE) {
            await this.analyzeRelationshipCreate(tx, proposedData, impacts, visitedNodes);
        }
        else if (changeType === impact_types_1.ChangeType.RELATIONSHIP_DELETE) {
        ...
        return {
            summary: {
                blocking: impacts.filter(i => i.level === impact_types_1.ImpactLevel.BLOCKING).length,
                warning: impacts.filter(i => i.level === impact_types_1.ImpactLevel.WARNING).length,
                info: impacts.filter(i => i.level === impact_types_1.ImpactLevel.INFO).length
            },
            graph_snapshot_hash: hash,
            impacts: impacts
        };
    }
    async analyzeLifecycleTransition(tx, urn, newState, impacts, visitedNodes) {
        ...
        if (newState === 'archived' || newState === 'deprecated') {
            ...
            for (const rel of inboundRels) {
                ...
                impacts.push({
                    level: impact_types_1.ImpactLevel.BLOCKING,
                    code: impact_types_1.ImpactCode.LIFECYCLE_BREAK,
                    entity_urn: rel.id,
    ...
    async analyzeRelationshipCreate(tx, data, impacts, visitedNodes) {
        ...
        if (!def) {
            impacts.push({
                level: impact_types_1.ImpactLevel.BLOCKING,
                code: impact_types_1.ImpactCode.GRAPH_INTEGRITY_BREAK,
        ...
        if (attrs.cardinality === '1-1') {
            ...
            if (existingFrom) {
                impacts.push({
                    level: impact_types_1.ImpactLevel.BLOCKING,
                    code: impact_types_1.ImpactCode.CARDINALITY_VIOLATION,
    ...
    async analyzeRelationshipDelete(tx, relId, impacts, visitedNodes) {
        ...
        impacts.push({
            level: impact_types_1.ImpactLevel.INFO,
            code: impact_types_1.ImpactCode.NO_IMPACT,
            entity_urn: relId,
            description: 'Relationship deletion analyzed. No blocking dependencies found.',
            path: []
        });
    }
    generateGraphSnapshotHash(nodes) {
        const sorted = [...nodes].sort();
        const content = sorted.join('|');
        return (0, crypto_1.createHash)('sha256').update(content).digest('hex');
    }
}
exports.RegistryImpactAnalysisEngine = RegistryImpactAnalysisEngine;
exports.registryImpactAnalysisEngine = new RegistryImpactAnalysisEngine();
