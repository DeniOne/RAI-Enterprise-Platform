...
exports.photoCompanyResultHandler = exports.PhotoCompanyResultHandler = void 0;
const prisma_1 = require("../../config/prisma");
const qualification_service_1 = require("../../services/qualification.service");
const logger_1 = require("../../config/logger");
class PhotoCompanyResultHandler {
    /**
    ...
    async handle(eventId, payload) {
        ...
        logger_1.logger.info(`[EventFlow] Processing PHOTOCOMPANY_RESULT for user ${payload.user_id}, shift ${payload.shift_id}`);
        try {
            ...
            try {
                const proposal = await qualification_service_1.qualificationService.proposeQualificationUpgrade(payload.user_id, metrics);
                ...
                if (proposal) {
                    ...
                    logger_1.logger.info(`[EventFlow] Created QUALIFICATION_PROPOSED event for user ${payload.user_id}, proposal ${proposal.proposalId}`);
                }
            }
            catch (qualError) {
                // Non-blocking: Qualification proposal failed but we still mark event as processed
                logger_1.logger.warn(`[EventFlow] Qualification proposal failed for user ${payload.user_id}`, { error: qualError });
            }
            ...
            logger_1.logger.info(`[EventFlow] Finished processing PHOTOCOMPANY_RESULT ${eventId}`);
        }
        catch (error) {
            logger_1.logger.error(`[EventFlow] Error processing PHOTOCOMPANY_RESULT ${eventId}`, { error });
            // We DON'T mark it as processed so it can be retried
            throw error;
        }
    }
}
exports.PhotoCompanyResultHandler = PhotoCompanyResultHandler;
exports.photoCompanyResultHandler = new PhotoCompanyResultHandler();
