...
class NotificationService {
    async createNotification(data) {
        ...
        await telegram_service_1.default.sendNotification(data.userId, `*${data.title}*\n\n${data.message}`);
        return notification;
    }
    ...
    async sendTaskNotification(taskId, assigneeId, action) {
    ...
    async sendTransactionNotification(userId, amount, currency, type) {
        let title = '';
        let message = '';
        if (type === 'task_reward') {
     ...
     */
    async sendCourseCompletedNotification(userId, payload) {
     ...
     */
    async sendQualificationProposedNotification(userId, payload) {
        await this.createNotification({
            userId,
