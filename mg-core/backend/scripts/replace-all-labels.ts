...
// COMPLETE словарь замен (English -> Russian) - ВСЕ найденные labels
...
const ENTITY_FILES = [
    ...
    '11_integration/03_data_import.entity.json'
];

async function replaceAllEnglishLabels() {
    console.log('Замена ВСЕХ английских labels на русские...\n');
    ...
    for (const file of ENTITY_FILES) {
        ...
        try {
            ...
            if (fileModified) {
                ...
                console.log(`✓ ${file} — заменено ${fileReplacements} labels`);
                updated++;
            }
        } catch (err: any) {
...
replaceAllEnglishLabels().catch(console.error);
