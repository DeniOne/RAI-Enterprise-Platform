...
import { UserStatus, FoundationStatus, AdmissionStatus, ProfileCompletionStatus } from '@prisma/client';
...
export enum RegistrationStep {
    ...
    PROFILE_REG_ADDRESS = 'PROFILE_REG_ADDRESS',
    PROFILE_RES_ADDRESS = 'PROFILE_RES_ADDRESS',
    PROFILE_CONTACTS = 'PROFILE_CONTACTS',
    PROFILE_PASSPORT = 'PROFILE_PASSPORT',
    COMPLETED = 'COMPLETED'
}
...
export class EmployeeRegistrationService {
    ...
    private async handlePositionStep(ctx: any, registration: any): Promise<void> {
        ...
        const buttons = positions.map(pos =>
            Markup.button.callback(pos.name, `position_${pos.id}`)
        );
    ...
    async handlePositionCallback(ctx: any, registration: any, positionId: string): Promise<void> {
        ...
        await ctx.reply(
            `✅ *Заявка принята!*\n\n` +
            `Твои данные отправлены на проверку HR-менеджеру. Пожалуйста, ожидай уведомления о решении.`,
            { parse_mode: 'Markdown' }
        );
    }
    ...
    private async handleBirthDateStep(ctx: any, registration: any): Promise<void> {
        ...
        await ctx.reply(
            ...
            `🏠 *Шаг: Адрес прописки*\n\n` +
            `Введите адрес вашей регистрации по паспорту:`,
            { parse_mode: 'Markdown' }
        );
    }

    private async handleAddressStep(ctx: any, registration: any, step: RegistrationStep): Promise<void> {
        if (!ctx.message?.text) {
        ...
        if (isRegAddress) {
            await prisma.$executeRaw`UPDATE employee_registration_requests SET registration_address = ${address}, current_step = ${nextStep}::registration_step, updated_at = NOW() WHERE id = ${registration.id}`;
        } else {
            await prisma.$executeRaw`UPDATE employee_registration_requests SET residential_address = ${address}, current_step = ${nextStep}::registration_step, updated_at = NOW() WHERE id = ${registration.id}`;
        }
    ...
    private async handlePhoneStep(ctx: any, registration: any): Promise<void> {
        ...
        if (registration.phone === null) {
            ...
            await prisma.$executeRaw`UPDATE employee_registration_requests SET phone = ${phone}, updated_at = NOW() WHERE id = ${registration.id}`;
        ...
        if (registration.email === null) {
            ...
            await ctx.reply(
                `✅ Email сохранен!\n\n` +
                `🎫 *Шаг: Скан паспорта*\n\n` +
                `Загрузи фото разворота паспорта с фотографией (как картинку или файл):`,
                { parse_mode: 'Markdown' }
            );
        }
    }
    ...
    async completeRegistration(ctx: Context, registration: any): Promise<void> {
        ...
        const user = await prisma.user.findUnique({ where: { telegram_id: registration.telegram_id } });
        ...
        if (user && user.foundation_status === 'ACCEPTED') {
            await prisma.$transaction([
                ...
                prisma.user.update({
                    ...
                    data: {
                        ...
                        profile_completion_status: ProfileCompletionStatus.COMPLETED
                    }
                })
            ]);
            ...
            await ctx.reply(
                `✅ *Профиль заполнен!*\n\n` +
                `Ваши данные приняты. После того как вы примете Базу, процесс зачисления будет завершен.`,
                { parse_mode: 'Markdown' }
            );
        }
     ...
     */
    async startPhase3(ctx: Context, registration: any): Promise<void> {
        await prisma.$executeRaw`
            UPDATE employee_registration_requests
        ...
        await prisma.user.update({
            where: { telegram_id: registration.telegram_id },
            data: { profile_completion_status: ProfileCompletionStatus.IN_PROGRESS }
        });
        ...
        await ctx.reply(
            ...
            `🎨 *Шаг: Фото профиля*\n\n` +
            `Пожалуйста, загрузи свое фото для корпоративного профиля.`,
            { parse_mode: 'Markdown' }
        );
    }
    ...
    private async notifyAdminsAboutNewRegistration(registration: any): Promise<void> {
        ...
        const message =
            `📋 *Новая заявка на регистрацию сотрудника*` + (registration.status === 'REVIEW' ? ` (Предв. заявка)` : ` (Завершение профиля)`) + `\n\n` +
    ...
    async approveRegistration(
        ...
        const registration = await prisma.employeeRegistrationRequest.findUnique({
        ...
        await prisma.user.upsert({
            ...
            create: {
                ...
                status: UserStatus.ACTIVE,
                foundation_status: FoundationStatus.NOT_STARTED,
                profile_completion_status: ProfileCompletionStatus.LOCKED,
                department_id: finalDepartmentId,
            }
        });
